...
export const useHandler = (
  ...
  const state = reactive<{ dragLeft: number; thumbLeft: number }>({ dragLeft: 0, thumbLeft: data.value.thumbX || 0 });
  ...
  const dragEvent = (e: Event | any) => {
    ...
    const thumbX = data.value.thumbX || 0;
    ...
    const moveEvent = (e: Event | any) => {
      ...
      event.move && event.move(currentThumbX, data.value.thumbY || 0);
    ...
    const upEvent = (e: Event | any) => {
      ...
      event.confirm &&
        event.confirm(
          { position: { x: Number.parseInt(currentThumbX.toString(), 10), y: data.value.thumbY || 0 } },
          () => {
            clear();
          }
        );
