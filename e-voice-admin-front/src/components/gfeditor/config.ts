import type { EngineInterface, PluginEntry, PluginOptions } from '@/components/gfeditor/emain';
...
import Image, { ImageUploader } from '@/components/gfeditor/plugin/image';
import File, { FileUploader } from '@/components/gfeditor/plugin/file';
...
import { ToolbarPlugin, fontFamilyDefaultData } from '@/components/gfeditor/toolbar';
...
export const pluginConfig: { [key: string]: PluginOptions } = {
  ...
  [ImageUploader.pluginName]: {
    file: {
      action: `${apiHost}/common/upload/image`,
      headers: () => {
    ...
    isRemote: (src: string) => !src.includes(apiHost)
  },
  // 附件
  [FileUploader.pluginName]: {
    action: `${apiHost}/upload/file`,
    headers: () => {
      // const token = getToken();
      // const API_SECRET =ApiSecret; //从config读取的配置-打包后可以修改-变化
      const timestamp: number = Date.parse(new Date().toString()) / 1000;
  ...
  [Fontfamily.pluginName]: {
    ...
    filter: (fontfamily: string) => {
      const item = fontFamilyDefaultData.find(item =>
        fontfamily.split(',').some(name => item.value.toLowerCase().includes(name.replace(/"/, '').toLowerCase()))
      );
  ...
  [Fontsize.pluginName]: {
    ...
    filter: (fontSize: string) => {
      return ['12px', '13px', '14px', '15px', '16px', '19px', '22px', '24px', '29px', '32px', '40px', '48px'].includes(
        fontSize
      );
    }
  },
  [LineHeight.pluginName]: {
    ...
    filter: (lineHeight: string) => {
      ...
      // 不满足条件就移除掉
      return ['1', '1.15', '1.5', '2', '2.5', '3'].includes(lineHeight);
    }
  }
};
