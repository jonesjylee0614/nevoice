import type { PluginEntry, PluginOptions } from '@/components/gfeditor/emain';
import { ElementPlugin, isEngine } from '@/components/gfeditor/emain';
...
export default class<T extends AlignmentOptions = AlignmentOptions> extends ElementPlugin<T> {
  kind = 'block';
  ...
  execute(align?: 'left' | 'center' | 'right' | 'justify') {
    ...
    block.setBlocks({
      style: {
        'text-align': !align || ['left'].includes(align) ? '' : align
      }
    });
  ...
  queryState() {
    ...
    if (topTags.includes(fisrtBlock.name)) {
  ...
  onBackspace = (event: KeyboardEvent) => {
    ...
    if (block.isLastOffset(range, 'end') || !block.isFirstOffset(range, 'start') || change.blocks.length > 1) return;
    // 改变对齐
    ...
    if (align === 'right') {
      event.preventDefault();
      commandApi.execute((this.constructor as PluginEntry).pluginName, 'center');
      return false;
    }
  };
