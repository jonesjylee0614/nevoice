...
import type { NodeInterface, PluginOptions } from '@/components/gfeditor/emain';
import { $, BlockPlugin, DATA_ELEMENT, DATA_ID, Tooltip, UI, getHashId, isEngine } from '@/components/gfeditor/emain';
...
export default class<T extends HeadingOptions = HeadingOptions> extends BlockPlugin<T> {
  attributes = {
    id: '@var0'
  };
  variable = {
  ...
  tagName = ['h1', 'h2', 'h3', 'h4', 'h5', 'h6'].filter(
    name => !this.options.enableTypes || this.options.enableTypes.includes(name)
  );
    ...
    container.find(this.tagName.join(',')).each(heading => {
      ...
      if (id) {
        ...
        button.on('click', e => {
          e.preventDefault();
          e.stopPropagation();
          const url = this.options.anchorCopy ? this.options.anchorCopy(id) : `${window.location.href}/${id}`;

          if (editor.clipboard.copy(url)) {
            editor.messageSuccess('copy', language.get('copy', 'success').toString());
          } else {
  ...
  updateAnchorPosition = () => {
    ...
    const left = Math.round(rect.left - rootRect.left - button.get<Element>()!.clientWidth - 1);
    const top = Math.round(rect.top - rootRect.top + rect.height / 2 - button.get<Element>()!.clientHeight / 2);
    button.css({
      top: `${top}px`,
      left: `${left}px`
    });
  ...
  showAnchor = () => {
    const editor = this.editor;
    if (!isEngine(editor) || this.tagName.length === 0 || this.options.showAnchor === false) return;
    ...
    if (
      block.length === 0 ||
      (button.length > 0 && button.find('.data-icon-'.concat(block.name)).length === 0) ||
      !editor.isFocus()
    ) {
      button.remove();
      Tooltip.hide();
    ...
    const rect = block.get<Element>()!.getBoundingClientRect();
    const left = Math.round(rect.left - parentRect.left - button.get<Element>()!.clientWidth - 1);
    const top = Math.round(rect.top - parentRect.top + rect.height / 2 - button.get<Element>()!.clientHeight / 2);
    button.css({
      top: `${top}px`,
      left: `${left}px`
    });
  ...
  execute(type: 'h1' | 'h2' | 'h3' | 'h4' | 'h5' | 'h6' | 'p') {
    ...
    // 未启用
    if (type !== 'p' && enableTypes && !enableTypes.includes(type)) return;
  ...
  queryState() {
    ...
    const name = this.tagName.find(name => blocks.some(block => block.name === name));
    return name || '';
  }
  ...
  hotkey() {
    ...
    ].filter(item => !enableTypes || enableTypes.includes(item.key));
  }
  ...
  onBackspace = (event: KeyboardEvent) => {
    ...
    if (this.tagName.includes(block.name) && node.isEmptyWithTrim(block) && block.parent()?.isEditable()) {
      event.preventDefault();
      blockApi.setBlocks('<p />');
      return false;
    }
    const parent = block.parent();
    if (this.tagName.includes(block.name) && (!parent || !node.isBlock(parent))) {
      event.preventDefault();
