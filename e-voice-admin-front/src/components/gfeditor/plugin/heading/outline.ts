import type { NodeInterface } from '@/components/gfeditor/emain';
import { $, CARD_KEY, DATA_ID, getTextNodes, isNodeEntry } from '@/components/gfeditor/emain';
...
class Outline {
   ...
   * @param {Node} root 根节点
   */
  getHeadings(root: Element | NodeInterface, filter: { [key: string]: boolean } = this.headingTags): Array<Element> {
  ...
  getText(element: Node) {
    const nodes = getTextNodes(element, (node: Node) => {
      return Boolean(!(node as Element).getAttribute(CARD_KEY));
    });
    let text = '';
  ...
  normalize(headings: Array<Element>): Array<any> {
    ...
    data.forEach(heading => {
      if (!numberList.includes(heading.level)) {
   ...
   * @returns {Array}
   */
  getFromDom(root: Element | NodeInterface, filter: { [key: string]: boolean } = this.headingTags): Array<any> {
    try {
