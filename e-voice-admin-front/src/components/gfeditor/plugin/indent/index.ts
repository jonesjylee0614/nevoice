...
import { $, Plugin, addUnit, isEngine, removeUnit } from '@/components/gfeditor/emain';
...
export default class<T extends IndentOptions = IndentOptions> extends Plugin<T> {
    ...
    if (node.isList(block)) {
      ...
      if (padding > 0) {
        if (removeUnit(block.css(TEXT_INENT_KEY))) {
          ...
          fontSize = !fontSize || fontSize.endsWith('em') ? editor.root.css('font-size') : fontSize;
          if (fontSize.endsWith('em')) {
            fontSize = $(document.body).css('font-size');
          }
          if (!fontSize.endsWith('px')) fontSize = '16px';
          const widthMax = width > 0 ? width / removeUnit(fontSize) : maxPadding;

          newValue = Math.min(
            currentValue.endsWith('px') ? newValue / removeUnit(fontSize) : newValue,
            maxPadding,
            widthMax
          );
    ...
    return [
      {
        from: (_, styles, attributes) => {
          return Boolean(styles['padding-left']) || Boolean(attributes[TEXT_INENT_KEY]);
        },
        to: (name, styles, attributes) => {
          ...
          if (valueStr.endsWith('px')) {
            ...
            // 如果本身没有字体大小就，获取编辑器根节点默认字体大小
            fontSize = !fontSize || fontSize.endsWith('em') ? this.editor.root.css('font-size') : fontSize;
            if (fontSize.endsWith('em')) {
              fontSize = $(document.body).css('font-size');
            }
            if (!fontSize.endsWith('px')) fontSize = '16px';
  ...
  onBackspace = (event: KeyboardEvent) => {
    ...
    if (block.name === 'li') {
      ...
      (((range.collapsed && blockApi.isLastOffset(range, 'end')) || !blockApi.isFirstOffset(range, 'start')) &&
  ...
  onTab = (event: KeyboardEvent) => {
    ...
    if (range.collapsed && list.isFirst(range)) {
      event.preventDefault();
      editor.command.execute((this.constructor as PluginEntry).pluginName, 'in');
      return false;
    }
    // <p><cursor />foo</p>
    if (!range.collapsed || block.isFirstOffset(range, 'start')) {
      event.preventDefault();
      editor.command.execute((this.constructor as PluginEntry).pluginName, 'in', true);
      return false;
    }
  };
  ...
  pasteEach = (node: NodeInterface) => {
    ...
    if (node.isElement() && !node.isCard() && editor.node.isBlock(node)) {
      ...
      if (Boolean(textIndentSource) && textIndentSource.endsWith('pt')) {
        ...
        if (textIndent) {
          ...
          newValue = Math.min(
            textIndent.endsWith('px') ? newValue / removeUnit(fontSize) : newValue,
            maxPadding,
            widthMax
          );
          if (newValue <= 0) node.css(TEXT_INENT_KEY, '');
          else {
            node.css(TEXT_INENT_KEY, addUnit(newValue > 0 ? newValue : 0, 'em'));
          }
        }
      }
    }
