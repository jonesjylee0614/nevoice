import type { NodeInterface, PluginOptions, RangeInterface } from '@/components/gfeditor/emain';
import { $, DATA_ELEMENT, Plugin, UI, isEngine, isNode } from '@/components/gfeditor/emain';
import './index.css';

export interface PaintformatOptions extends PluginOptions {
  removeCommand?: string | ((range: RangeInterface) => void);
  paintBlock?: (currentBlocl: NodeInterface, block: NodeInterface) => boolean | void;
}

const PAINTFORMAT_CLASS = 'data-paintformat-mode';

export default class<T extends PaintformatOptions = PaintformatOptions> extends Plugin<T> {
  ...
  onBeforeCommandExecute = (name: string) => {
    if (name !== 'paintformat' && !this.isFormat && this.event) {
  ...
  onContainerMouseUp = (e: MouseEvent) => {
    ...
    this.isFormat = false;
    if (this.type === 'single') this.removeActiveNodes(editor.container[0].ownerDocument!);
  };
  ...
  paintFormat(activeMarks: NodeInterface[], activeBlocks?: NodeInterface[]) {
    ...
    if (range.collapsed) {
      let dummy = $(`<img ${DATA_ELEMENT}="${UI}" role="format-dummy" style="display: none;" />`);
    ...
    activeBlocks.forEach(block => {
      ...
      if (block.name !== currentBlock.name) {
        ...
        if (block.name === 'p') {
          ...
          const plugin = blockApi.findPlugin(block);
          const curPlugin = blockApi.findPlugin(currentBlock.name === 'li' ? currentBlock.parent()! : currentBlock);
  ...
  queryState() {
    return Boolean(this.activeMarks);
  }
