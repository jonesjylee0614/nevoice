...
import { $, DATA_ELEMENT, Position, UI, isMobile } from '@/components/gfeditor/emain';
...
class Toolbar {
  ...
  show(target: NodeInterface, forceEdit?: boolean) {
    if (this.target?.equal(target) && Boolean(this.root?.parent()?.length)) return;
    this.target = target;
    this.create();
    ...
    const name = (!href || forceEdit) && !this.engine.readonly ? 'am-link-editor' : 'am-link-preview';
