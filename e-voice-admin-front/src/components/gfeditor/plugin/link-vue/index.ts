...
import { $, InlinePlugin, isEngine } from '@/components/gfeditor/emain';
...
export default class<T extends LinkOptions = LinkOptions> extends InlinePlugin<T> {
    ...
    if (isEngine(editor)) {
      if (this.options.enableToolbar !== false) {
        this.toolbar = new Toolbar(editor, {
          onConfirm: this.options.onConfirm as any
        });
      }
  ...
  query = () => {
    ...
    if (inlineNode && inlineNode.length > 0 && !inlineNode.isCard()) {
      const range = change.range.get();
      if (range.collapsed || (inlineNode.contains(range.startNode) && inlineNode.contains(range.endNode))) {
  ...
  pasteHtml = (child: NodeInterface) => {
    if (child.isText()) {
      ...
      if (/^https?:\/\/\S+$/.test(text.toLowerCase().trim()) && inline.closest(child).equal(child)) {
