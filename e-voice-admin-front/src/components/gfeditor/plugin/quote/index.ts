...
import { $, BlockPlugin, isEngine } from '@/components/gfeditor/emain';
import './index.css';
...
export default class<T extends QuoteOptions = QuoteOptions> extends BlockPlugin<T> {
  ...
  onBackspace = (event: KeyboardEvent) => {
    ...
    if (parentBlock && parentBlock.name === 'blockquote' && node.isBlock(block)) {
      event.preventDefault();
      if (block.prevElement()) {
        change.mergeAfterDelete(block);
      } else if (node.isEmpty(parentBlock)) {
  ...
  onEnter = (event: KeyboardEvent) => {
    ...
    if (parent?.name === this.tagName && block.name === 'p' && block.nextElement()) {
      event.preventDefault();
