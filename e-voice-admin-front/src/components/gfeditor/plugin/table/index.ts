...
import type { NodeInterface, SchemaBlock, SchemaInterface } from '@/components/gfeditor/emain';
import {
  $,
  CARD_KEY,
  CARD_VALUE_KEY,
...
import TableComponent, { Helper, Template } from './component';
import locales from './locale';
import type { TableOptions, TableValue } from './types';
...
class Table<T extends TableOptions = TableOptions> extends Plugin<T> {
  ...
  onCopy = (event: ClipboardEvent) => {
    ...
    const component = card.find<TableValue, TableComponent>(range.commonAncestorNode, true);
  ...
  pasteSchema = (schema: SchemaInterface) => {
    (schema.data.blocks as SchemaBlock[]).forEach(blockSchema => {
      ...
      if (!blockSchema.allowIn.includes('td')) {
  ...
  execute(rows?: number, cols?: number): void {
    ...
    editor.card.insert<TableValue>(TableComponent.cardName, {
      rows: rows || 3,
      cols: cols || 3,
      overflow: Boolean(this.options.overflow)
    });
  }
  ...
  convertToPX(value: string) {
    ...
    if (match && match[2] === 'pt') {
      return `${String(Math.round((Number.parseInt(match[1], 10) * 96) / 72))}px`;
    }
    return value;
  }

  pasteEach = (node: NodeInterface) => {
    if (node.name === 'div' && node.hasClass('editor-table-wrapper') && node.first()?.name === 'table') {
  ...
  pasteHtml = (root: NodeInterface) => {
    const editor = this.editor;
    if (!isEngine(editor)) return;
    const clearWH = (node: NodeInterface, type: 'width' | 'height' = 'width') => {
      const dataWidth = node.attributes('data-width');
      const width = dataWidth || node.css(type);
    ...
    tables.each((_, index) => {
      ...
      const background = node?.css('background') || node?.css('background-color');
      ...
      trs.each(child => {
        ...
        if (tds && tds?.length > 0) {
          const spans = tds.toArray().map(td => (td[0] as HTMLTableCellElement).rowSpan);
  ...
  parseHtml = (root: NodeInterface, callback?: (node: NodeInterface, value: TableValue) => NodeInterface) => {
    const editor = this.editor;
    const results: NodeInterface[] = [];
    root
      .find(`[${CARD_KEY}="${TableComponent.cardName}"],[${READY_CARD_KEY}="${TableComponent.cardName}"]`)
      .each(tableNode => {
        const node = $(tableNode);
        let table = node.find('table');
        const value = decodeCardValue<TableValue>(node.attributes(CARD_VALUE_KEY));
        if (table.length === 0) {
        ...
        table = $(
          `<div class="editor-table-wrapper" style='width:100%;overflow:auto;'  data-id='${tableId}'  data-table-no-border='${Boolean(
            table.attributes('data-table-no-border')
          )}'>`
        ).append(table);
