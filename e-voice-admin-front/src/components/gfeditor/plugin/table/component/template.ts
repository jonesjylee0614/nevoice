...
import type { TableInterface, TableMenu, TableValue, TemplateInterface } from '../types';
...
class Template implements TemplateInterface {
  ...
  renderRowsHeader(rows: number) {
    return (
      ...
      `<div class="${ROW_ADD_BUTTON_CLASS_NAME}"><span class="data-icon data-icon-plus"></span><div class="${ROW_ADD_BUTTON_SPLIT_CLASS_NAME}"></div></div>${`<div class="${ROWS_HEADER_ITEM_CLASS_NAME}" draggable="true">
  ...
  renderColsHeader(cols: number) {
    return (
      ...
      `<div class="${COL_ADD_BUTTON_CLASS_NAME}"><span class="data-icon data-icon-plus"></span><div class="${COL_ADD_BUTTON_SPLIT_CLASS_NAME}"></div></div>${`<div class="${COLS_HEADER_ITEM_CLASS_NAME}" draggable="true">
   ...
   */
  htmlEdit({ rows, cols, html, noBorder, overflow }: TableValue, menus: TableMenu): string {
    cols = cols === -Infinity ? 1 : cols;
    rows = rows === -Infinity ? 1 : rows;
    cols = cols === Infinity ? 10 : cols;
    rows = rows === Infinity ? 10 : rows;
    const tds = `<td ${DATA_TRANSIENT_ATTRIBUTES}="table-cell-selection">${this.getEmptyCell()}</td>`.repeat(cols);
    ...
    const tableHeader = `<div ${DATA_ELEMENT}="${UI}" class="${HEADER_CLASS_NAME}"><div class="${HEADER_CLASS_NAME}-item"></div></div>`;
    ...
    menuBar = [`<div ${DATA_ELEMENT}="${UI}" class="${MENUBAR_CLASS_NAME}">${menuBar.join('')}</div>`];
    ...
    return `<div ${DATA_TRANSIENT_ATTRIBUTES}="*" class="${TABLE_WRAPPER_CLASS_NAME} ${
      ...
      cols
    )}${table}${placeholder}${tableHighlight}</div>${this.renderRowsHeader(rows)}${menuBar}</div>`;
  }
