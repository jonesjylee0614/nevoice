...
import type { HelperInterface, TableModel, TableModelCol, TableModelEmptyCol, TableOptions } from '../types';
...
class Helper implements HelperInterface {
  ...
  isEmptyModelCol(model: TableModelCol | TableModelEmptyCol): model is TableModelEmptyCol {
  ...
  getTableModel(table: NodeInterface): TableModel {
    const model: Array<Array<TableModelCol | TableModelEmptyCol>> = [];
  ...
  normalize(table: NodeInterface) {
    ...
    if (cols.length !== 0) {
      for (let c = cols.length - 1; c >= 0; c--) {
        ...
        if (_width) {
          cols.eq(c)?.attributes('width', Number.parseInt(_width, 10));
        }
      ...
      const widths = (function (table) {
        ...
        tds.each((_, index) => {
          ...
          if (!$td) return;
          let colWidth: string | Array<string> = $td.attributes('data-colwidth');
          ...
          if (colWidth) {
            ...
            tdWidth = Number.parseInt(tdWidth, 10) / tdColSpan;
          }
          for (let o = 0; tdColSpan > o; o++) {
            if (colWidth && colWidth[o]) {
              widthArray.push(Number.parseInt(colWidth[o], 10));
            } else if (tdWidth) {
              widthArray.push(Number.parseInt(tdWidth.toString(), 10));
            } else {
      ...
      for (let f = 0; model.cols > f; f++) {
        ...
        if (widths[f]) {
          (node as HTMLElement).setAttribute('width', (widths[f] || '').toString());
        }
    ...
    trs.each((_, index) => {
      ...
      if (!$tr) return;
      let height = Number.parseInt($tr.css('height'), 10);
      height ||= this.#editor.plugin.findPlugin<TableOptions>('table')?.options.rowMinHeight || 0;
    ...
    tds.each((_, index) => {
      ...
      if (editableElement.length === 0) {
        ...
        tdNode.empty();
        tdNode.append(Template.EmptyCell(!isEngine(this.#editor) || this.#editor.readonly));
  ...
  copyTo(from: NodeInterface | Node, to: NodeInterface | Node) {
    ...
    if (editableElement.length === 0) {
      to.html(Template.EmptyCell(!isEngine(this.#editor) || this.#editor.readonly));
  ...
  copyHTML(html: string) {
    this.clipboard = {
      html,
      text: $(html).get<HTMLElement>()?.textContent || ''
    };
  }
  /**
   * 获取复制的数据
  ...
  trimBlankSpan(node: NodeInterface) {
    ...
    while (
      ...
      (node[i] as HTMLElement).textContent?.trim() === ''
    ) {
      i++;
    }
    ...
    while (
      ...
      (node[j] as HTMLElement).textContent?.trim() === ''
    ) {
      j--;
    }
  ...
  /**
   * table 结构标准化，补齐丢掉的单元格和行 场景1. number 拷贝过来的 html 中，如果这一行没有单元格，就会省掉 tr，渲染的时候会有问题 场景2.
   * 从网页中鼠标随意选取表格中的一部分，会丢掉没有选中的单元格，需要补齐单元格
   *
   * @param {NodeInterface} table 表格 Dom
   * @returns {NodeInterface} 修复过的 table dom
   */
  normalizeTable(table: NodeInterface) {
    ...
    if (Number.parseInt(width, 10) === 0) {
      ...
      table.css('width', `${Number.parseInt(width, 10)}px`);
    ...
    if (cols.length !== 0) {
      for (let c = cols.length - 1; c >= 0; c--) {
        ...
        if (_width && !_width.endsWith('%')) {
          const widthValue = Number.parseInt(_width, 10);
          if (!Object.is(widthValue, Number.NaN)) {
      ...
      const widths = (function (table) {
        ...
        tds.each((_, index) => {
          const tdNode = tds.eq(index);
          if (!tdNode) return;
          let colWidth: string | Array<string> = tdNode.attributes('data-colwidth');
