...
import type { EditorInterface, NodeInterface } from '@/components/gfeditor/emain';
import {
  $,
  CardActiveTrigger,
  getComputedStyle,
...
class ControllBar extends EventEmitter2 implements ControllBarInterface {
  ...
  constructor(editor: EditorInterface, table: TableInterface, options: ControllOptions) {
    super();
    this.table = table;
    this.editor = editor;
  ...
  renderRowBars(refershSize: boolean = true) {
    ...
    this.rowDeleteButton = this.rowsHeader?.find(Template.ROW_DELETE_BUTTON_CLASS);
    // 行增加按钮
    this.rowAddButton?.removeAllEvents();
    this.rowAddButton = this.rowsHeader?.find(Template.ROW_ADD_BUTTON_CLASS);
    this.rowAddButtonSplit = this.rowAddButton?.find(Template.ROW_ADD_BUTTON_SPLIT_CLASS);
    ...
    this.rowAddButton
      ...
      .on('mousedown', (event: MouseEvent) => {
        event.preventDefault();
        this.table.command.insertRowAt(this.moveRowIndex, 1, this.rowAddAlign !== 'down');
      });
    if (refershSize) {
      ...
      const rowTrigger = this.rowsHeader?.find(Template.ROWS_HEADER_TRIGGER_CLASS);
  ...
  renderColBars(refershSize: boolean = true) {
    ...
    this.colDeleteButton = this.table.wrapper?.find(Template.COL_DELETE_BUTTON_CLASS);
    // 列增加按钮
    this.colAddButton?.removeAllEvents();
    this.colAddButton = this.colsHeader?.find(Template.COL_ADD_BUTTON_CLASS);
    this.colAddButtonSplit = this.colAddButton?.find(Template.COL_ADD_BUTTON_SPLIT_CLASS);
    ...
    this.colAddButton
      ...
      .on('mousedown', (event: MouseEvent) => {
        event.preventDefault();
        if (this.moveColIndex > -1) this.table.command.insertColAt(this.moveColIndex, 1, this.colAddAlign !== 'right');
      });
  ...
  renderColSize() {
    ...
    const newHeight = (tableModel?.height || 0) + (this.colsHeader?.height() || 0);
    colTrigger?.each(col => {
  ...
  bindEvents() {
    ...
    this.viewport?.on(isMobile ? 'touchstart' : 'mousedown', (event: MouseEvent) => {
      if (!event.target) return;
      const targetNode = $(event.target);
      if (!isEngine(editor) || !event.target || !this.viewport?.equal(targetNode)) return;
      event.preventDefault();
  ...
  onMouseMoveColsHeader(event: MouseEvent) {
    ...
    const splitHeight = (this.table.selection.tableModel?.height || 0) + itemNode.height() + 4;
  ...
  onMouseMoveRowsHeader(event: MouseEvent) {
    ...
    const splitWidth = (this.table.selection.tableModel?.width || 0) + itemNode.width() + 4;
    const width = Math.min(viewportElement.offsetWidth + 4, splitWidth);
  ...
  activeHeader() {
    ...
    } else if (tableHeaderElement?.classList.contains('selected')) {
    ...
    if (allCol && !allRow) {
      ...
      if (tr) {
        ...
        this.rowDeleteButton?.css('top', `${top - this.rowDeleteButton.height()}px`);
      }
    } else {
  ...
  onChangeColWidth(event: MouseEvent | TouchEvent) {
    ...
    // 鼠标移动宽度
    let width = (event instanceof MouseEvent ? event.clientX : event.touches[0].clientX) - this.dragging.x;
    // 获取合法的宽度
    const colWidth = Math.max(this.COL_MIN_WIDTH, this.changeSize.width + width);
    // 需要移动的宽度
    ...
    // 设置列头宽度
    this.tableRoot?.find('col').eq(this.changeSize.index)?.attributes('width', colWidth);
    // 设置表格宽度
    this.tableRoot?.css('width', `${tableWidth}px`);
  }
  ...
  onChangeRowHeight(event: MouseEvent | TouchEvent) {
    ...
    this.tableRoot?.find('tr').eq(this.changeSize.index)?.css('height', `${rowHeight}px`);
  }
  ...
  onDragStartColsHeader = (event: DragEvent) => {
    ...
    colBar
      .find('.drag-info')
      .html(
        this.editor.language.get<string>('table', 'draggingCol').replace('$data', this.draggingHeader.count.toString())
      );
  ...
  onDragStartRowsHeader = (event: DragEvent) => {
    ...
    rowBar
      .find('.drag-info')
      .html(
        this.editor.language.get<string>('table', 'draggingRow').replace('$data', this.draggingHeader.count.toString())
      );
    ...
    if (element.closest(Template.COLS_HEADER_CLASS).length > 0) {
      ...
      const left =
        this.draggingHeader.index !== colBars.length
          ? colBars.eq(this.draggingHeader.index)!.get<HTMLElement>()!.offsetLeft + 2
          : colBars.eq(this.draggingHeader.index - 1)!.get<HTMLElement>()!.offsetLeft +
            colBars.eq(this.draggingHeader.index - 1)!.get<HTMLElement>()!.offsetWidth +
            2;
      const viewportElement = this.viewport?.get<HTMLElement>()!;
      ...
      const height = (this.table.selection.tableModel?.height || 0) + colBars.height();
      ...
      const top =
        this.draggingHeader.index !== rowBars.length
          ? rowBars.eq(this.draggingHeader.index)!.get<HTMLElement>()!.offsetTop + 2
          : rowBars.eq(this.draggingHeader.index - 1)!.get<HTMLElement>()!.offsetTop +
            rowBars.eq(this.draggingHeader.index - 1)!.get<HTMLElement>()!.offsetHeight -
            2;
      ...
      this.placeholder?.css('top', `${top + removeUnit(paddingTop || '0') + (colBars?.height() || 0) - 2}px`);
      this.placeholder?.css('display', 'block');
    }
  }
  ...
  onDragCol = (event: DragEvent) => {
    ...
    if (colBar.length > 0) {
      ...
      const _dropCol = event.offsetX > colBar.get<HTMLElement>()!.offsetWidth / 2 ? currentCol + 1 : currentCol;
    ...
    const [_, col] = this.table.selection.getCellPoint(td);
    let dropCol = col;
    let _passWidth = 0;
  ...
  onDragRow = (event: DragEvent) => {
    ...
    if (rowBar.length > 0) {
      ...
      const _dropRow = event.offsetY > rowBar.get<HTMLElement>()!.offsetHeight / 2 ? currentRow + 1 : currentRow;
  ...
  removeCol(index: number) {
    ...
    this.colsHeader?.css('width', `${headerElement.offsetWidth - item.offsetWidth}px`);
  ...
  showContextMenu(event: MouseEvent) {
    ...
    menuItems.each(menu => {
      ...
      if (this.getMenuDisabled(action)) {
          ...
          inputNode
            .on('blur', () => {
              inputElement.value = Math.min(
                Number.parseInt(inputElement.value, 10) || 1,
                this.MAX_INSERT_NUM
              ).toString();
            })
            .on('keydown', event => {
        ...
        const selectArea = selection.getSelectArea();
        const isInsertCol = ['insertColLeft', 'insertColRight'].includes(action);
        const isInsertRow = ['insertRowUp', 'insertRowDown'].includes(action);
        if (isInsertCol) {
    ...
    const wrapperRect = this.table.wrapper.get<HTMLElement>()!.getBoundingClientRect();
  ...
  handleTriggerMenu(menu: NodeInterface) {
    if (!menu.hasClass('disabled')) {
      const action = menu.attributes('data-action') as any;
      const inputNode = menu.find(`input${Template.MENUBAR_ITEM_INPUT_CALSS}`);
      let args: any;
      if (inputNode.length > 0) {
        args = Math.min(Number.parseInt(inputNode.get<HTMLInputElement>()?.value || '1', 10), this.MAX_INSERT_NUM);
      }
  ...
  drawBackgroundColor(color?: string) {
    ...
    selection.each(cell => {
      if (!helper.isEmptyModelCol(cell) && cell.element) {
        if (!color || color === 'transparent') cell.element.style.removeProperty('background-color');
  ...
  setAlign(align?: 'top' | 'middle' | 'bottom') {
    ...
    selection.each(cell => {
      if (!helper.isEmptyModelCol(cell) && cell.element) {
        if (!align || align === 'top') cell.element.style.removeProperty('vertical-align');
