...
import { $, EDITABLE_SELECTOR, isEngine, isHotkey, isMobile, isNode, removeUnit } from '@/components/gfeditor/emain';
import type {
  TableInterface,
  TableModel,
  TableModelCol,
...
class TableSelection extends EventEmitter2 implements TableSelectionInterface {
    ...
    tableModel.table.forEach((cols, row) => {
      cols.forEach((cell, col) => {
        if (!this.table.helper.isEmptyModelCol(cell)) {
          ...
          } else if (cell.element.classList.contains('table-last-row')) cell.element.classList.remove('table-last-row');
        }
      });
    });
  }
  ...
  bindEvents() {
    ...
    this.table.wrapper?.on('mousedown', this.onTdMouseDown).on('keydown', this.onKeydown);
  }

  unbindEvents() {
    document.removeEventListener('keydown', this.onShiftKeydown);
    document.removeEventListener('keyup', this.onShiftKeyup);
    this.table.wrapper?.off('mousedown', this.onTdMouseDown).off('keydown', this.onKeydown);
  }
  ...
  each(fn: (cell: TableModelCol | TableModelEmptyCol, row: number, col: number) => void, reverse: boolean = false) {
    const { tableModel } = this;
    if (!tableModel) return;
    const { begin, end } = this.getSelectArea();
    ...
    const col = this.tableModel?.table[row]?.findIndex(cell =>
      td.equal(
        (this.table.helper.isEmptyModelCol(cell)
          ? (this.tableModel?.table[cell.parent.row][cell.parent.col] as TableModelCol)
          : cell
        ).element!
      )
    );
  ...
  selectCol(begin: number, end: number = begin) {
    ...
    if (this.isShift) {
      ...
      if (this.beginAllCol) {
        ...
        const [_, col] = this.getCellPoint(this.prevMouseDownTd);
        begin = col;
  ...
  selectRow(begin: number, end: number = begin) {
    ...
    if (this.isShift) {
      ...
      if (this.beginAllRow) {
        ...
        const [row, _] = this.getCellPoint(this.prevMouseDownTd);
        begin = row;
  ...
  select(begin: { row: number; col: number }, end: { row: number; col: number }) {
    if (!this.tableModel) return;
    ...
    this.tableRoot?.find('td[table-cell-selection]').removeAttributes('table-cell-selection');

    const calc = (): Record<'beginCol' | 'beginRow' | 'endRow' | 'endCol', number> => {
      if (!this.tableModel)
        return {
          beginCol,
          beginRow,
  ...
  onTdMouseDown = (event: MouseEvent | TouchEvent) => {
    ...
    const isSelection = Boolean(td.attributes('table-cell-selection'));
    // shift 多选
    if (this.isShift) {
      ...
      if (this.prevMouseDownTd) {
        ...
        if (this.tableModel) {
          ...
          if (!this.table.helper.isEmptyModelCol(tdModel) && tdModel.element) this.prevMouseDownTd = $(tdModel.element);
        }
      }
    ...
    if (event instanceof MouseEvent && event.button !== 2) this.select({ row, col }, { row, col });

    // 点击单元格空白处，聚焦内部编辑区域
    if (
      target.name === 'td' &&
      (isSelection || !range.startNode.closest('td').equal(td) || !range.endNode.closest('td').equal(td))
    ) {
      if (event instanceof MouseEvent && event.button === 2 && Boolean(target.attributes('table-cell-selection'))) {
        return;
      }
    ...
    if (event instanceof MouseEvent && event.button === 2) {
      ...
      if (editableElement.length > 0) {
        ...
        if (block) {
          ...
          // 节点不可见不处理
          if (
            (block.get<HTMLElement>()?.offsetTop || 0) + (block.get<Element>()?.clientHeight || 0) >
  ...
  addDragEvent() {
    ...
    document.addEventListener(isMobile ? 'touchend' : 'mouseup', this.removeDragEvent);
    document.addEventListener(isMobile ? 'touchmove' : 'mousemove', this.onDragMove);
  }
  ...
  selectUp(event: KeyboardEvent, td: NodeInterface) {
    ...
    if (count === 0) {
      ...
      if (rangeRect.width !== 0 && rangeRect.height === 0 && rangeRect.top - (tdRect?.top || 0) > 10) return;
    }
  ...
  selectDown(event: KeyboardEvent, td: NodeInterface) {
    ...
    if (count === 0) {
      ...
      const rangeRect = range.getBoundingClientRect();
      const tdRect = td.find(EDITABLE_SELECTOR).getBoundingClientRect();
      if (rangeRect.width !== 0 && rangeRect.height === 0 && (tdRect?.bottom || 0) - rangeRect.bottom > 10) return;
    }
  ...
  getSelectionHtml(all: boolean = false) {
    ...
    for (let c = begin.col; c <= end.col; c++) {
      ...
      tableWidth += Number.parseInt(colElement.width, 10);
    }
  ...
  isRowSelected() {
    return Boolean(this.selectArea) && this.selectArea!.allRow;
  }

  isColSelected() {
    return Boolean(this.selectArea) && this.selectArea!.allCol;
  }

  isTableSelected() {
    return Boolean(this.selectArea) && this.selectArea!.allCol && this.selectArea!.allRow;
  }
  ...
  showHighlight(area: TableSelectionArea) {
      ...
      for (let col = fBeginCol; col <= fEndCol; col++) {
        ...
        if (this.table.helper.isEmptyModelCol(cell)) {
          ...
          if (!this.table.helper.isEmptyModelCol(parent)) {
            if (parent.rowSpan > 1 && end.row < parent.rowSpan - 1 + cell.parent.row)
              end.row = parent.rowSpan - 1 + cell.parent.row;
            if (parent.colSpan > 1 && end.col < parent.colSpan - 1 + cell.parent.col)
              end.col = parent.colSpan - 1 + cell.parent.col;
          }
        } else if (!this.table.helper.isEmptyModelCol(cell)) {
          if (cell.rowSpan > 1 && end.row < cell.rowSpan - 1 + row) end.row = cell.rowSpan - 1 + row;
          if (cell.colSpan > 1 && end.col < cell.colSpan - 1 + col) end.col = cell.colSpan - 1 + col;
        }
      }
    }
    ...
    const sLeft =
      removeUnit(this.table.wrapper?.find('.data-scrollbar')?.css('left') || '0') +
      removeUnit(this.table.wrapper?.css('margin-left') || '0');
    left += sLeft;

    const headerHeight = this.colsHeader?.find(Template.COLS_HEADER_ITEM_CLASS).get<HTMLElement>()?.offsetHeight || 0;
    top += headerHeight;
