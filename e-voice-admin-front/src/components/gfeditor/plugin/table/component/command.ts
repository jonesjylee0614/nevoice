...
import type { ClipboardData, EditorInterface, NodeInterface } from '@/components/gfeditor/emain';
import { $, DATA_ID, DATA_TRANSIENT_ATTRIBUTES, Parser, isEngine } from '@/components/gfeditor/emain';
import type { TableCommandInterface, TableInterface } from '../types';
...
class TableCommand extends EventEmitter2 implements TableCommandInterface {
  ...
  insertColAt(index: number, count: number, isLeft?: boolean, widths?: number | Array<number>, ...args: any) {
  ...
  insertCol(position?: 'left' | 'end' | 'right', count: number = 1, ...args: any) {
    const { selection, wrapper } = this.table;
    const { tableModel } = selection;
      ...
      for (let _c = selectArea.end.col; _c >= selectArea.begin.col; _c--) {
        ...
        if (helper.isEmptyModelCol(tdModel)) {
          ...
          if (!helper.isEmptyModelCol(parentTd) && tdModel.parent.col < selectArea.begin.col) {
            const colRemoved = Math.min(count, tdModel.parent.col + parentTd.colSpan - selectArea.begin.col);
            if (parentTd.element) {
              (parentTd.element as HTMLTableDataCellElement).colSpan = parentTd.colSpan - colRemoved;
            }
          }
          continue;
        } else {
          if (tdModel.isMulti) {
            ...
            if (cutHeader) {
              ...
              const td = trs.eq(r)?.get<HTMLTableRowElement>()?.insertCell(insertIndex);
              if (!td) return;
  ...
  insertRowAt(index: number, count: number, isUp?: boolean, ...args: any) {
    ...
    const insertTdProps: Array<{ tdBase: HTMLTableCellElement }> = [];
    ...
    trModel.forEach((tdModel, c) => {
      if (!helper.isEmptyModelCol(tdModel) && tdModel.isMulti) {
        if (!isUp && tdModel.rowSpan > 1 && insertRow <= baseRow + tdModel.rowSpan - 1) {
          (tdModel.element as HTMLTableCellElement).rowSpan = tdModel.rowSpan + count;
        } else {
          insertTdProps.push({
      ...
      if (helper.isEmptyModelCol(tdModel)) {
        ...
        if (
          ...
          (parentTd.element as HTMLTableCellElement).rowSpan = parentTd.rowSpan + count;
        } else if (!helper.isEmptyModelCol(parentTd) && tdModel.parent.row < baseRow && tdModel.parent.col === c) {
          insertTdProps.push({
  ...
  insertRow(position?: 'up' | 'end' | 'down', count: number = 1, ...args: any) {
    ...
    const insertRow = isUp ? baseRow : baseRow;
      ...
      trModel.forEach((tdModel, c) => {
        if (!helper.isEmptyModelCol(tdModel) && tdModel.isMulti && tdModel.rowSpan > 1) {
          ...
          if (r + tdModel.rowSpan - 1 > end.row) {
            ...
            const td = (trs[end.row + 1] as HTMLTableRowElement).insertCell(insertIndex);
        ...
        if (helper.isEmptyModelCol(tdModel)) {
          ...
          if (!helper.isEmptyModelCol(parentTd) && tdModel.parent.row < begin.row) {
            const _cutCount = Math.min(count, tdModel.parent.row + parentTd.rowSpan - begin.row);
            (parentTd.element as HTMLTableDataCellElement).rowSpan = parentTd.rowSpan - _cutCount;
          }
        }
      });
    };
  ...
  hasCopyData = () => {
    return Boolean(this.table.helper.getCopyData());
  };
  ...
  mergeCell(...args: any) {
    ...
    selection.each((tdModel, r, c) => {
      ...
      if (tdModel.element) {
        ...
        if (tdModel.element.textContent.trim() !== '') {
          content.unshift($(tdModel.element).find(Template.TABLE_TD_CONTENT_CLASS).html());
        }
      ...
      for (let r = rows - 1; r >= row; r--) {
        ...
        for (let c = cols - 1; c >= col; c--) {
          ...
          if (!helper.isEmptyModelCol(tdModel) && tdModel.isMulti) {
            ...
            _td2.setAttribute(DATA_TRANSIENT_ATTRIBUTES, 'table-cell-selection');
