...
class TableComponent<V extends TableValue = TableValue> extends Card<V> implements TableInterface<V> {
  readonly contenteditable: string[] = [`div${Template.TABLE_TD_CONTENT_CLASS}`];
  ...
  colMinWidth = this.editor.plugin.findPlugin<TableOptions>('table')?.options.colMinWidth || 40;
  rowMinHeight = this.editor.plugin.findPlugin<TableOptions>('table')?.options.rowMinHeight || 35;
  maxInsertNum = this.editor.plugin.findPlugin<TableOptions>('table')?.options.maxInsertNum || 30;
  ...
  init() {
    ...
    if (isEngine(editor)) {
      ...
      editor.event.listeners['keydown:tab'].unshift((event: KeyboardEvent) => {
        ...
        if (
          td.length > 0 &&
          (block.isLastOffset(range, 'end') || (closestBlock.name !== 'li' && node.isEmptyWidthChild(closestBlock)))
        ) {
          let next = td.next();
          if (!next) {
      ...
      editor.on('keydown:down', event => {
        ...
        const tdRect = contentElement.get<HTMLElement>()!.getBoundingClientRect();
  ...
  toolbar(): (ToolbarItemOptions | CardToolbarItemOptions)[] {
    ...
    const getItems = (): (ToolbarItemOptions | CardToolbarItemOptions)[] => {
      ...
      const language = editor.language.get('table') as any;
      const funBtns: Array<ToolbarItemOptions | CardToolbarItemOptions> = [
        ...
        },
        {
          key: 'border',
          type: 'button',
          title: super.getValue()?.noBorder ? language.showBorder : language.noBorder,
      ...
      const toolbars: Array<ToolbarItemOptions | CardToolbarItemOptions> = [
        ...
        },
        {
          ...
          onClick: () => {
            ...
            editor.messageSuccess('copy', editor.language.get<string>('copy', 'success'));
          }
        },
        {
          key: 'delete',
  ...
  drawBackground?(node: NodeInterface, range: RangeInterface): DOMRect | void | false | RangeInterface[] {
  ...
  onChange = (trigger: 'remote' | 'local' = 'local') => {
    const editor = this.editor;
    if (isEngine(editor) && trigger === 'local' && editor.ot.isStopped()) return;
  ...
  overflow(max: number) {
    ...
    if (overflowWidth > 0 && !this.isMaximize) {
      this.wrapper?.css('margin-right', `-${overflowWidth > max ? max : overflowWidth}px`);
  ...
  updateScrollbar = () => {
    ...
    const hideHeight = (this.wrapper?.getBoundingClientRect()?.bottom || 0) - (this.wrapper?.getViewport().bottom || 0);
  ...
  didRender() {
    ...
    this.command.init();
    if (!isEngine(editor) || editor.readonly) this.toolbarModel?.setOffset([0, 0]);
  ...
  remoteRefresh() {
    if (!this.wrapper || this.wrapper.length === 0 || !this.wrapper[0].parentNode) return;
    // 重新绘制列头部和行头部
    ...
    colElements.forEach((colElement, index) => {
      ...
      colItems.eq(index)?.css('width', `${Math.max(Number.parseInt(width, 10), this.colMinWidth)}px`);
    });
    ...
    rowElements.forEach((rowElement, index) => {
      rowItems.eq(index)?.css('height', Math.max(Number.parseInt(rowElement.css('width'), 10), this.rowMinHeight));
    });
    // this.conltrollBar.refresh();
    this.scrollbar?.refresh();
    ...
    this.remoteRefreshTimeout = setTimeout(() => {
      ...
      this.wrapper?.find(EDITABLE_SELECTOR).each(editableNode => {
        ...
        if (!editableElement.hasAttribute(DATA_CONTENTEDITABLE_KEY)) {
          editableElement.setAttribute(DATA_CONTENTEDITABLE_KEY, this.template.isReadonly ? 'false' : 'true');
        }
      });
    }, 10);
  }
  ...
  render() {
    ...
    // 重新渲染
    if (this.wrapper && this.wrapper.length > 0 && Boolean(this.wrapper[0].parentNode)) {
      this.remoteRefresh();
      return;
    }
    const value = this.getValue();
    ...
    if (value.width) this.wrapper.find('table').css('width', `${value.width}px`);
    return this.wrapper;
  }
