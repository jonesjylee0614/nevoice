...
import { $, DATA_ELEMENT, Position, TRIGGER_CARD_ID, UI, isMobile } from '@/components/gfeditor/emain';
...
class Color {
  ...
  constructor(editor: EditorInterface, cardId: string, options: Options) {
    ...
    this.#button.find('.table-color-dropdown-button-text').on('mousedown', (event: MouseEvent) => {
      event.preventDefault();
  ...
  select(color: string) {
    ...
    this.#button.find('svg').css('margin-bottom', color === 'transparent' ? -5 : -3);
  }
  ...
  render() {
    ...
    const getItem = (color: { color: string; border?: string }, display?: boolean) => {
      // 接近白色的颜色，需要添加一个边框。不然看不见
      const state = this.toState(color.color || '#FFFFFF');
      const needBorder = ['#ffffff', '#fafafa', 'transparent'].includes(state.hex);
      const item = $(`<span class="data-table-color-tool-item${
        ...
        color.border ? `;${!needBorder ? `border:1px solid ${color.border}` : ''}` : ''
      }"><svg
            style="fill: ${color.color.toUpperCase() === '#8C8C8C' ? '#FFFFFF' : '#8C8C8C'};
            display: ${color.color === this.#color && display !== false ? 'block' : 'none'};"
    ...
    const nofillText = this.#editor.language.get('table', 'color', 'nonFillText');
  ...
  windowClick = (event: MouseEvent) => {
    const { target } = event;
    if (!target) return;
    if ($(target).closest('.data-table-color-tool,.table-color-dropdown-arrow').length === 0) this.remove();
  };
