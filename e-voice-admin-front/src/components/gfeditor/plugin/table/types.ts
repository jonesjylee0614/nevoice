...
export interface HelperInterface {
   ...
   * @returns {tdModel[][]} result 一个二维数组 [ [tdModel, tdModel...], [tdModel, tdModel...], ... ]
   * @tdModel:
   *  {
   ...
   * @returns {nativeNode} 修复过的 table dom
   */
  normalize(table: NodeInterface): NodeInterface;

  /**
   * firefox 下的拖拽需要这样处理 clearData 是为了防止新开 tab hack: 如果不 setData, firefox 不会触发拖拽事件，但设置 data 之后，又会开新 tab, 这里设置一个 firefox
   * 不识别的 mimetype: aomao
   *
   * @param event
   */
...
export interface TableInterface<V extends TableValue = TableValue> extends CardInterface<V> {
...
export interface TableCommandInterface extends EventEmitter2 {
  ...
  insertColAt(index: number, count: number, isLeft?: boolean, widths?: number | Array<number>, ...args: any): void;

  insertCol(position?: 'left' | 'end' | 'right', count?: number, ...args: any): void;
  ...
  insertRow(position?: 'up' | 'end' | 'down', count?: number, ...args: any): void;
...
export interface TableSelectionInterface extends EventEmitter2 {
  ...
  each(fn: (cell: TableModelCol | TableModelEmptyCol, row: number, col: number) => void, reverse?: boolean): void;
  ...
  select(start: { row: number; col: number }, end: { row: number; col: number }): void;
