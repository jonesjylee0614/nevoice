...
import { $, escape, isEngine, isHotkey, isMobile } from '@/components/gfeditor/emain';
import type { CodeBlockEditorInterface, Options } from './types';
const qa = [
  'c',
  'cpp',
  'csharp',
...
class CodeBlockEditor implements CodeBlockEditorInterface {
  ...
  getConfig(value: string, mode?: string): EditorConfiguration {
    ...
    let tabSize = mirror ? mirror.getOption('indentUnit') : qa.includes(mode || '') ? 4 : 2;
  ...
  create(mode: string, value: string, options?: EditorConfiguration) {
    ...
    const mirror = CodeMirror(this.container.find('.data-codeblock-content').get<HTMLElement>()!, {
      value,
      mode: syntaxMode,
      lineNumbers: true,
      lineWrapping: false,
    ...
    mirror.setOption('extraKeys', {
      Enter: mirror => {
        ...
        Object.keys(config).forEach(key => {
          return mirror.setOption(key as keyof EditorConfiguration, config[key as keyof EditorConfiguration]);
        });
  ...
  update(mode: string, code?: string) {
    ...
    mirror?.setOption('readOnly', Boolean(!isEngine(editor) || editor.readonly));
    this.save();
  }
  ...
  toHtml(col: number, text: string, style: string = '', tabSize: number = 0) {
    ...
    if (style) {
      ...
      const spanElement = `<span ${styleStr ? `style="${styleStr}"` : ''}>${escape(content)}</span>`;
      html += spanElement;
    } else {
      html += content;
    }
  ...
  /**
   * 代码来自 runmode addon 支持行号需要考虑复制粘贴问题
   *
   * runmode 本身不支持行号，见 https://github.com/codemirror/CodeMirror/issues/3364 可参考的解法
   * https://stackoverflow.com/questions/14237361/use-codemirror-for-standalone-syntax-highlighting-with-line-numbers
   *
   * ref:
   *
