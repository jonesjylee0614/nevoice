...
import type { CardToolbarItemOptions, CardValue, ToolbarItemOptions } from '@/components/gfeditor/emain';
import { $, Card, CardActiveTrigger, CardType, isEngine, isServer } from '@/components/gfeditor/emain';
import CodeBlockEditor from './editor';
import renderSelect from './select';
import modeDatas from './mode';
import type { CodeBlockEditorInterface } from './types';
import './index.css';
...
class CodeBlcok<V extends CodeBlockValue = CodeBlockValue> extends Card<V> {
  ...
  toolbar(): Array<CardToolbarItemOptions | ToolbarItemOptions> {
    ...
    const getItems = (): Array<CardToolbarItemOptions | ToolbarItemOptions> => {
          ...
          { key: 'copy', type: 'copy' },
          {
            ...
            getState: () => {
              if (this.#viewAutoWrap === undefined) {
                this.#viewAutoWrap = Boolean(this.getValue()?.autoWrap);
              }
      ...
      return [
        ...
        },
        {
          ...
          content: editor.language.get<string>(CodeBlcok.cardName, 'autoWrap'),
          getState: () => {
            return Boolean(this.getValue()?.autoWrap);
          },
          onClick: () => {
    ...
    const options = editor.plugin.findPlugin<CodeBlockOptions>('codeblock')?.options;
  ...
  render() {
    ...
    if (isEngine(this.editor)) {
      if (this.mirror) {
        ...
        this.codeEditor.setAutoWrap(Boolean(value?.autoWrap));
        return;
      }
      this.mirror = this.codeEditor?.create(mode, code, {
        lineWrapping: Boolean(value?.autoWrap)
      });
    } else {
