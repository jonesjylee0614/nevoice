...
import type { NodeInterface, PluginOptions, SchemaBlock } from '@/components/gfeditor/emain';
import { $, ListPlugin, PluginEntry, isEngine } from '@/components/gfeditor/emain';
...
export default class<T extends UnorderedlistOptions = UnorderedlistOptions> extends ListPlugin<T> {
  ...
  isCurrent(node: NodeInterface) {
    return !node.hasClass(this.editor.list.CUSTOMZIE_UL_CLASS) && node.name === 'ul';
  }
