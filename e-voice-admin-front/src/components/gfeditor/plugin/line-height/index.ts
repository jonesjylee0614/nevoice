import type { NodeInterface, PluginOptions, SchemaGlobal } from '@/components/gfeditor/emain';
import { Plugin, isEngine } from '@/components/gfeditor/emain';
...
export default class<T extends LineHeightOptions = LineHeightOptions> extends Plugin<T> {
  ...
  queryState() {
    ...
    blocks.forEach(block => {
      if (node.isNestedBlock(block)) {
        const lineHeightSource = block.get<HTMLElement>()?.style.lineHeight || '';
