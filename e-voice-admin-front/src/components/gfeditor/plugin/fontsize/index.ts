...
import { MarkPlugin, isEngine } from '@/components/gfeditor/emain';
...
export default class<T extends FontsizeOptions = FontsizeOptions> extends MarkPlugin<T> {
  ...
  defaultSize = this.options.defaultSize || this.editor.container.css('font-size') || '14px';
  ...
  init() {
    ...
    if (this.options.defaultSize) editor.container.css('font-size', this.defaultSize);
  }
