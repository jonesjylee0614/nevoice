...
import type { File, NodeInterface, PluginOptions, RequestData, RequestHeaders } from '@/components/gfeditor/emain';
import type { VideoStatus, VideoValue } from './component';
import type VideoComponent from './component';
...
export default class<T extends VideoUploaderOptions = VideoUploaderOptions> extends Plugin<T> {
  ...
  async execute(files?: Array<File> | MouseEvent | string, ...args: any) {
    ...
    const { action, data, type, contentType, multiple, crossOrigin, withCredentials, headers, name } = this.options;
    ...
    request.upload(
      {
        ...
        onReady: fileInfo => {
          if (!isEngine(editor) || Boolean(this.cardComponents[fileInfo.uid])) return;
          const component = card.insert<VideoValue, VideoComponent<VideoValue>>('video', {
            status: 'uploading',
            name: fileInfo.name,
            size: fileInfo.size
          });
        ...
        onSuccess: (response, file) => {
          ...
          const width: number = response.width || (response.data && response.data.width);
          const height: number = response.height || (response.data && response.data.height);
          let status: VideoStatus = response.status || (response.data && response.data.status);
          ...
          if (!result.result) {
            card.update<VideoValue>(component.id, {
              status: 'error',
              message: typeof result.data === 'string' ? result.data : language.get<string>('video', 'uploadError')
            });
          }
          // 成功
          else {
        ...
        onError: (error, file) => {
          ...
          card.update<VideoValue>(component.id, {
            status: 'error',
            message: error.message || language.get<string>('video', 'uploadError')
          });
          delete this.cardComponents[file.uid || ''];
        }
      },
  ...
  query(
    video_id: string,
    success: (data?: { url: string; name?: string; cover?: string; download?: string; status?: VideoStatus }) => void,
