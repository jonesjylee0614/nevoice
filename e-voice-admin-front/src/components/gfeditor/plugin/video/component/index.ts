...
class VideoComponent<T extends VideoValue = VideoValue> extends Card<T> {
  ...
  renderTemplate(value: VideoValue) {
    ...
    const titleElement = name ? `<div class="data-video-title">${escape(name)}</div>` : '';
    if (status === 'uploading') {
      return `
            <div class="data-video">
    ...
    return `
        <div class="data-video">
			...
			${videoPlugin && (videoPlugin.options as any).showTitle !== false ? titleElement : ''}
        </div>
        `;
  }
  ...
  initPlayer() {
    ...
    video.onloadedmetadata = () => {
      ...
      if (fullEditor && this._fullWidth && (value.naturalWidth < this._fullWidth || typeof fullEditor === 'number')) {
    ...
    setTimeout(() => {
      video.controls = true;
      if (!isEngine(editor)) (video as any).controlsList = 'nodownload';
    }, 0);
  }
  ...
  resetSize() {
    ...
    if (typeof fullEditor === 'number') this.videoContainer.css('height', height > 0 ? `${height}px` : '');
  }
  ...
  onWindowResize = () => {
    ...
    if (this.resizer) {
      ...
      this.resizer.setSize(this.videoContainer?.width() || 0, this.videoContainer?.height() || 0);
    }
  };
  ...
  initResizer() {
    ...
    const resizer = new Resizer({
      ...
      onChange: a => this.changeSize(a.width, a.height)
    });
