import type { CardInterface, NodeInterface, PluginEntry, SchemaInterface } from '@/components/gfeditor/emain';
import {
  $,
  CARD_KEY,
  CARD_VALUE_KEY,
...
export default class VideoPlugin<T extends VideoOptions = VideoOptions> extends Plugin<T> {
  ...
  async waiting(
    ...
    const check = (component: CardInterface) => {
      return (
        ...
        (component as VideoComponent<VideoValue>).getValue()?.status === 'uploading'
      );
    };
    // 找到不合格的组件
  ...
  parseHtml = (root: NodeInterface, callback?: (node: NodeInterface, value: VideoValue) => NodeInterface) => {
    const results: NodeInterface[] = [];
    root
      .find(`[${CARD_KEY}="${VideoComponent.cardName}"],[${READY_CARD_KEY}="${VideoComponent.cardName}"]`)
      .each(cardNode => {
        const node = $(cardNode);
        ...
        if (value?.url && value.status === 'done') {
          ...
          const html = `<div data-type="${VideoComponent.cardName}"  data-value="${encodeCardValue(
            value
          )}"><video controls src="${sanitizeUrl(
            onBeforeRender ? onBeforeRender('query', url, this.editor) : url
          )}" poster="${
            !cover ? 'none' : sanitizeUrl(onBeforeRender ? onBeforeRender('cover', cover, this.editor) : cover)
