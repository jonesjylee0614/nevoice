import type { PluginOptions } from '@/components/gfeditor/emain';
import { Plugin, isEngine } from '@/components/gfeditor/emain';

export interface RemoveformatOptions extends PluginOptions {
  hotkey?: string | Array<string>;
}
export default class<T extends RemoveformatOptions = RemoveformatOptions> extends Plugin<T> {
  ...
  execute() {
    ...
    if (marks.length > 0) {
      ...
      blocks.forEach(block => {
        const plugin = blockApi.findPlugin(block.name === 'li' ? block.parent()! : block);
        if (plugin) {
