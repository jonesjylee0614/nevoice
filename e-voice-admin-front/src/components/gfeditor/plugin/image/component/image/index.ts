...
import type { ImageOptions, PswpInterface } from '@/components/gfeditor/plugin/image/types';
import type { EditorInterface, NodeInterface } from '@/components/gfeditor/emain';
import { $, CardType, Resizer, Tooltip, escape, isEngine, isMobile, sanitizeUrl } from '@/components/gfeditor/emain';
...
export type Options = {
  ...
  /** 状态 uploading 上传中 done 上传完成 error 错误 */
  status: Status;
  /** 图标链接 */
  src: string;
  /** 标题 */
...
export const winPixelRatio = typeof window !== 'undefined' ? window.devicePixelRatio : 1;
let pswp: PswpInterface | undefined;
class Image {
  ...
  openZoom = (event: MouseEvent | TouchEvent) => {
    ...
    editor.container
      ...
      .forEach((imageNode, index) => {
        ...
        const imageWidth = Number.parseInt(image.css('width'), 10);
        const imageHeight = Number.parseInt(image.css('height'), 10);
