import type { ImageOptions } from '@/components/gfeditor/plugin/image/types';
import type { CardToolbarItemOptions, CardValue, NodeInterface, ToolbarItemOptions } from '@/components/gfeditor/emain';
import { Card, CardType, isEngine, isMobile } from '@/components/gfeditor/emain';
...
class ImageComponent<T extends ImageValue = ImageValue> extends Card<T> {
  ...
  setSize(size: Size, loaded?: boolean) {
    ...
    if (!loaded || !value.size || !value.size.height || !value.size.width) this.setValue({ size } as T);
    if (this.widthInput) {
  ...
  onInputChange(width: string | number, height: string | number) {
    ...
    this.image?.changeSize(Number.parseInt(width.toString(), 10), height);
  }
      ...
      const resizerItems: (CardToolbarItemOptions | ToolbarItemOptions)[] = [
        {
          ...
          onChange: value => {
            const height = Math.round(Number.parseInt(value, 10) * (this.image?.rate || 1));
        ...
        },
        {
          ...
          onChange: value => {
            const width = Math.round(Number.parseInt(value, 10) / (this.image?.rate || 1));
