...
import { $, isHotkey, isMobile } from '@/components/gfeditor/emain';
import type { PswpInterface } from '@/components/gfeditor/plugin/image/types';
...
class Pswp extends EvemtEmitter3 implements PswpInterface {
  ...
  bindClickEvent() {
    const onClick = (event: MouseEvent | TouchEvent) => {
      const node =
        window.TouchEvent && event instanceof TouchEvent ? $(event.touches[0].target) : $(event.target || []);
  ...
  renderCounter() {
    this.barUI
      ...
      .html(`${(this.pswpUI?.getCurrentIndex() || 0) + 1} / ${this.pswpUI?.items.length || ''}`);
  }

  getCurrentZoomLevel() {
    return (
      (this.zoom && Number(this.zoom.toFixed(2))) || (this.pswpUI && Number(this.pswpUI.getZoomLevel().toFixed(2))) || 0
    );
  }
  ...
  getInitialZoomLevel() {
    ...
    return Number(this.pswpUI.currItem.initialZoomLevel?.toFixed(2) || 0);
  }
