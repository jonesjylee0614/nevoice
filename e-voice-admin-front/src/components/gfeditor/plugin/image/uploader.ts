...
import type { ImageValue } from './component';
import type ImageComponent from './component';
...
export default class<T extends ImageUploaderOptions = ImageUploaderOptions> extends Plugin<T> {
  ...
  async execute(files?: Array<File> | string | MouseEvent) {
    ...
    const { action, data, type, contentType, multiple, crossOrigin, withCredentials, headers, name } =
      this.options.file;
  ...
  pasteEach = (node: NodeInterface) => {
    ...
    // 图片带链接
    /**
     * if (node.name === 'a' && node.find('img').length > 0) { const img = node.find('img'); const href =
     * node.attributes('href'); const target = node.attributes('target'); const src = img.attributes('src') ||
     * img.attributes('data-src'); const alt = img.attributes('alt'); if (!src) { node.remove(); return; }
     * this.editor.card.replaceNode(node, 'image', { src, status: (isRemote && isRemote(src)) ||
     * /^data:image//i.test(src) ? 'uploading' : 'done', alt, link: { href, target }, percent: 0 }); }
     */
    // 图片
    if (node.name === 'img') {
  ...
  async uploadAddress(src: string, component: ImageComponent) {
    ...
    editor.request.ajax({
      ...
      success: response => {
        let src =
          response.url || (response.data && response.data.url) || response.src || (response.data && response.data.src);
  ...
  pasteAfter = () => {
    ...
    editor.container.find('[data-card-key=image]').each((node, key) => {
      ...
      if (
        value?.status !== 'uploading' ||
        Object.keys(this.cardComponents).find(key => this.cardComponents[key].id === component.id)
      ) {
        return;
      }
