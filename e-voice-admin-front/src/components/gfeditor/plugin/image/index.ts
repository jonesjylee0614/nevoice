import type { CardInterface, NodeInterface, PluginEntry } from '@/components/gfeditor/emain';
import {
  $,
  CARD_KEY,
  CARD_TYPE_KEY,
...
import type { ImageUploaderOptions } from './uploader';
...
export default class<T extends ImageOptions = ImageOptions> extends Plugin<T> {
  ...
  async waiting(
    ...
    const check = (component: CardInterface) => {
      return (
        ...
        (component as ImageComponent<ImageValue>).getValue()?.status === 'uploading'
      );
    };
    // 找到不合格的组件
  ...
  parseHtml = (root: NodeInterface, callback?: (node: NodeInterface, value: ImageValue) => NodeInterface) => {
    ...
    const editor = this.editor;
    root
      .find(`[${CARD_KEY}="${ImageComponent.cardName}"],[${READY_CARD_KEY}="${ImageComponent.cardName}"]`)
      .each(cardNode => {
        const node = $(cardNode);
