...
import type { ConversionData, PluginOptions, SchemaMark } from '@/components/gfeditor/emain';

export interface BackcolorOptions extends PluginOptions {
  hotkey?: { key: string; args: Array<string> };
}
export default class<T extends BackcolorOptions = BackcolorOptions> extends MarkPlugin<T> {
  ...
  conversion(): ConversionData {
    return [
      {
        from: (name, styles) => {
          return name === this.tagName && Boolean(styles.background);
        },
