import type { CardToolbarItemOptions, NodeInterface, ToolbarItemOptions } from '@/components/gfeditor/emain';
import {
  $,
  Card,
  CardType,
...
export default class FileCard<V extends FileValue = FileValue> extends Card<V> {
  ...
  getMaxWidth = () => {
    ...
    return block.get<Element>()!.clientWidth - 36 - (sizeElement?.get<Element>()?.clientWidth || 0);
  };
  ...
  updateMaxWidth = () => {
    ...
    this.root.find('.data-file-title').css('max-width', `${Math.max(maxWidth, 0)}px`);
  };
  ...
  toolbar() {
    ...
    const options = editor.plugin.findPlugin<FileOptions>('file')?.options ?? {};
    const getItems = () => {
      ...
      if (status === 'done') {
        if (preview) {
          ...
          items.push({
            ...
            link: !options.onPreview ? sanitizeUrl(this.onBeforeRender('preview', preview)) : undefined,
        ...
        if (download) {
          ...
          items.push({
            ...
            link: !options.onDownload ? sanitizeUrl(this.onBeforeRender('download', download)) : undefined,
  ...
  renderView() {
    // 默认点击都是下载
    const value = this.getValue();
    const options = this.editor.plugin.findPlugin<FileOptions>('file')?.options ?? {};
    const downloadUrl = sanitizeUrl(this.onBeforeRender('download', value.download || ''));
