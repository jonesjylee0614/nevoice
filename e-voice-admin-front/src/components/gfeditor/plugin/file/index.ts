import type { CardInterface, NodeInterface, SchemaInterface } from '@/components/gfeditor/emain';
import {
  $,
  CARD_KEY,
  CARD_VALUE_KEY,
...
export default class<T extends FileOptions = FileOptions> extends Plugin<T> {
  ...
  async waiting(
    ...
    const check = (component: CardInterface) => {
      return (
        ...
        (component as FileComponent<FileValue>).getValue()?.status === 'uploading'
      );
    };
    // 找到不合格的组件
    const find = () => {
  ...
  parseHtml = (root: NodeInterface, callback?: (node: NodeInterface, value: FileValue) => NodeInterface) => {
    const results: NodeInterface[] = [];
    root
      .find(`[${CARD_KEY}="${FileComponent.cardName}"],[${READY_CARD_KEY}="${FileComponent.cardName}"`)
      .each(cardNode => {
        const node = $(cardNode);
        ...
        if (value?.url && value.status === 'done') {
          const html = `<span data-type="${FileComponent.cardName}" data-value="${encodeCardValue(
            value
