...
import type FileComponent from './component';
...
export default class<T extends FileUploaderOptions = FileUploaderOptions> extends Plugin<T> {
  ...
  async execute(files?: Array<File> | MouseEvent) {
    ...
    const { action, type, contentType, multiple, crossOrigin, withCredentials, headers, name } = this.options;
    ...
    request.upload(
      {
        ...
        onReady: fileInfo => {
          ...
          const component = card.insert<FileValue, FileComponent<FileValue>>('file', {
            status: 'uploading',
            name: fileInfo.name,
            size: fileInfo.size
          });
        ...
        onSuccess: (response, file) => {
          ...
          const preview = response.preview || (response.data && response.data.preview);
          const download = response.download || (response.data && response.data.download);
          let result: {
            result: boolean;
            data:
              | {
          ...
          if (parse) {
            ...
            if (customizeResult.result) {
              ...
              if (typeof customizeResult.data === 'string')
                ...
                res.url = customizeResult.data.url;
          ...
          if (!result.result) {
            card.update<FileValue>(component.id, {
              status: 'error',
              message: typeof result.data === 'string' ? result.data : language.get<string>('file', 'uploadError')
            });
          } else {
            const value: any =
        ...
        onError: (error, file) => {
          ...
          card.update<FileValue>(component.id, {
            status: 'error',
            message: error.message || language.get<string>('file', 'uploadError')
          });
          delete this.cardComponents[file.uid || ''];
        }
      },
