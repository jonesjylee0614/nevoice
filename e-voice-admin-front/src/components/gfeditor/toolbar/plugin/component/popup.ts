...
import { $, Range, UI_SELECTOR, isEngine, isMobile } from '@/components/gfeditor/emain';
...
export default class Popup {
  ...
  constructor(editor: EditorInterface, options: PopupOptions = {}) {
    ...
    (this.#editor.scrollNode?.get<HTMLElement>() || document.body).appendChild(this.#root[0]);
    if (isEngine(editor)) {
  ...
  onSelect = () => {
    ...
    if (subRanges.length === 0 || (this.#editor.card.active && !this.#editor.card.active.isEditable)) {
      this.hide();
      return;
    }
    ...
    this.showContent(() => {
      ...
      this.#align =
        ...
        (!topRange.startNode.equal(selection.focusNode!) || selection.focusOffset > selection.anchorOffset)
          ? 'bottom'
          : 'top';
      const space = 12;
      ...
      if (
        this.#align === 'top' &&
        targetRect.top - rootRect.height - space < window.innerHeight - (this.#editor.scrollNode?.height() || 0)
      ) {
        this.#align = 'bottom';
      } else if (this.#align === 'bottom' && targetRect.bottom + rootRect.height + space > window.innerHeight) {
        this.#align = 'top';
      }
      ...
      const top =
        this.#align === 'top'
          ? targetRect.top - rootRect.height - space - (scrollNodeRect?.top || 0) + (scrollElement?.scrollTop || 0)
          : targetRect.bottom + space - (scrollNodeRect?.top || 0) + (scrollElement?.scrollTop || 0);
  ...
  hide = (event?: MouseEvent) => {
    if (event?.target) {
      ...
      if (target.closest('.data-toolbar-popup-wrapper').length > 0 || target.closest(UI_SELECTOR).length > 0) return;
    }
    this.#root.css({
      left: '0px',
      top: '-9999px'
