...
import type { EngineInterface, NodeInterface } from '@/components/gfeditor/emain';
import { $, Position } from '@/components/gfeditor/emain';
...
export interface CollapseComponentInterface {
  ...
  bindEvents(): void;
  remove(): void;
  render(container: NodeInterface, target: NodeInterface, data: Array<CollapseGroupProps>): void;
}
...
class CollapseComponent implements CollapseComponentInterface {
  ...
  select(index: number) {
    this.root?.find('.toolbar-collapse-item-active').removeClass('toolbar-collapse-item-active');
    this.root?.find('.toolbar-collapse-item').eq(index)?.addClass('toolbar-collapse-item-active');
  }
  ...
  scroll(direction: 'up' | 'down') {
    ...
    this.select(index);
    let offset = 0;
    this.root.find('.toolbar-collapse-group-title,.toolbar-collapse-item').each(node => {
  ...
  render(container: NodeInterface, target: NodeInterface, data: Array<CollapseGroupProps>) {
    this.unbindEvents();
    this.remove();
