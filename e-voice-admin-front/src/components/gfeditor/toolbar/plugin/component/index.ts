import type { CardValue, NodeInterface } from '@/components/gfeditor/emain';
import { $, Card, CardType, DATA_CONTENTEDITABLE_KEY, isEngine, isHotkey, isServer } from '@/components/gfeditor/emain';
import type { CollapseGroupProps, CollapseItemProps } from '../../types';
...
class ToolbarComponent<V extends ToolbarValue = ToolbarValue> extends Card<V> {
  ...
  getData(): Data {
    ...
    const data: Data | { title: any; items: Omit<CollapseItemProps, 'engine'>[] }[] = [];
    const defaultConfig = getToolbarDefaultConfig(editor);
    const collapseConfig = defaultConfig.find(({ type }) => type === 'collapse') as any;
    ...
    collapseGroups.forEach((group: any) => {
      ...
      group.items.forEach((item: any) => {
        ...
        if (collapseItem) {
          items.push({
            ...
            disabled: collapseItem.onDisabled ? collapseItem.onDisabled() : !editor.command.queryEnabled(name)
          });
  ...
  search(keyword: string) {
    ...
    this.#collapseData.forEach((group: any) => {
      group.items.forEach((item: any) => {
        if (item.search && item.search.toLowerCase().includes(keyword)) {
  ...
  changeToText() {
    ...
    const content = this.keyword?.get<HTMLElement>()?.textContent || '';
    this.remove();
  ...
  handleInput() {
    ...
    const content = this.keyword?.get<HTMLElement>()?.textContent?.replace(/[\r\n]/g, '') || '';
    const component = this.component;
    // 内容为空
    if (content === '') {
      component?.remove();
  ...
  resetPlaceHolder() {
    if (this.keyword?.get<HTMLElement>()?.textContent === '/') this.placeholder?.show();
  ...
  render(data?: any): string | void | NodeInterface {
    ...
    const language = editor.language.get<{ placeholder: string }>('toolbar', 'component');
