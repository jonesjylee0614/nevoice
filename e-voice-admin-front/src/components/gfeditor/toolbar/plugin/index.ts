import type { EditorInterface, NodeInterface, PluginOptions } from '@/components/gfeditor/emain';
import { Plugin, isEngine, isSafari } from '@/components/gfeditor/emain';
import type { CollapseItemProps, GroupItemProps } from '../types';
  ...
  onSlash = (event: KeyboardEvent) => {
    ...
    if (text === '' || (text === '/' && isSafari) || event.ctrlKey || event.metaKey) {
      ...
      if (range.collapsed) {
        ...
        const card = editor.card.insert(ToolbarComponent.cardName, {}, data) as ToolbarComponent<ToolbarValue>;
        card.setData(data);
