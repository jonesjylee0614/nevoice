...
import { CARD_SELECTOR, Range, isEngine } from '@/components/gfeditor/emain';
...
export const getToolbarDefaultConfig = (engine: EngineInterface): Array<ToolbarItemProps> => {
  ...
  return [
      ...
      groups: [
        {
          items: [
            ...
            },
            {
              ...
              onDisabled: () => {
                // 有激活卡片 或者没有启用插件
                return Boolean(engine.card.active) || !engine.command.queryEnabled('table');
              },
              prompt:
                Boolean(engine.card.active) || !engine.command.queryEnabled('table')
                  ? undefined
                  : h(TableSelector, {
            ...
            },
            {
              ...
              title: language.imgattachment.title,
              search: '附件管理器,imgattachment'
            }
            // {
            // 	name: 'math',
            // 	icon: `<span>
    ...
    },
    {
      ...
      ].map(item => (item.key === fontSizeInContainer ? { ...item, isDefault: true } : item)),
      onDisabled: () => {
    ...
    },
    {
      ...
      onActive: items => {
        ...
        return items.find(item => familys.includes((item as any).faimlyName.trim().toLowerCase()))?.key || '';
      }
    },
    {
      type: 'button',
    ...
    },
    {
      ...
      onDisabled: () => {
        const { card } = engine;
        const range = isEngine(engine) ? engine.change.range.get() : Range.from(engine);
        if (!range) return !engine.command.queryEnabled('link');
        const cardComponent = card.find(range.startNode);
        return (
          (Boolean(cardComponent) && !cardComponent.isCursor(range.startNode)) ||
          range.containsCard() ||
