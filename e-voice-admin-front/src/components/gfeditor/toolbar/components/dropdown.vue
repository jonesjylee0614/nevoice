<template>
  <div
    ...
    :class="[{ 'toolbar-dropdown-right': isRight }, className]"
    @click="triggerClick"
        ...
        <template #default>
          <slot :item="content">
            <span
              ...
              class="data-icon"
              :class="[`data-icon-${buttonContent.icon}`]"
            />
            <span
              v-if="buttonContent && !buttonContent.icon && buttonContent.content"
...
export default defineComponent({
  ...
  setup(props, cxt) {
    ...
    const buttonContent = ref<DropdownListItem | { icon?: string; content?: string } | undefined>(undefined);
    ...
    const update = (values?: string | number | string[]) => {
      if (props.single !== false) values = Array.isArray(values) && values.length > 0 ? values[0] : values;
      const item = props.items.find(
        item =>
          (typeof values === 'string' && item.key === values) || (Array.isArray(values) && values.includes(item.key))
      );
      const defaultItem =
        props.items.length > 0 ? props.items.find(item => item.isDefault === true) || props.items[0] : null;
      ...
      if (item) {
        if (cxt.slots.default) {
          ...
          buttonContent.value = {
            icon: item.icon,
            content: typeof item.content === 'function' ? item.content(props.engine) : item.content
          };
        }
        ...
        if (!Array.isArray(values) || values.length < 1) {
          buttonContent.value = {
            icon: props.icon,
            content: typeof props.content === 'function' ? props.content(props.engine) : props.content
          };
        }
      } else if (defaultItem) {
        buttonContent.value = {
          icon: defaultItem.icon,
          content: typeof defaultItem.content === 'function' ? defaultItem.content(props.engine) : defaultItem.content
        };
      }
      valuesVar.value = values || (props.icon || props.content ? '' : defaultItem ? defaultItem.key : '');
    };
    ...
    const hide = (event?: MouseEvent) => {
      if (event && targetRef.value && targetRef.value.element && targetRef.value.element.contains(event.target as Node))
        return;
      visible.value = false;
    };
...
.editor-toolbar-popup .toolbar-dropdown .toolbar-dropdown-list:not(.toolbar-dropdown-placement-bottom) {
  bottom: 32px;
  top: auto;
  overflow: auto;
}
...
.editor-toolbar-popup .toolbar-dropdown.toolbar-dropdown-right .toolbar-dropdown-list {
  right: 0px;
}
