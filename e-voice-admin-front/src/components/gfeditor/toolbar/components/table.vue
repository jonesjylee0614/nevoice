<template>
  <div data-element="ui" class="data-toolbar-table-selector">
    <div v-for="row in currentRows" :key="row" class="data-toolbar-table-selector-tr">
      <div
        v-for="col in currentCols"
        :key="col"
        class="data-toolbar-table-selector-td"
        :class="[{ actived: row - 1 < selectedRows && col - 1 < selectedCols }]"
...
const triggerSelect = (event: MouseEvent, rows: number, cols: number) => {
  if (props.onSelect) props.onSelect(event, rows + 1, cols + 1);
};
const triggerHover = (rows: number, cols: number) => {
