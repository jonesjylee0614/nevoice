<template>
    ...
    >
      <template #content>
        <div
          ...
          :class="[{ 'editor-toolbar-mobile': isMobile && !popup, 'editor-toolbar-popup': popup }]"
          data-element="ui"
        >
          <template v-for="(item, index) in items" :key="index">
            <AmButton v-if="item.type === 'button'" :key="index" v-bind="item" placement="top" :engine="engine" />
            <AmDropdown v-if="item.type === 'dropdown'" :key="index" v-bind="item" placement="top" :engine="engine" />
            <AmColor v-if="item.type === 'color'" :key="index" v-bind="item" placement="top" :engine="engine" />
            <AmCollapse v-if="item.type === 'collapse'" :key="index" v-bind="item" :engine="engine" />
          </template>
        </div>
      </template>
  ...
  <div v-if="!icon && !content" class="editor-toolbar-group">
    <template v-for="(item, index) in items" :key="index">
      <AmButton v-if="item.type === 'button'" :key="index" v-bind="item" :engine="engine" />
      <AmDropdown v-if="item.type === 'dropdown'" :key="index" v-bind="item" :engine="engine" />
