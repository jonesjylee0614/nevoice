<template>
  <div ref="buttonRef" class="toolbar-dropdown colorpicker-button" :class="[{ 'toolbar-dropdown-right': isRight }]">
    <div
      class="toolbar-dropdown-trigger colorpicker-button-group"
      :class="[{ 'colorpicker-button-group-active': visible }]"
    >
      <AmButton
  ...
  setup(props) {
    ...
    const getContent = () => {
      return typeof props.content === 'string'
        ? props.content
        : props.content(currentColor.value, Palette.getStroke(currentColor.value), props.disabled);
    };
