...
export const autoGetHotkey = (engine: EngineInterface, name: string, itemKey?: string) => {
  ...
  if (plugin && plugin.hotkey) {
    ...
    if (key) {
      if (Array.isArray(key)) {
        if (itemKey) {
          const index = key.findIndex((k: any) => typeof k === 'object' && k.args === itemKey);
...
export const isSupportFontFamily = (font: string) => {
  ...
  if (supportFontFamilyCache[font] !== undefined) return supportFontFamilyCache[font];
  let width;
