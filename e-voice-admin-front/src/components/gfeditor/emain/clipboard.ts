...
import type { ClipboardInterface, EditorInterface } from './types';
...
export const isDragEvent = (event: DragEvent | ClipboardEvent): event is DragEvent => {
  return Boolean((<DragEvent>event).dataTransfer);
};
...
export default class Clipboard implements ClipboardInterface {
  ...
  getData(event: DragEvent | ClipboardEvent) {
    const transfer = isDragEvent(event) ? event.dataTransfer : event.clipboardData;
    ...
    try {
      if (transfer?.items && transfer.items.length > 0) {
        Array.from(transfer.items).forEach(item => {
          ...
          if (file !== null) {
            if (file.type && file.type.includes('image/png') && !file.lastModified) {
    ...
    if (html === '' && text && /^.+\.\w+$/.test(text) && files.length > 0) {
      text = ''; // 在图片上，点击右键复制
    } else if (text === '' && html && /^(<meta.+?>)?<img.+?>$/.test(html) && files.length > 0) {
  ...
  write(event: ClipboardEvent, range?: RangeInterface) {
    ...
    event?.clipboardData?.setData('text/html', `<meta name="source" content="aomao" />${html}`);
  ...
  copy(data: Node | string, trigger: boolean = false) {
      ...
      if (isHtml) {
        copyTo(data, {
          ...
          onCopy: (clipboardData: any) => {
            clipboardData.setData('text/plain', new Parser(data, this.editor).toText());
          }
        });
      } else {
        copyTo(data, {
    ...
    const range = selection ? Range.from(editor, selection) || Range.create(editor) : Range.create(editor);
    const cloneRange = range.cloneRange();
    const block = $(`<div class="${VIEW_CLASS_NAME}" ${DATA_ELEMENT}="${ROOT}">&#8203;</div>`);
    block.css({
      position: 'fixed',
      top: 0,
      clip: 'rect(0, 0, 0, 0)'
  ...
  cut() {
    ...
    for (let i = 0; i < listElements.length; i++) {
      ...
      childs.each(child => {
        if ((child as HTMLElement).textContent === '' || (isSafari && (child as HTMLElement).textContent === '\n')) {
