...
import type { CardInterface, ChangeInterface, CommandInterface, EditorInterface } from './types';
...
class Command implements CommandInterface {
  ...
  queryEnabled(name: string) {
    ...
    if (cardApi.active) {
      ...
      return cardApi.active.isEditable;
    }
    // TODO:查询当前所处位置的插件
    return true;
  }
  ...
  executeMethod(name: string, method: string, ...args: any) {
    const editor = this.editor;
    const plugin = editor.plugin.components[name] as any;
