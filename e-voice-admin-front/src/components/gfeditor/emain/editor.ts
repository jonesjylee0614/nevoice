...
import type { ListModelInterface } from './types/list';
...
import { CARD_ELEMENT_KEY, CARD_KEY, DATA_ELEMENT, DATA_ID, ROOT } from './constants';
import { isEngine } from './utils';
import Parser from './parser';

class Editor<T extends EditorOptions = EditorOptions> implements EditorInterface<T> {
  ...
  get scrollNode(): NodeInterface | null {
    ...
    let sn = scrollNode ? (typeof scrollNode === 'function' ? scrollNode() : scrollNode) : null;
  ...
  constructor(selector: Selector, options?: EditorOptions) {
    ...
    if (!fontElement && iconFonts !== false) {
      ...
      if (Array.isArray(iconFonts)) {
        iconFonts = iconFonts.map(font => `url('${font.url}') format('${font.format}')`).join(',');
      }
    ...
    // 多语言
    this.language = new Language(this.options.lang || 'zh-CN', merge(language, options?.locale));
    // 事件管理
    ...
    conversionDefault.forEach(rule => this.conversion.add(rule.from, rule.to));
    // 卡片
  ...
  init() {
    ...
    const configData = typeof config === 'function' ? config(this) : (config ?? {});
  ...
  on<R = any, F extends EventListener<R> = EventListener<R>>(eventType: string, listener: F, rewrite?: boolean) {
  ...
  getSelectionData(range?: RangeInterface): Record<'html' | 'text', string> | undefined {
    ...
    let card = range.startNode.closest(`[${CARD_KEY}]`, node => {
      return $(node).isEditable() ? undefined : node.parentNode || undefined;
    });
    if (card.length > 0 && !range.collapsed && range.endOffset === 0) {
      ...
      if (!range.collapsed && range.endOffset > 0 && range.endContainer.childNodes[range.endOffset - 1] === card[0]) {
        const cardCenter = range.startNode.closest(`[${CARD_ELEMENT_KEY}="center"]`, node => {
          return $(node).isEditable() ? undefined : node.parentNode || undefined;
        });
    ...
    if (card.length > 0) {
      const cardCenter = root.closest(`[${CARD_ELEMENT_KEY}="center"]`, node => {
        return $(node).isEditable() ? undefined : node.parentNode || undefined;
      });
      if (cardCenter.length === 0) {
        range.select(card);
    ...
    // 复制纯文本，获取外层的样式包裹层
    const nodes: Node[] = [];
    if (root.isText() && contents.childNodes.length === 1 && contents.firstChild?.nodeType === Node.TEXT_NODE) {
    ...
    contents.querySelectorAll('li').forEach(child => {
      ...
      if (!dataId) return;
      const curentElement = this.container.get<HTMLElement>()?.querySelector(`[${DATA_ID}=${dataId}]`);
      // 补充自定义列表丢失的卡片
      if (node.isCustomize(childElement) && !childElement.first()?.isCard() && curentElement?.firstChild) {
        childElement.prepend(node.clone($(curentElement.firstChild), true, false));
      }
      let parent: NodeInterface | Node | null | undefined = curentElement?.parentElement;
