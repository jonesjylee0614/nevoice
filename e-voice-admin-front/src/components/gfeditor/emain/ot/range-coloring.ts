...
import { TinyCanvas, escape, removeUnit } from '../utils';
...
import type { Attribute, CursorRect, Member, RangeColoringInterface } from '../types/ot';
import type { DrawStyle, TinyCanvasInterface } from '../types/tiny-canvas';
import Range, { isRangeInterface } from '../range';
import type { CardEntry, CardInterface } from '../types/card';
import { DATA_COLOR, DATA_ELEMENT, DATA_UUID, EDITABLE_SELECTOR } from '../constants';
import { $ } from '../node';
...
class RangeColoring implements RangeColoringInterface {
  ...
  drawSubRang(node: NodeInterface, canvas: TinyCanvasInterface, range: RangeInterface, style: DrawStyle) {
  ...
  drawBackground(range: RangeInterface, options: { uuid: string; color: string }) {
    ...
    let child = (this.engine.scrollNode ?? this.root).find(`.${USER_BACKGROUND_CLASS}[${DATA_UUID}="${uuid}"]`);
    if (child && child.length > 0) {
      child.attributes(DATA_COLOR, color.toString());
      targetCanvas = child[0].__canvas;
      targetCanvas.clear();
    } else {
      child = $(`<div class="${USER_BACKGROUND_CLASS}" ${DATA_UUID}="${uuid}" ${DATA_COLOR}="${color}" />`);
    ...
    const parentWidth = containerElement.get<Element>()?.clientWidth || containerElement.width();
  ...
  getCursorRect(selector: RangeInterface | NodeInterface, leftSpace: number = 2): CursorRect {
    ...
    if (isRangeInterface(selector)) {
      ...
      if (startNode.isElement() && rect.height === 0) {
        let childNode: NodeInterface | null = $(startNode[0].childNodes[range.startOffset]);
  ...
  drawCursor(selector: RangeInterface | NodeInterface, member: Member, showInfo?: boolean) {
    const { uuid, name, color } = member;
    let cursorRect = this.getCursorRect(selector);
    let childCursor = this.root.children(`.${USER_CURSOR_CLASS}[${DATA_UUID}="${uuid}"]`);
    if (childCursor && childCursor.length > 0) {
      this.setCursorRect(childCursor, cursorRect);
    } else {
      const userCursor = `
            <div class="${USER_CURSOR_CLASS}" ${DATA_UUID}="${uuid}">
                <div class="${USER_CURSOR_TRIGGER_CLASS}">${escape(name || '')}</div>
            </div>`;
    ...
    if (childCursor && childCursor[0]) {
      ...
      const maximizeCard = this.engine.card.components.find(component => component.isMaximize);
      if (maximizeCard) {
        const card = this.engine.card.closest(isRangeInterface(selector) ? selector.startNode : selector, true);
  ...
  drawCard(node: NodeInterface, cursor: NodeInterface, member: Member) {
    ...
    let mask = this.root.children(`.${USER_MASK_CLASS}[${DATA_UUID}="${member.uuid}"]`);
    ...
    mask = $(`<div class="${USER_MASK_CLASS}" ${DATA_UUID}="${member.uuid}" />`);
    mask[0].__node = node[0];
  ...
  drawRange(range: RangeInterface, member: Member, showInfo?: boolean) {
    ...
    if (removeCardMasks.length > 0) {
      this.root
        .get<Element>()
        ?.querySelectorAll(removeCardMasks.map(uuid => `.${USER_MASK_CLASS}[${DATA_UUID}="${uuid}"]`).join(','))
        ?.forEach(child => {
    ...
    if (cardInfo && !cardInfo.isEditable) {
      const root = this.setCardActivatedByOther(cardInfo, member) || cardInfo.root;
      ...
      if (singleCard) {
        ...
        const root = this.setCardSelectedByOther(singleCard, member) || singleCard.root;
  ...
  updateCursorPosition() {
    this.root
      ...
      .forEach(child => {
        ...
        if (target.startContainer || $(target).closest('body').length !== 0) {
  ...
  updateBackgroundAlpha(range: RangeInterface) {
    ...
    this.root
      ...
      .forEach(child => {
        const trigger = child.querySelector<HTMLElement>(`.${USER_CURSOR_TRIGGER_CLASS}`);
