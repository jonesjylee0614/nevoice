...
import type { Attribute, Member, SelectionInterface } from '../types/ot';
...
class OTSelection extends EvemtEmitter3 implements SelectionInterface {
  ...
  getCardResizeRange(card: CardInterface) {
    if (card?.getSelectionNodes) {
      ...
      if (nodes.length > 0) {
        ...
        range.setEnd(end, end.isText() ? end.text().length : end.get<Element>()?.childNodes.length || 0);
        return range;
      }
    }
    return null;
  ...
  setAttribute(attr: Attribute, member: Member, refreshBG = false) {
    ...
    if (attr.force || !isEqual(item || {}, attr)) {
      this.data.set(attr.uuid, { ...attr, active: !item });
  ...
  removeAttirbute(uuid: string) {
    ...
    if (uuid === this.current?.uuid) this.emit('change', { uuid, remove: true });
  ...
  onSelectionChange(range: RangeInterface, force = false, refreshBG = false) {
    ...
    this.setAttribute({ ...current, path, uuid: this.current.uuid, force }, this.current, refreshBG);
