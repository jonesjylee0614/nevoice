...
import type { Doc, Op } from 'sharedb';
...
class OTModel extends EvemtEmitter3 implements OTInterface {
  ...
  constructor(engine: EngineInterface) {
    ...
    this.debounceRefreshAttributes = debounce((uids: string[]) => {
      this.selection.refreshAttributes(...this.members.filter(m => uids.includes(m.uuid)));
    }, 200);
  }
  ...
  colors = ['#597EF7', '#73D13D', '#FF4D4F', '#9254DE', '#36CFC9', '#FFA940', '#F759AB', '#40A9FF'];
  ...
  initRemote(doc: Doc, defaultValue?: string, onSelectionChange?: (path: Attribute) => void) {
  ...
  handleChange = (ops: Op[]) => {
    ...
    if (ops.find(op => ('od' in op || 'oi' in op) && op.p.includes(CARD_VALUE_KEY))) {
      change.change(false);
    }
  };
  ...
  submitOps(ops: Op[]) {
    ...
    tempDoc.submitOp(ops, null, (err: any) => {
      ...
      this.doc.submitOp(
        ...
        error => {
          if (error) {
            this.engine.messageError('ot', 'SubmitOps Error:', error, 'OPS:', ops, 'DATA:', this.doc?.data);
            // 重置
            this.doc?.destroy();
          }
          this.selection.emitSelectChange(true);
  ...
  renderSelection(attributes: Attribute[] | Attribute) {
    ...
    attributes.forEach(attribute => {
      if (this.currentMember && attribute.uuid === this.currentMember.uuid) return;
      const member = this.members.find(m => m.uuid === attribute.uuid);
      if ('remove' in attribute || !member) this.selection.removeAttirbute(attribute.uuid);
      else {
