...
import type { Doc, Path, StringDeleteOp, StringInsertOp } from 'sharedb';
import isEqual from 'lodash/isEqual';
import type { DocInterface, EngineInterface, TargetOp } from '../types';
import { DATA_ID, JSON0_INDEX } from '../constants';
import { $ } from '../node';
import { isRoot } from '../node/utils';
import { decodeCardValue } from '../utils';
import { CARD_KEY, CARD_LOADING_KEY, CARD_TYPE_KEY, CARD_VALUE_KEY } from './../constants/card';
import { findFromDoc, isTransientAttribute, isTransientElement, toJSON0 } from './utils';
...
export default class Producer extends EvemtEmitter3 {
  ...
  handleAttributes(
    ...
    Object.keys(newAttributes).forEach(key => {
      ...
      // 旧属性没有，新增属性
      if (!Object.hasOwn(oldAttributes, key)) {
      ...
      if (newAttr !== oldAttr) {
        ...
        if (key === CARD_VALUE_KEY) {
          ...
          if (component?.writeHistoryOnValueChange && component.writeHistoryOnValueChange(value) === false) {
            nl = true;
          }
        }
        ops.push({
    ...
    Object.keys(oldAttributes).forEach(key => {
      // 旧属性有，新增没有，删除
      if (!Object.hasOwn(newAttributes, key)) {
        ops.push({
          id: oId,
          nl: isLoadingCard,
          bi: oBi,
    ...
    if (isText) {
      if (newText !== oldText) {
        ...
        for (let i = 0; i < tOps.length; i++) {
          ops.push({ ...tOps[i], id: oId, bi: oBi, nl: isLoadingCard });
        }
      }
    }
  ...
  isLoadingCard = (child: any) => {
    if (child.length > JSON0_INDEX.ATTRIBUTE && !Array.isArray(child[1]) && typeof child[1] === 'object') {
      ...
      if (cardKey && cardType && dataId) {
        const cardNode = this.engine.container.get<Element>()?.querySelector(`[${DATA_ID}="${dataId}"]`);
  ...
  handleChildren = (
    ...
    // 2.2 旧节点有子节点数据
      ...
      // 2.2.3 新节点有子节点数据，并且不是字符
        ...
        // eslint-disable-next-line no-lonely-if
        if (
          newChildren.every(child => (Array.isArray(child) ? child[JSON0_INDEX.ATTRIBUTE][DATA_ID] : false)) &&
          oldChildren.every(child => (Array.isArray(child) ? child[JSON0_INDEX.ATTRIBUTE][DATA_ID] : false))
        ) {
          // 先找出需要删除的旧节点
          for (let c = oldChildren.length - 1; c >= 0; c--) {
            const oldChild = oldChildren[c];
            const newChild = newChildren.find(
              child => child[JSON0_INDEX.ATTRIBUTE][DATA_ID] === oldChild[JSON0_INDEX.ATTRIBUTE][DATA_ID]
            );
            if (!newChild) {
              ops.push({
                id: oId,
          ...
          for (let c = 0; c < newChildren.length; c++) {
            ...
            const oldChild = oldChildren.find(
              child => child[JSON0_INDEX.ATTRIBUTE][DATA_ID] === newChild[JSON0_INDEX.ATTRIBUTE][DATA_ID]
            );
            if (!oldChild) {
              ops.push({
                id,
            ...
            // 对比有差异的子节点
              ...
              // 比较属性
              const newAttributes = newChild[JSON0_INDEX.ATTRIBUTE] as Record<string, any>;
              const oldAttributes = oldChild[JSON0_INDEX.ATTRIBUTE] as Record<string, any>;
              const newPath = path.concat(c + JSON0_INDEX.ELEMENT);
              ops.push(...this.handleAttributes(id, oBi, newPath, oldAttributes, newAttributes, isLoadingCard));
              // 比较子节点
              ops.push(
                ...this.handleChildren(
                  id,
          ...
          for (let c = 0; c < newChildren.length; c++) {
            ...
            if (!oldChild) {
              ...
              if (
                ...
                newChild[JSON0_INDEX.TAG_NAME] === oldChild[JSON0_INDEX.TAG_NAME]
              ) {
                // 比较属性
                const newAttributes = newChild[JSON0_INDEX.ATTRIBUTE] as Record<string, any>;
                const oldAttributes = oldChild[JSON0_INDEX.ATTRIBUTE] as Record<string, any>;
  ...
  handleMutations(records: MutationRecord[]) {
    ...
    if (data.length === 0) {
      ...
      for (let r = 0; r < records.length; r++) {
        ...
        if (
          type === 'attributes' &&
          ((record.attributeName && isTransientAttribute(target, record.attributeName)) || isRT)
        ) {
          continue;
        }

        if (target instanceof Text) target = target.parentElement;
        if (!target || !target.isConnected || !(target instanceof Element) || isTransientElement(target)) continue;
        ...
        if (
          ...
          (!targetRoots.includes(target) && targetRoots.every(root => !root.contains(target)))
        ) {
  ...
  diff(root: Element, data: any = this.doc?.data || []) {
    ...
    if (!id) {
      const cRoot = this.engine.block.closest($(root), node => Boolean(node.attributes(DATA_ID))).get<Element>();
    ...
    const oldValue = isRootId ? data : findFromDoc(data, attributes => attributes[DATA_ID] === id);
    if (!oldValue) return [];
    // 比较新旧数据
    const { path } = oldValue;
    ...
    // 1. 根节点属性变更
    if (id !== 'root') {
      const newAttributes = newJson[JSON0_INDEX.ATTRIBUTE] as Record<string, any>;
      const oldAttributes = oldValue.attributes;
      ops.push(...this.handleAttributes(id, path.length, path, oldAttributes, newAttributes, isLoadingCard));
    }
    // 2. 子节点变更
    const newChildren = newJson.slice(2);
    const oldChildren = Array.isArray(oldValue) ? oldValue.slice(2) : oldValue.children;
    ops.push(
      ...this.handleChildren(
        isRootId ? '' : id,
