...
import type { ListDeleteOp, ListInsertOp, Op, Path, StringDeleteOp, StringInsertOp } from 'sharedb';
import type { NodeInterface } from '../types/node';
import { ANCHOR, CURSOR, FOCUS } from '../constants/selection';
import { CARD_EDITABLE_KEY, CARD_KEY, CARD_LOADING_KEY, CARD_SELECTOR, READY_CARD_KEY } from '../constants/card';
import {
  DATA_ELEMENT,
  DATA_ID,
...
import { getParentInRoot, toHex, unescape, unescapeDots } from '../utils';
...
export const isTransientElement = (
  ...
  if (element.nodeType === Node.ELEMENT_NODE) {
    ...
    // 范围标记
    if ([CURSOR, ANCHOR, FOCUS].includes(dataElement)) {
      return true;
    }

    // data-element=ui 属性
    if (Boolean(element.getAttribute(DATA_TRANSIENT_ELEMENT)) || dataElement === UI) {
      return true;
    }
    ...
    const closestNode = closest(element, `${CARD_SELECTOR},${UI_SELECTOR}`, getParentInRoot);
    ...
    // 当前是卡片，父级也是卡片
    const parentCard = closest(parent, CARD_SELECTOR, getParentInRoot) as Element;
...
export const isTransientAttribute = (node: NodeInterface | Node, attr: string) => {
  const element = (isNode(node) ? node : node[0]) as Element;
  if (isRoot(element)) return true;
  if (isCard(element) && ['id', 'class', 'style', CARD_LOADING_KEY, CARD_EDITABLE_KEY].includes(attr)) return true;
  const transient = element.getAttribute(DATA_TRANSIENT_ATTRIBUTES);
  if (
    transient === '*' ||
    (transient && transient.split(',').some(value => value.trim().toLowerCase() === attr.toLowerCase()))
  )
    return true;
  return false;
};
...
export const isReverseOp = (op: Op, next: Op) => {
  ...
  if (deleteStringOp.sd && insertStringNext.si) {
    return isEqual(deleteStringOp.sd, insertStringNext.si) && isEqual(op.p, next.p);
  }
...
const isReversePath = (op: Path, next: Path, length: number = 1): boolean => {
  ...
  nextClone[nextClone.length - 1] = (nextClone[nextClone.length - 1] as number) - length;
...
export const toDOM = (ops: Op[] | Op[][]): Node => {
  ...
  for (; i < ops.length; i++) {
    if (Array.isArray(ops[i])) {
      ...
      if (prevOp && prevOp.toString() === '[object Object]' && Object.keys(prevOp).includes(READY_CARD_KEY)) {
        continue;
      }
      const child = toDOM(ops[i] as Op[]);
      if (child) fragment.appendChild(child);
    } else if (ops[i].toString() === '[object Object]') {
      if (elementName) {
        ...
        if (attributeKeys.includes(CARD_KEY) && !attributeKeys.includes(READY_CARD_KEY)) {
          element?.setAttribute(READY_CARD_KEY, ops[i][CARD_KEY]);
          element?.removeAttribute(CARD_KEY);
        }
      }
    } else if (typeof ops[i] === 'number' || typeof ops[i] === 'string') {
...
const childToJSON0 = (node: NodeInterface | Node, values: Array<{} | string>) => {
...
export const toJSON0 = (node: NodeInterface | Node): string | undefined | (string | {})[] => {
...
export const getValue = (data: any, path: Path, id?: string) => {
  ...
  for (let i = 0; i < path.length && value !== undefined; i++) {
    value = value[path[i]];
    if (!hasValue && id && Array.isArray(value) && value.length > 0 && value[1][DATA_ID] === id) {
      hasValue = true;
    }
  }
...
export const findFromDoc = (data: any, callback: (attributes: Record<string, string>) => boolean): DocData | null => {
...
export const findCardForDoc = (
  ...
  if (result) {
    ...
    return {
      attributes,
      rendered: Array.isArray(children) && Array.isArray(children[2]) && Array.isArray(children[2][2])
    };
  }
};
