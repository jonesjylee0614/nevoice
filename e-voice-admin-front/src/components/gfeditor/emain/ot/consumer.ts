import type { Op, Path, StringInsertOp } from 'sharedb';
import Range from '../range';
import { unescape, unescapeDots } from '../utils/string';
import { JSON0_INDEX } from '../constants/ot';
import type { EngineInterface } from '../types/engine';
import type { ConsumerInterface, RemoteAttr, RemotePath, TargetOp } from '../types/ot';
import type { NodeInterface } from '../types/node';
import { getDocument } from '../utils';
import { $ } from '../node';
import { CARD_LOADING_KEY, DATA_ID, EDITABLE_SELECTOR } from '../constants';
import type { RangePath } from '../types';
...
class Consumer implements ConsumerInterface {
  ...
  fromRemoteAttr(attr: RemoteAttr) {
    ...
    const node = this.engine.container.get<Element>()?.querySelector(`[${DATA_ID}="${id}"]`);
    if (!node) return;
    ...
    if (text?.startsWith(leftText)) {
      ...
      while (nextChild && (nextChild.nodeType !== 3 || (nextChild.textContent?.length || 0) < offset)) {
    ...
    if (text?.endsWith(rightText)) {
      ...
      while (prevChild && (prevChild.nodeType !== 3 || (prevChild.textContent?.length || 0) < offset)) {
        if (prevChild.textContent?.length || offset > 0) {
    ...
    let nextChild: Node | null | undefined = node.firstChild;
    while (nextChild && (nextChild.nodeType !== 3 || (nextChild.textContent?.length || 0) < offset)) {
      if (nextChild.textContent?.length || offset > 0) {
        offset -= nextChild.textContent?.length || 0;
        nextChild = nextChild.nextSibling;
      } else {
        nextChild = nextChild.firstChild;
  ...
  setAttribute(root: NodeInterface, path: Path, attr: string, value: string, isRemote?: boolean) {
    const { card } = this.engine;
    ...
    if (domNode && domNode.length > 0 && !domNode.isRoot()) {
      ...
      if (domNode.isCard()) {
        ...
        if (component.isEditable && component.onChange) component.onChange(isRemote ? 'remote' : 'local', domNode);
      }
    }
    return domNode;
  }
  ...
  insertNode(root: NodeInterface, path: Path, value: string | Op[] | Op[][], isRemote?: boolean) {
    const { engine } = this;
    ...
    if (
      ...
      !domBegine.isRoot() &&
      (!root.isCard() ||
        (root.isEditableCard() && (domBegine.length === 0 || domBegine.closest(EDITABLE_SELECTOR).isEditable())))
    ) {
      const element = typeof value === 'string' ? document.createTextNode(value) : toDOM(value);
  ...
  insertText(root: NodeInterface, path: Path, offset: number, text: string) {
    ...
    const nodeValue = startNode && startNode.nodeValue ? startNode.nodeValue : '';
    const value = nodeValue.substring(0, offset) + text + nodeValue.substring(offset);
    if (startNode && startNode.parentNode === endNode) startNode.nodeValue = value;
    else if (value) {
  ...
  setRangeAfterOp(op: TargetOp) {
    ...
    if ('si' in op || 'sd' in op) {
      ...
      const rangeOffset = offset! + (stringInsertOp.si ? stringInsertOp.si.length : 0);
  ...
  setRangeByRemotePath(path: RemotePath) {
    try {
      ...
      const range = selection ? Range.from(this.engine, selection)?.cloneRange() : undefined;
      if (!range) return;
    ...
    if (path) {
      ...
      if (start && end) {
        ...
        const startChild = start.id
          ...
          : container.getChildByPath(startClone, child => !isTransientElement($(child)));
        if (!startChild) return;
        const endChild = end.id
          ? container.find(`[${DATA_ID}="${end.id}"]`).get<Node>()
          : container.getChildByPath(endClone, child => !isTransientElement($(child)));
        if (!endChild) return;
        ...
        try {
          ...
          if (startChild.nodeName === 'BR' || this.engine.node.isVoid(startChild)) {
          ...
          if (!range.collapsed) {
            ...
            if (startCard && endCard && startCard?.root.equal(endCard.root)) {
              let startEditableElement = range.startNode.closest(EDITABLE_SELECTOR);
