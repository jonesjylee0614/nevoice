...
import { escape, getListStyle, getStyleMap, removeUnit, transformCustomTags, unescape } from '../utils';
import { $ } from '../node';
...
class Parser implements ParserInterface {
  ...
  constructor(
    ...
    if (typeof source === 'string') {
      ...
      source = source.replace(/<p(>|\s+[^>]*>)/gi, '<paragraph$1').replace(/<\/p(>|\s+[^>]*>)/gi, '</paragraph$1');
  ...
  convert(conversion: ConversionInterface, node: NodeInterface, schema?: SchemaInterface) {
    ...
    while (value) {
      ...
      if (nodeApi.isBlock(newNode, schema)) {
        // 排除之前的过滤规则后再次过滤
        value = conversion.transform(newNode, r => !oldRules.includes(r));
        continue;
      }

      // 排除之前的过滤规则后再次过滤
      value = conversion.transform(node, r => !oldRules.includes(r));
    }
    return convertAfterNode;
  }
  normalize(root: NodeInterface, schema: SchemaInterface, conversion: ConversionInterface | null) {
    ...
    if (!this.isNormalize) {
      ...
      const cursors = root.find(`${CURSOR_SELECTOR},${ANCHOR_SELECTOR},${FOCUS_SELECTOR}`);
    ...
    const filter = (node: NodeInterface, type: string = 'mark') => {
      ...
      if (
        ...
        newNode.append(children.length > 0 ? children : type === 'block' ? '<br />' : $('\u200B', null));
        node.remove();
        node = newNode;
        return;
      }
    ...
    root.traverse(node => {
      ...
      if (node.isElement()) {
        ...
        if (nodeApi.isMark(node, schema)) {
          ...
          if (rule) {
            ...
            let type = schema.getType(
              newNode,
              rule => rule.name === newNode!.name && rule.type === 'mark' && !oldRules.includes(rule)
            );
            if (!type) {
              if (conversion) {
            ...
            while (type === 'mark') {
              ...
              appendTarget.append(children.length > 0 ? children : $('\u200B', null));
              ...
              if (!type) {
                if (conversion) {
                  const newChildren = this.convert(conversion, newNode, schema);
                  if (newChildren && newChildren.length > 0) {
                    newNode = newNode.length > 0 ? newNode.children() : newChildren;
                    type = 'mark';
                    continue;
                  }
                }
        ...
        if (/^\n/.test(text) || /^\s/.test(text)) {
          ...
          const prevType: string | undefined = prev ? schema.getType(prev) : undefined;
          const nextType: string | undefined = next ? schema.getType(next) : undefined;
          // 节点前面
  ...
  traverse(
    ...
    while (child) {
      ...
      if (child.isElement()) {
        ...
        // Card Combine 相关节点
        if (['left', 'right'].includes(attributes[CARD_ELEMENT_KEY])) {
          child = child.next();
          continue;
        }
        let passed = true;
        let type: 'inline' | 'block' | 'mark' | undefined;
  ...
  toValue(
    ...
    this.traverse(root, schema, conversion, {
      onOpen: (child, name, attributes, styles) => {
        ...
        if (nodeApi.isVoid(name, schema || undefined) && child.get<HTMLElement>()?.childNodes.length === 0) {
          result.push(' />');
        } else {
          result.push('>');
        }
      ...
      onText: (_, text) => {
        if (replaceSpaces && text.length > 1) {
          text = text.replace(/[\u00A0 ]+/g, item => {
            const strArray = [];
            item = item.replace(/\u00A0/g, ' ');
            for (let n = 0; n < item.length; n++) strArray[n] = n % 2 == 0 ? item[n] : ' ';
      ...
      onClose: (_, name) => {
        if (nodeApi.isVoid(name, schema || undefined)) return;
  ...
  /** 返回DOM树 */
  toDOM(schema: SchemaInterface | null = null, conversion: ConversionInterface | null) {
   ...
   */
  toText(schema: SchemaInterface | null = null, includeCard?: boolean, formatOL: boolean = true) {
    const root = this.root.clone(true);
    const result: Array<string> = [];
    const editor = this.editor;
    this.traverse(
