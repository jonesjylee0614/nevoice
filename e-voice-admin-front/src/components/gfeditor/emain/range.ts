import type { Path } from 'sharedb';
import type { NodeInterface } from './types/node';
import type { RangeInterface, RangePath } from './types/range';
import { isMobile } from './utils';
import { CARD_ELEMENT_KEY, CARD_LEFT_SELECTOR, CARD_RIGHT_SELECTOR, CARD_SELECTOR } from './constants/card';
import { ANCHOR, CURSOR, FOCUS } from './constants/selection';
import { DATA_ELEMENT, DATA_ID, DATA_TRANSIENT_ELEMENT, UI } from './constants/root';
import Selection from './selection';
import type { SelectionInterface } from './types/selection';
import type { EditorInterface } from './types/editor';
import { $ } from './node';
...
class Range implements RangeInterface {
  ...
  static create: (editor: EditorInterface, doc?: Document, point?: { x: number; y: number }) => RangeInterface;
  static from: (
  ...
  insertNode(node: Node | NodeInterface): void {
    ...
    if (!$(node).isCursor() && startNode.get<Node>()?.childNodes.length === 1 && startNode.first()?.name === 'br') {
  ...
  /** 将选择标记从 TextNode 扩大到最近非TextNode节点 range 实质所选择的内容不变 */
  enlargeFromTextNode = () => {
  ...
  /** 将选择标记从非 TextNode 缩小到TextNode节点上，与 enlargeFromTextNode 相反 range 实质所选择的内容不变 */
  shrinkToTextNode = () => {
  ...
  /**
   * 扩大边界<p><strong><span>[123</span>abc]</strong>def</p> to<p>[<strong><span>123</span>abc</strong>]def</p>
   *
   * @param range 选区
   * @param toBlock 是否扩大到块级节点
   * @param toTop 是否尽可能扩大的可编辑节点下
   */
  enlargeToElementNode = (toBlock: boolean = false, toTop: boolean = true) => {
  ...
  /**
   * 缩小边界<body>[<p><strong>123</strong></p>]</body> to<body><p><strong>[123]</strong></p></body>
   *
   * @param range 选区
   */
  shrinkToElementNode = () => {
    ...
    while (
      ...
      !childDom.isCursor() &&
      !node.isVoid(child) &&
      (!childDom.isCard() || childDom.isEditableCard() || childDom.find(CARD_LEFT_SELECTOR).length > 0)
    ) {
    ...
    while (
      ...
      !node.isVoid(child) &&
      !childDom.isCursor() &&
      (!childDom.isCard() || childDom.isEditableCard() || childDom.find(CARD_RIGHT_SELECTOR).length > 0)
    ) {
   ...
   */
  getSubRanges = (includeCard: boolean = false, filterSingleSelectableCard = true) => {
    ...
    this.commonAncestorNode.traverse(child => {
      if (child.isText()) {
        ...
        if (
          !cardComponent ||
          (filterSingleSelectableCard && (cardComponent.constructor as CardEntry).singleSelectable === false)
        )
          return;
  ...
  findElements = () => {
    const { startContainer, endContainer, startOffset, endOffset, collapsed } = this;
  ...
  scrollRangeIntoView = () => {
    ...
    const rect = this.collapsed ? (root as Element).getBoundingClientRect() : this.getClientRect();
  ...
  scrollIntoViewIfNeeded = (container = this.editor.container, view: NodeInterface) => {
    if (this.collapsed) {
  ...
  containsCard = () => {
    ...
    return (
      !collapsed &&
      ((commonAncestorNode.type !== 3 && commonAncestorNode.find(CARD_SELECTOR).length > 0) ||
  ...
  handleBr = (isLeft?: boolean) => {
    ...
    block.find('br').each(br => {
      ...
      if (
        ((!prev || (parent?.hasClass(list.CUSTOMZIE_LI_CLASS) && parent?.first()?.equal(prev))) &&
          next &&
          next.name !== 'br' &&
          !next.isCursor()) ||
        (!next && prev && prev.name !== 'br')
      ) {
        if (isLeft && prev && !(parent?.hasClass(list.CUSTOMZIE_LI_CLASS) && parent?.first()?.equal(domBr.prev()!)))
          return;
        domBr.remove();
      }
    });
    ...
    if (!first || (children.length === 1 && block.hasClass(list.CUSTOMZIE_LI_CLASS) && first?.isCard())) {
      block.append($('<br />'));
      return this;
    }
    ...
    if (
      ...
      first?.isCard() &&
      ['cursor', 'anchor', 'focus'].includes(block.last()?.attributes(DATA_ELEMENT) || '')
    ) {
  ...
  /** 获取开始位置前的节点 <strong>foo</strong>|bar */
  getPrevNode = () => {
  ...
  /** 获取结束位置后的节点 foo|<strong>bar</strong> */
  getNextNode = () => {
    this.enlargeFromTextNode();
  ...
  filterPath(includeCardCursor: boolean = false) {
    ...
    return (node: Node) => {
      const element = $(node);
      if (includeCardCursor && ['left', 'right', 'center', 'body'].includes(element.attributes(CARD_ELEMENT_KEY))) {
  ...
  toPath(includeCardCursor: boolean = false, root: NodeInterface = this.editor.container) {
    ...
    const getPath = (node: NodeInterface, offset: number): RangePath => {
      ...
      const path = node.getPath(
        root,
        node.parent()?.isRoot() ? undefined : this.filterPath(includeCardCursor),
        (index, path, node) => {
          // 找不到索引，就重置之前的位置
          if (index === -1) {
            rootBeginId = '';
...
Range.fromPath = (
  ...
  const getNode = (path: Path, context: Element = editor.container.get<Element>()!) => {
    ...
    for (let i = 0; i < path.length; i++) {
      ...
      while (domChild) {
        if (
          ...
          (!domChild.getAttribute(DATA_TRANSIENT_ELEMENT) && domChild.getAttribute(DATA_ELEMENT) !== UI) ||
          (includeCardCursor && ['left', 'right'].includes(domChild.getAttribute(CARD_ELEMENT_KEY) || ''))
        ) {
  ...
  const setRange = (method: string, range: RangeInterface, node: Node | null, offset: number) => {
    if (node !== null) {
      if (offset < 0) {
        offset = 0;
      }
  ...
  const startNode = getNode(
    path.start.bi > -1 && beginContext instanceof Element ? startPath.slice(path.start.bi) : startPath,
    beginContext instanceof Element ? beginContext : undefined
  );
  const endContext = path.end.id ? root.get<Element>()?.querySelector(`[${DATA_ID}="${path.end.id}"]`) : root;
  const endNode = getNode(
    path.end.bi > -1 && endContext instanceof Element ? endPath.slice(path.end.bi) : endPath,
...
export const isSelection = (param: Window | globalThis.Selection | globalThis.Range): param is globalThis.Selection => {
  return (param as globalThis.Selection).getRangeAt !== undefined;
};
export const isRange = (param: Window | globalThis.Selection | globalThis.Range): param is globalThis.Range => {
  return (param as globalThis.Range).collapsed !== undefined;
};
export const isRangeInterface = (selector: NodeInterface | RangeInterface): selector is RangeInterface => {
  return Boolean(selector) && (<RangeInterface>selector).base !== undefined;
};
