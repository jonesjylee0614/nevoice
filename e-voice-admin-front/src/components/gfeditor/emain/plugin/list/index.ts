import type { NodeInterface, PluginOptions } from '../../types';
import { CARD_KEY, READY_CARD_KEY } from '../../constants';
import type { ListInterface } from '../../types/list';
import type { PluginEntry as PluginEntryType } from '../../types/plugin';
import BlockEntry from '../block';
import { $ } from '../../node';
import { isEngine } from '../../utils';
import './index.css';

abstract class ListEntry<T extends PluginOptions = PluginOptions> extends BlockEntry<T> implements ListInterface<T> {
  cardName?: string;
  ...
  queryState() {
    ...
    return editor.list.getPluginNameByNodes(editor.change.blocks) === (this.constructor as PluginEntryType).pluginName;
  }
  ...
  pasteInsert = () => {
    ...
    if (customizeItems && customizeItems.length > 0) {
      customizeItems.each(node => {
        const domNode = $(node);
        if (domNode.find(`[${CARD_KEY}=${this.cardName}],[${READY_CARD_KEY}=${this.cardName}]`).length === 0)
