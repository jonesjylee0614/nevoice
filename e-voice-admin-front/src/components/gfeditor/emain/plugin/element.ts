...
import type { ConversionData, ElementPluginInterface, NodeInterface, PluginInterface, PluginOptions } from '../types';
import type { SchemaAttributes, SchemaGlobal, SchemaRule, SchemaStyle, SchemaValue } from '../types/schema';
import { $ } from '../node';
  ...
  implements ElementPluginInterface<T>
{
  ...
  /**
   * 标签样式，可选 使用变量表示值时，固定规则：@var0 @var1 @var2 ... 分别表示执行 command.execute 时传入的 参数1 参数2 参数3 ... {
   * value:string,format:(value:string) => string } 可以在获取节点属性值时，对值进行自定义格式化处理
   */
  readonly style?: {
    [key: string]: string | { value: string; format: (value: string) => string };
  };
  /**
   * 标签属性，可选 使用变量表示值时，固定规则：@var0 @var1 @var2 ... 分别表示执行 command.execute 时传入的 参数1 参数2 参数3 ... {
   * value:string,format:(value:string) => string } 可以在获取节点属性值时，对值进行自定义格式化处理
   */
  readonly attributes?: {
    [key: string]: string | { value: string; format: (value: string) => string };
  };
  /** 在 style 或者 attributes 使用变量表示的值规则 key 为如上所诉的变量名称 @var0 @var1 @var2 ... */
    ...
    if (this.style) {
      Object.keys(this.style).forEach(styleName => {
        ...
        styleValue.match(/@var\d/g)?.forEach(regMatch => {
          const index = Number.parseInt(regMatch.replace('@var', ''), 10);
          styleValue = (styleValue as string).replace(new RegExp(regMatch, 'gm'), args[index] || '');
        });
  ...
  setAttributes(node: NodeInterface | Node, ...args: Array<any>) {
    ...
    if (this.attributes) {
      Object.keys(this.attributes).forEach(attributesName => {
        ...
        if (typeof attributesValue === 'object') attributesValue = attributesValue.value;
        // 替换变量
        attributesValue.match(/@var\d/g)?.forEach(regMatch => {
          const index = Number.parseInt(regMatch.replace('@var', ''), 10);
          attributesValue = (attributesValue as string).replace(new RegExp(regMatch, 'gm'), args[index] || '');
        });
  ...
  getStyle(node: NodeInterface | Node) {
    ...
    if (this.style && this.isSelf(node)) {
      Object.keys(this.style).forEach(styleName => {
        ...
        let value = styleName.toLowerCase().includes('color')
  ...
  isSelf(node: NodeInterface | Node) {
    if (isNode(node)) node = $(node);
    let schema: SchemaRule | SchemaGlobal | Array<SchemaRule> | undefined = this.schema();
    if (Array.isArray(schema)) schema = schema.find(({ name }) => name === (node as NodeInterface).name);
    if (!schema) return false;
    return (
      (Array.isArray(this.tagName) ? this.tagName.includes(node.name) : node.name === this.tagName) &&
  ...
  schema(): SchemaRule | SchemaGlobal | Array<SchemaRule> {
    ...
    if (typeof this.tagName === 'string') (this.sechamCache as SchemaRule).name = this.tagName.toLowerCase();
...
export const isElementPlugin = (plugin: PluginInterface): plugin is ElementPluginInterface => {
