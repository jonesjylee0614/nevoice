  ...
  implements InlineInterface<T>
{
  ...
  queryState() {
    ...
    if (!this.style && !this.attributes) return change.inlines.some(node => node.name === this.tagName);
    // 获取属性和样式限制内的值集合
...
export const isInlinePlugin = (plugin: PluginInterface): plugin is InlineInterface => {
