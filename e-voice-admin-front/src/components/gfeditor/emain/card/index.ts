...
import type { CardEntry, CardInterface, CardModelInterface, CardValue } from '../types/card';
import type { NodeInterface } from '../types/node';
import type { RangeInterface } from '../types/range';
import type { EditorInterface } from '../types/editor';
import { decodeCardValue, encodeCardValue, isEngine, transformCustomTags } from '../utils';
import { $ } from '../node';
import { isNode } from '../node/utils';
import { toJSON0 } from '../ot/utils';
import { CardActiveTrigger, CardType } from './enum';
import { Backspace, Default, Down, Enter, Left, Right, Up } from './typing';
import './index.css';
...
class CardModel implements CardModelInterface {
  ...
  init(cards: Array<CardEntry>) {
    ...
    if (isEngine(editor)) {
      ...
      typing.getHandleListener('enter', 'keydown')?.on(event => enter.trigger(event));
      // 删除事件
      const backspace = new Backspace(editor);
      typing.getHandleListener('backspace', 'keydown')?.on(event => backspace.trigger(event));
      // 方向键事件
      const left = new Left(editor);
      typing.getHandleListener('left', 'keydown')?.on(event => left.trigger(event));

      const right = new Right(editor);
      typing.getHandleListener('right', 'keydown')?.on(event => right.trigger(event));

      const up = new Up(editor);
      typing.getHandleListener('up', 'keydown')?.on(event => up.trigger(event));

      const down = new Down(editor);
      typing.getHandleListener('down', 'keydown')?.on(event => down.trigger(event));

      const _default = new Default(editor);
      typing.getHandleListener('default', 'keydown')?.on(event => _default.trigger(event));
    }
    ...
    editor.scrollNode?.get<HTMLElement>()?.addEventListener('scroll', this.renderAsyncComponents);
  ...
  renderAsyncComponents = async () => {
    ...
    this.renderTimeout = setTimeout(() => {
      ...
      components.forEach(async card => {
        ...
        if (
          card.root.length === 0 ||
          (editor.root.inViewport(card.root, true) && editor.trigger('card:async-render-component', card) !== false)
        ) {
  ...
  find<E extends CardValue = {}, T extends CardInterface<E> = CardInterface<E>>(
    ...
    const cards = this.components.filter(item => {
      ...
      if (item.root.name !== (isNode(selector) ? selector.nodeName.toString().toLowerCase() : selector.name))
        return false;
  ...
  getSingleCard<E extends CardValue = {}, T extends CardInterface<E> = CardInterface<E>>(range: RangeInterface) {
    let card = this.find<E, T>(range.commonAncestorNode);
    if (!card) card = this.getSingleSelectedCard<E, T>(range);
    return card;
  }

  getSingleSelectedCard<E extends CardValue = {}, T extends CardInterface<E> = CardInterface<E>>(
  ...
  // 插入Card
  insertNode<E extends CardValue = {}, T extends CardInterface<E> = CardInterface<E>>(
    range: RangeInterface,
    card: T,
    ...args: any
  ) {
  ...
  replaceNode(node: NodeInterface, name: string, value?: CardValue) {
    ...
    const cardNode = transformCustomTags(
      `<card ${dataId ?? `${DATA_ID}="${dataId}"`} type="${type}" name="${name}" value="${encodeCardValue(
        value
      )}"></card>`
    );
  ...
  activate(node: NodeInterface, trigger?: CardActiveTrigger, event?: MouseEvent) {
    const editor = this.editor;
    ...
    if (!container.get() || editor.container.equal(container)) {
      ...
      const isCurrentActiveCard = card && active && active.root.equal(card.root);
      ...
      if (card) {
        ...
        if (!isCurrentActiveCard) {
          ...
          if (!card.isEditable && (card.constructor as CardEntry).autoSelected !== false) card.select(!card.isEditable);
          card.activate(true);
    ...
    if (
      ...
      if (
        (range.startNode.closest(EDITABLE_SELECTOR).length > 0 &&
          (!event || (event instanceof MouseEvent && (!event.target || !this.closest(event.target as Node, false))))) ||
        card.isEditable ||
        card.isMaximize
      )
        return;
  ...
  insert<E extends CardValue = {}, T extends CardInterface<E> = CardInterface<E>>(
    name: string,
    value?: E,
    ...args: any
  ) {
  ...
  update(selector: NodeInterface | Node | string, value: CardValue, ...args: any) {
    const editor = this.editor;
    if (!isEngine(editor)) return;
    const change = editor.change;
  ...
  replace<E extends CardValue = {}, T extends CardInterface<E> = CardInterface<E>>(
  ...
  // 创建Card DOM 节点
  create<E extends CardValue = {}, T extends CardInterface<E> = CardInterface<E>>(
    name: string,
    options?: {
      value?: E;
      root?: NodeInterface;
    ...
    // center
    const center = $(`<${tagName} ${component.isEditable ? `${DATA_TRANSIENT_ATTRIBUTES}='*'` : ''}/>`);
    center.attributes(CARD_ELEMENT_KEY, 'center');

    // body
    const body = $('<'.concat(tagName, ' ').concat(CARD_ELEMENT_KEY, '="body" />'));
    // center
    body.append(center);
    root.append(body);
  ...
  createCursor(component: CardInterface) {
    ...
    const hasFocus = cardClazz.focus !== undefined ? cardClazz.focus : isEngine(editor) && !editor.readonly;
    ...
    if (hasFocus) {
      ...
      if (!left) {
        center.before($(`<span ${CARD_ELEMENT_KEY}="left" ${DATA_TRANSIENT_ELEMENT}="true">&#8203;</span>`));
      }
      if (!right) {
        center.after($(`<span ${CARD_ELEMENT_KEY}="right" ${DATA_TRANSIENT_ELEMENT}="true">&#8203;</span>`));
      }
    } else {
      left?.remove();
      right?.remove();
   ...
   */
  render(container?: NodeInterface, callback?: (count: number) => void, lazyRender = this.lazyRender) {
    ...
    allCards.forEach(cardNode => {
      ...
      const childCards = cardNode.find(`${CARD_SELECTOR},${READY_CARD_SELECTOR}`).toArray();
    ...
    readyCards.forEach(cardNode => {
      ...
      if (this.classes[name]) {
        ...
        Object.keys(attributes).forEach(attributesName => {
          if (
            (attributesName.indexOf('data-') === 0 && attributesName.indexOf('data-card') !== 0) ||
    ...
    asyncRenderCards.forEach(card => {
      // 可编辑卡片在远程模式下不进行异步渲染
      if (lazyRender && (card.constructor as CardEntry).lazyRender && (!card.isEditable || !isRemote)) {
    ...
    if (card.contenteditable.length > 0) {
      center.find(card.contenteditable.join(',')).each(node => {
        ...
        if (child.attributes(DATA_CONTENTEDITABLE_KEY) !== undefined)
          child.attributes(DATA_CONTENTEDITABLE_KEY, !isEngine(editor) || editor.readonly ? 'false' : 'true');
  ...
  destroy() {
    ...
    editor.scrollNode?.get<HTMLElement>()?.removeEventListener('scroll', this.renderAsyncComponents);
    window.removeEventListener('scroll', this.renderAsyncComponents);
    editor.off('card:async-render', this.renderAsyncComponents);
  }

  // 焦点移动到上一个 Block
  focusPrevBlock(card: CardInterface, range: RangeInterface, hasModify: boolean) {
    const editor = this.editor;
  ...
  // 焦点移动到下一个 Block
  focusNextBlock(card: CardInterface, range: RangeInterface, hasModify: boolean) {
    const editor = this.editor;
    if (!isEngine(editor)) throw 'Engine not initialized';
    let nextBlock;
    if (card.type === 'inline') {
