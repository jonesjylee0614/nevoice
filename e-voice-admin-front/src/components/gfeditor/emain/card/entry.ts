...
import type { ToolbarItemOptions } from '../types/toolbar';
import type { TinyCanvasInterface } from '../types/tiny-canvas';
...
abstract class CardEntry<T extends CardValue = CardValue> implements CardInterface<T> {
  ...
  get loading() {
    return Boolean(this.root.attributes(CARD_LOADING_KEY));
  }
  ...
  private getId(curId?: string) {
    ...
    if (curId && !idCache.includes(curId)) return curId;
    let id = random();
    while (idCache.includes(id)) id = random();
    return id;
  }
  ...
  findByKey(key: string) {
    ...
    if (index > -1) {
      return children.toArray().find(child => child.attributes(CARD_ELEMENT_KEY) === key);
    }
  ...
  isCenter(node: NodeInterface) {
    const center = node.closest(
      this.type === CardType.BLOCK ? `div[${CARD_ELEMENT_KEY}=center]` : `span[${CARD_ELEMENT_KEY}=center]`
    );
    return center.length > 0 && Boolean(this.findByKey('center')?.equal(center));
  }
  ...
  isLeftCursor(node: NodeInterface) {
    ...
    return cursor.length > 0 && Boolean(this.findByKey('left')?.equal(cursor));
  }
  ...
  isRightCursor(node: NodeInterface) {
    ...
    return cursor.length > 0 && Boolean(this.findByKey('right')?.equal(cursor));
  }
  ...
  private initToolbar() {
    if (this.toolbar) {
      if (!this.toolbarModel) this.toolbarModel = new Toolbar(this.editor, this);
      if (this.activated) {
        this.toolbarModel.show();
      }
    } else {
  ...
  private initResize() {
    if (this.resize) {
      ...
      const container = typeof this.resize === 'function' ? this.resize() : this.findByKey('body');
  ...
  beforeRender() {
    ...
    const loadingElement = $(
      `<${this.type === CardType.BLOCK ? 'div' : 'span'} class="${CARD_LOADING_KEY}" ${DATA_ELEMENT}="${UI}" />`
    );
