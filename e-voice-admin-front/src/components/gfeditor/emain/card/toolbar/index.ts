...
import { DATA_CONTENTEDITABLE_KEY, DATA_ELEMENT, TRIGGER_CARD_ID, UI } from '../../constants';
import { $ } from '../../node';
...
export const isCardToolbarItemOptions = (
  ...
  return !['button', 'input', 'dropdown', 'node', 'switch'].includes(item.type);
};
...
class CardToolbar implements CardToolbarInterface {
  ...
  defaultAlign: Placement = 'topLeft';
  ...
  setDefaultAlign(align: Placement) {
    ...
    switch (item.type) {
      ...
      case 'copy':
        return {
          key: 'copy',
          type: 'button',
          content: item.content || `<span class="data-icon data-icon-copy"></span>`,
          ...
          onClick: (e, node) => {
            ...
            if (result) editor.messageSuccess('copy', language.get<string>('copy', 'success'));
            else editor.messageError('copy', language.get<string>('copy', 'error'));
          }
        };
      case 'delete':
        return {
          key: 'delete',
          type: 'button',
          content: item.content || `<span class="data-icon data-icon-delete"></span>`,
      ...
      case 'maximize':
        return {
          key: 'maximize',
          type: 'button',
          content: item.content || `<span class="data-icon data-icon-maximize"></span>`,
      ...
      case 'more':
        return {
          key: 'more',
          type: 'dropdown',
          content: item.content || `<span class="data-icon data-icon-more"></span>`,
  ...
  create() {
    ...
    if (items.length > 0) {
      const dnd: CardToolbarItemOptions | undefined = config.find(
        item => isCardToolbarItemOptions(item) && (item as CardToolbarItemOptions).type === 'dnd'
    ...
    if (!this.card.isMaximize) {
      if (this.dndNode.length > 0) {
        ...
        setTimeout(() => {
          ...
          this.position.bind(this.dndNode!, this.card.root, 'leftTop', this.offset);
        }, 10);
      }
    } else {
  ...
  showCardToolbar(event?: MouseEvent): void {
    ...
    if (container && container.length > 0) {
      ...
      container.attributes('toolbar-trigger-key', (card.constructor as CardEntry).cardName);
      ...
      setTimeout(() => {
        position.bind(
          ...
          this.offset,
          rect => {
            if (this.offset && this.offset.length === 4 && rect.align === 'bottomLeft' && rect.align !== prevAlign) {
              position.setOffset([this.offset[2], this.offset[3]]);
              prevAlign = rect.align;
              position.update(false);
            } else if (this.offset && rect.align === this.defaultAlign && rect.align !== prevAlign) {
  ...
  private createDnd(content: string, title: string) {
    const dndNode = $(
      ...
      }" drag-card-trigger="${this.card.id}" ${DATA_CONTENTEDITABLE_KEY}="false">
