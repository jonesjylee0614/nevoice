...
export const getDocument = (node?: Node): Document => {
  if (typeof document === 'undefined' && typeof global.__amWindow === 'undefined')
 ...
 * @returns {Array} 获取的文本节点
 */
export const getTextNodes = (node: Node, filter?: (node: Node) => boolean): Array<Node> => {
...
export const getParentInRoot = (node: Node) => {
  return node.nodeType === getDocument().ELEMENT_NODE && (node as Element).getAttribute(DATA_ELEMENT) === ROOT
    ? undefined
