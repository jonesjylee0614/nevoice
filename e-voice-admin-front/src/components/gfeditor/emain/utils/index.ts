...
import type { EditorInterface, EngineInterface, ViewInterface } from '../types';
 ...
 * @param editor 编辑器
 */
export const isEngine = (editor: EditorInterface): editor is EngineInterface => {
...
export const createMarkdownIt = (editor: EditorInterface, presetName: Markdown.PresetName = 'default') => {
...
export const convertMarkdown = (
  ...
  tokens.forEach((token, index) => {
    ...
    if (type === 'inline' && children) {
      ...
      if (checkInline && children.find(child => child.type === 'image' || child.type.endsWith('_open'))) {
        isHit = true;
      }
    ...
    if (nesting === 1) {
      ...
      if (nodeContent.length > 0 && !nodeContent[nodeContent.length - 1] && blockTags.includes(tag))
