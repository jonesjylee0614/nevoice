...
import { CARD_EDITABLE_KEY, CARD_TYPE_KEY, CARD_VALUE_KEY, READY_CARD_KEY } from '../constants/card';
 ...
 */
export const toCamelCase = (value: string, type: CamelCaseType = CamelCaseType.LOWER): string => {
  return value
    .split('-')
    .map((str, index) => {
...
export const toHex = (color: string): string => {
  const hex = (num: string) => {
    const numChar = Number.parseInt(num, 10).toString(16).toUpperCase();
...
export const getAttrMap = (value: string): { [k: string]: string } => {
  ...
  while ((match = reg.exec(value))) {
    const key: string = (match[1] || match[2] || match[4] || match[6]).toLowerCase();
    const val: string = (match[2] ? match[3] : match[4] ? match[5] : match[7]) || '';
    map[key] = val;
  }
...
export const addUnit = (value: string | number, unit: string = 'px') => {
  return value && /^-?\d+(?:\.\d+)?$/.test(value.toString()) ? value + unit : value;
};
...
export const removeUnit = (value: string) => {
  ...
  return value && (match = /^((-?\d+)(\.\d+)?)/.exec(value))
    ? Math.floor(Number.parseFloat(match[1]) * 10000) / 10000
    : 0;
};
...
export const transformCustomTags = (value: string) => {
  return value
    .replace(/<anchor\s*\/>/gi, '<span '.concat(DATA_ELEMENT, '="').concat(ANCHOR, '"></span>'))
    .replace(/<focus\s*\/>/gi, '<span '.concat(DATA_ELEMENT, '="').concat(FOCUS, '"></span>'))
    .replace(/<cursor\s*\/>/gi, '<span '.concat(DATA_ELEMENT, '="').concat(CURSOR, '"></span>'))
...
export const formatEngineValue = (value: string) => {
  ...
  return /^<p(\s[^>]*?)><br \/><\/p>$/i.test(newValue) ? value.replace(RegExp.$1, '') : value;
};
