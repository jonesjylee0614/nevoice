...
class Scrollbar extends EvemtEmitter3 {
  ...
  setContentNode(content?: NodeInterface | Node) {
    this.content = content ? (isNode(content) ? $(content) : content) : content;
  }
  ...
  init() {
    ...
    if (!hasScrollbar) {
      ...
      if (this.shadow) {
        this.shadowLeft = $(`<div ${DATA_ELEMENT}="${UI}" class="scrollbar-shadow-left"></div>`);
        this.shadowRight = $(`<div ${DATA_ELEMENT}="${UI}" class="scrollbar-shadow-right"></div>`);
  ...
  getWidth() {
    ...
    const offsetWidth = this.scrollval?.getOffsetWidth ? this.scrollval.getOffsetWidth(width) : width;
    return offsetWidth;
  }
  ...
  refresh = () => {
    ...
    if (element) {
      ...
      const scrollWidth = contentElement ? this.content!.width() + sPLeft + sPRight : element.scrollWidth;
      const scrollHeight = contentElement ? this.content!.height() + sPTop + sPBottom : element.scrollHeight;
      ...
      if (this.x) {
        ...
        const display =
          ...
          (contentElement && Math.round(this.content!.width()) <= Math.round(this.oWidth - sPLeft - sPRight))
            ? 'none'
            : 'block';
      ...
      if (this.y) {
        ...
        const display =
          ...
          (contentElement && Math.round(this.content!.height()) <= Math.round(this.oHeight - sPTop - sPBottom))
            ? 'none'
            : 'block';
      ...
      if (this.x && contentElement && element.scrollWidth - sPLeft - sPRight > this.content!.width()) {
        this.isScrolling = true;
        let left = element.scrollWidth - sPLeft - sPRight - this.content!.width();
        if (this.scrollval) {
          const { onScrollX, getScrollLeft } = this.scrollval;

          left = getScrollLeft ? getScrollLeft(-0) + element.scrollLeft - left : element.scrollLeft - left;
          if (left < 0) left = 0;
          if (onScrollX) {
      ...
      if (this.y && contentElement && element.scrollHeight - sPTop - sPBottom !== this.content!.height()) {
        this.isScrolling = true;
        element.scrollTop -= element.scrollHeight - sPTop - sPBottom - this.content!.height();
        scrollEnd();
        return;
      }
  ...
  wheelXScroll = throttle(
    (event: any) => {
      ...
      const dir = (isMacos ? event.wheelDeltaX : event.wheelDelta / 120 || -event.detail) > 0 ? 'up' : 'down';
      const containerElement = this.container.get<HTMLElement>();
      if (!containerElement) return;
      const width = this.container.width();
      const containerWidth = this.scrollval?.getOffsetWidth ? this.scrollval.getOffsetWidth(width) : width;
      const step = Math.max(containerWidth / (isMacos ? 20 - Math.abs(event.wheelDelta) : 8), 20);
      ...
      left = dir === 'up' ? Math.max(0, left) : Math.min(left, this.sWidth - this.oWidth);
      if (this.scrollval) {
        const { onScrollX } = this.scrollval;
        if (onScrollX) {
          const result = onScrollX(left);
  ...
  wheelYScroll = throttle(
    (event: any) => {
      ...
      const step = Math.max(containerHeight / (isMacos ? 20 - Math.abs(event.wheelDelta) : 8), 20);
      let top = containerElement.scrollTop + (dir === 'up' ? -step : step);
      top = dir === 'up' ? Math.max(0, top) : Math.min(top, this.sHeight - this.oHeight);
  ...
  bindWheelScroll = (event: any) => {
    ...
    const isX =
      this.x && event.wheelDeltaX !== event.wheelDeltaY && Math.abs(event.wheelDeltaX) > Math.abs(event.wheelDeltaY);
    if (isX) {
      if (this.slideX && this.slideX.css('display') !== 'none') {
        this.wheelXScroll(event);
      }
    } else if (this.y && event.wheelDeltaY !== 0) {
      if (this.slideY && this.slideY.css('display') !== 'none') this.wheelYScroll(event);
    }
  };
  ...
  scrollX = (event: MouseEvent | TouchEvent) => {
    if (this.slideXDragging) {
      ...
      let left = this.reverse ? position - (offset.x - point) : position + (offset.x - point);
  ...
  scrollY = (event: MouseEvent | TouchEvent) => {
    if (this.slideYDragging) {
      ...
      const offset = this.getEventClientOffset(event);
      let top = this.reverse ? position - (offset.y - point) : position + (offset.y - point);
      top = Math.max(0, Math.min(top, this.oHeight - this.yHeight));
      this.slideY?.css('top', `${top}px`);
      let min = top / (this.oHeight - this.yHeight);
      min = Math.min(1, min);
      this.container.get<HTMLElement>()!.scrollTop = (this.sHeight - this.oHeight) * min;
    }
  };
  ...
  scrollXEnd = () => {
    ...
    this.reverse = false;
    document.body.removeEventListener(isMobile ? 'touchmove' : 'mousemove', this.scrollX);
    document.body.removeEventListener(isMobile ? 'touchend' : 'mouseup', this.scrollXEnd);
    this.container.removeClass('scrolling');
  };

  scrollYEnd = () => {
    this.slideYDragging = undefined;
    document.body.removeEventListener(isMobile ? 'touchmove' : 'mousemove', this.scrollY);
    document.body.removeEventListener(isMobile ? 'touchend' : 'mouseup', this.scrollYEnd);
    this.container.removeClass('scrolling');
  };
  ...
  scrollXStart = (event: MouseEvent | TouchEvent) => {
    ...
    this.slideXDragging = {
      point: offset.x,
      position: Number.parseInt(this.slideX?.css('left') || '0')
    };
    document.body.addEventListener(isMobile ? 'touchmove' : 'mousemove', this.scrollX);
    document.body.addEventListener(isMobile ? 'touchend' : 'mouseup', this.scrollXEnd);
  };
  ...
  scrollYStart = (event: MouseEvent | TouchEvent) => {
    ...
    this.slideYDragging = {
      point: offset.y,
      position: Number.parseInt(this.slideY?.css('top') || '0')
    };
    document.body.addEventListener(isMobile ? 'touchmove' : 'mousemove', this.scrollY);
    document.body.addEventListener(isMobile ? 'touchend' : 'mouseup', this.scrollYEnd);
  };

  bindXScrollEvent = () => {
    if (this.x) {
      this.slideX?.on(isMobile ? 'touchstart' : 'mousedown', this.scrollXStart);
    }
  };

  bindYScrollEvent = () => {
    if (this.y) {
      this.slideY?.on(isMobile ? 'touchstart' : 'mousedown', this.scrollYStart);
    }
  };
  ...
  reRenderShadow = (width: number) => {
    if (this.shadow) {
      const element = this.container.get<HTMLElement>();
      if (element) {
        this.shadowLeft?.css('left', `${this.scrollval ? element.scrollLeft : width}px`);
      }
