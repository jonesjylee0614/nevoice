...
class Selection implements SelectionInterface {
  ...
  has() {
    return Boolean(this.focus) && Boolean(this.anchor);
  }
  ...
  move() {
    ...
    if (this.key) {
      ...
      if (!this.focus.inEditor() || !this.focus.get<Element>()?.isConnected) {
        this.focus = root.find(`[data-${this.focus.attributes(DATA_ELEMENT)}-id="${this.key}"]`);
      }
      if (!this.anchor.inEditor() || !this.anchor.get<Element>()?.isConnected) {
        this.anchor = root.find(`[data-${this.anchor.attributes(DATA_ELEMENT)}-id="${this.key}"]`);
      }
    }
    const { node } = this.editor;

    if (!this.anchor.get()?.isConnected) {
      this.anchor = this.range.commonAncestorNode.find(`[${DATA_ELEMENT}="anchor"]`);
    }
    if (!this.focus.get()?.isConnected) {
      this.focus = this.range.commonAncestorNode.find(`[${DATA_ELEMENT}="focus"]`);
    }
    if (this.anchor.equal(this.focus)) {
      ...
      if (_parent.name === 'p' && _parent.get<Node>()?.childNodes.length === 0) {
        _parent.append($('<br />'));
      }
      return;
    }
  ...
  getNode(
    ...
    if (position === 'left' || position === 'center') {
      ...
      if (!this.key) {
        focus = focus.toArray().find(node => !node.attributes(`data-${selectionNode.attributes(DATA_ELEMENT)}-id`));
      }
      let isRemove = false;
      node.traverse(node => {
        ...
        if (
          isRemove &&
          callback(node) &&
          (node.attributes(DATA_ELEMENT) !== selectionNode.attributes(DATA_ELEMENT) ||
    ...
    if (position === 'right' || position === 'center') {
      ...
      if (!this.key) {
        anchor = anchor.toArray().find(node => !node.attributes(`data-${selectionNode.attributes(DATA_ELEMENT)}-id`));
      }
      let isRemove = false;
      node.traverse(node => {
