import type { EngineInterface, NodeInterface } from '../../types';
...
class Backspace {
  ...
  trigger(event: KeyboardEvent, isDeepMerge?: boolean) {
    ...
    if (range.collapsed) {
      ...
      if (block.name === 'li' && list.isFirst(range)) {
        // 内容已经删除过了
      ...
      if (startBlock.name === 'li' || endBlock.name === 'li') {
        event.preventDefault();
        ...
        if (customeCard?.isCard()) {
          if (list.isEmptyItem(startBlock)) {
            ...
            startBlock.remove();
            if (parent && node.isCustomize(parent) && parent.get<Node>()?.childNodes.length === 0) {
              parent.remove();
            }
          }
        }
        const startParent = startBlock.name === 'li' ? startBlock.parent() : null;
        const endParent = endBlock.name === 'li' ? endBlock.parent() : null;
        if (startBlock.name === 'li' && list.isFirst(range)) {
          cloneRange.setStartBefore(startBlock);
        }
        if (endBlock.name === 'li' && blockApi.isLastOffset(range, 'end')) {
          cloneRange.setEndAfter(endBlock);
        }
        change.delete(cloneRange, isDeepMerge);
        // 光标在列表的最后一行，并且开始光标不在最后一行
        if (startBlock.inEditor() && !startBlock.equal(endBlock) && endBlock.inEditor() && endBlock.name === 'li') {
        ...
        if (startBlock.name === 'li') {
          if (node.isCustomize(startBlock) && startBlock.get<Node>()?.childNodes.length === 0) startBlock.remove();
        }
