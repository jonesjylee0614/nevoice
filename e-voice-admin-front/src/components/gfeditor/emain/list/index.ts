...
import type { EditorInterface, NodeInterface, PluginEntry, RangeInterface } from '../types';
import type { ListInterface, ListModelInterface } from '../types/list';
...
class List implements ListModelInterface {
  ...
  init() {
    ...
    if (isEngine(editor)) {
      ...
      editor.typing.getHandleListener('enter', 'keydown')?.on(event => enter.trigger(event));
      // 删除事件
      this.backspaceEvent = new Backspace(editor);
      editor.typing.getHandleListener('backspace', 'keydown')?.on(event => this.backspaceEvent?.trigger(event));
    }
  }
  ...
  isEmptyItem(node: NodeInterface): boolean {
    ...
    return (
      ...
      (nodeApi.isEmpty(node) ||
        ...
        (children.length === 1
          ? (nodeApi.isCustomize(node) && children[0].isCard()) || children[0].name === 'br'
          : // 子节点有两个，并且是自定义列表而且第一个是卡片，并且第二个节点是br标签
            children.length === 2 &&
            nodeApi.isCustomize(node) &&
            Boolean(children[0].isCard()) &&
            children[1].name === 'br'))
    );
  }
  ...
  isSame(sourceNode: NodeInterface, targetNode: NodeInterface) {
    ...
    if (node.isCustomize(sourceNode) !== node.isCustomize(targetNode)) return false;
    // 判断缩进是否一致
    const sourceIndent = Number.parseInt(sourceNode.attributes(this.INDENT_KEY), 10) || 0;
    const targetIndent = Number.parseInt(targetNode.attributes(this.INDENT_KEY), 10) || 0;
   ...
   */
  isSpecifiedType(blocks: Array<NodeInterface>, name: 'ul' | 'ol' = 'ul', card?: string) {
    const { node } = this.editor;
    let isSame = true;
    blocks.forEach(block => {
      // 如果节点内包含了列表节点，则跳过此节点
      if (!['li', 'p'].includes(block.name) && (block.name === name || block.find(name).length > 0)) return;
      ...
      switch (block.name) {
        case 'li':
          ...
          if (card) {
            ...
            isSame &&= node.isCustomize(block) && (firstChild?.attributes(CARD_KEY) || '') === card;
          } else {
            isSame &&= !node.isCustomize(block);
          }
          break;
        case 'p':
  ...
  getPluginNameByNode(block: NodeInterface) {
    ...
    // 如果是li标签
    if (name === 'li' && block.parent()) return getName(block.parent()!);
    return '';
  }
  ...
  getPluginNameByNodes(blocks: Array<NodeInterface>) {
    ...
    for (let i = 0; i < blocks.length; i++) {
      ...
      switch (block.name) {
        ...
        default:
          // 如果节点内包含了列表节点，则跳过此节点
          if (this.editor.node.isBlock(block) && block.find('ul,ol').length > 0) break;
          else return '';
      }
   ...
   */
  unwrap(blocks: Array<NodeInterface>, normalBlock: NodeInterface = $('<p />')) {
    let indent = 0;
    ...
    blocks.forEach(block => {
      ...
      if (node.isList(block)) {
        indent = Number.parseInt(block.attributes(this.INDENT_KEY), 10) || 0;
        node.unwrap(block);
      }
      ...
      if (block.name === 'li') {
        ...
        if (styles['text-align']) this.addAlign(toBlock, styles['text-align'] as any);
  ...
  split(range?: RangeInterface) {
    ...
    if (blocks.length > 0 && (blocks[0].name === 'li' || blocks[blocks.length - 1].name === 'li')) {
      ...
      if (
        ...
        const newStart =
          (Number.parseInt(beforeListElement.attributes('start'), 10) || 1) + beforeListElement.find('li').length;
  ...
  merge(blocks?: Array<NodeInterface>, range?: RangeInterface) {
    ...
    const selection = blocks ? undefined : cloneRange.shrinkToElementNode().createSelection();
  ...
  addStart(block?: NodeInterface) {
    ...
    const startIndent = Number.parseInt(block.attributes(this.INDENT_KEY), 10) || 0;
    ...
    while (prevNode && node.isList(prevNode)) {
      if (prevNode.name === 'ol') {
        const prevIndent = Number.parseInt(prevNode.attributes(this.INDENT_KEY), 10) || 0;
        const prevStart = Number.parseInt(prevNode.attributes('start'), 10) || 1;
        ...
        cacheIndent = Number.parseInt(prevNode.attributes(this.INDENT_KEY), 10) || 0;
      }
    ...
    while (nextNode) {
      if (node.isList(nextNode)) {
        const nextIndent = Number.parseInt(nextNode.attributes(this.INDENT_KEY), 10) || 0;
        const nextStart = Number.parseInt(nextNode.attributes('start'), 10);
        ...
        if (nextNode.name === 'ol') {
          ...
          } else if (currentStart && currentStart !== nextStart) {
            if (currentStart > 1) nextNode.attributes('start', currentStart);
            else nextNode.removeAttributes('start');
            startCache[nextIndent] = currentStart + _len;
          } else {
  ...
  getIndent(block: NodeInterface) {
    if (this.editor.node.isList(block)) {
      return Number.parseInt(block.attributes(this.INDENT_KEY), 10) || 0;
    }
    return 0;
  }
  /**
   ...
   * @returns
   */
  addAlign(block: NodeInterface, align?: 'left' | 'center' | 'right' | 'justify') {
   ...
   * @param value 卡片值
   */
  addCardToCustomize(node: NodeInterface | Node, cardName: string, value?: any) {
    if (isNode(node)) node = $(node);
    // 必须是li标签
    if (node.name !== 'li') return;
    // 第一个子节点必须不是相同卡片
    const first = node.first();
    if (first?.isBlockCard() || (first?.isCard() && first.attributes(CARD_KEY) === cardName)) return;
    const editor = this.editor;
    // 创建卡片
    const { card } = editor;
   ...
   * @param value 卡片值
   */
  addReadyCardToCustomize(node: NodeInterface | Node, cardName: string, value?: any) {
    if (isNode(node)) node = $(node);
    // 必须是li标签
    if (node.name !== 'li') return;
    // 第一个子节点必须不是卡片
  ...
  insert(fragment: DocumentFragment, range?: RangeInterface) {
    ...
    startElement = startElement?.closest('li', n =>
      node.isBlock(n) && n.nodeName !== 'LI' ? undefined : n.parentElement || undefined
    );
    // 非li不操作
    ...
    const startIsMerge = !node.isList(beginNode) || this.isSame(listElement, beginNode);
    if (startIsMerge) {
      ...
      if (beforeELement?.name === 'li') {
        ...
        if (beginNode.isBlockCard()) {
          ...
          beforeELement?.append(node.isBlock(beginNode) ? beginNode.children() : beginNode);
      ...
      const beforeELement = startLi.parent()?.prev()?.last();
      if (beforeELement && (node.isEmpty(beforeELement) || this.isEmptyItem(beforeELement))) {
        beforeELement.remove();
        if (beforeELement.parent()?.get<Node>()?.childNodes.length === 0) beforeELement.parent()?.remove();
      }
    }
    // 只有一行
    if (
      ...
      if (!beginNode.isBlockCard()) {
        ...
        if (this.isEmptyItem(startLi) && parent.get<Node>()?.childNodes.length === 1) {
          parent.remove();
        }
    ...
    const endIsMerge = (!node.isList(endNode) || this.isSame(listElement, endNode)) && !endNode.isBlockCard();
    ...
    const mergeLists: NodeInterface[] = prevListElement ? [prevListElement] : [];
    // 需要判断最后一个是否交给最后一个节点做合并，如果集合总含有列表就不合并
    for (let i = 0; i < (endIsMerge && !hasList ? fragmentLength - 1 : fragmentLength); i++) {
      ...
      if (node.isList(childElement)) {
        ...
        if (node.isCustomize(startListElment)) {
          ...
          if (firstCard && firstCard.isCard()) {
            const cardName = firstCard.attributes(CARD_KEY) || firstCard.attributes('name');
            const customizeList = this.toCustomize(childElement, cardName);
            if (customizeList) {
              (Array.isArray(customizeList) ? customizeList : [customizeList]).forEach(child => {
          ...
          // 非自定义列表
          const listElements = this.toNormal(childElement, startListElment.name as any);
          (Array.isArray(listElements) ? listElements : [listElements]).forEach(child => {
    ...
    if (!startIsMerge && node.isEmptyWidthChild(startListElment)) startListElment.remove();
    // 后续不需要拼接到最后节点
    if (fragment.childNodes.length === 0) {
      ...
      if (node.isEmpty(startLi) || this.isEmptyItem(startLi)) {
        ...
        if (prevElement && node.isList(prevElement)) prevElement.last()?.append(startLi.children());
        // 把光标位置放到前面的li里面
    ...
    if (lasetELement) {
      ...
      if (endNode.name === 'br') endNode.remove();
        ...
        if (node.isCustomize(lasetELement)) {
          ...
          lasetELement.first()?.after(node.isBlock(endNode) ? endNode.children() : endNode);
        } else {
   ...
   * @returns
   */
  blockToItem(block: NodeInterface, root: NodeInterface, cardName?: string, value?: string) {
    const item = $('<li></li>');
    const { node, schema } = this.editor;
  ...
  toCustomize(
    ...
    const customizeRoot = $(`<${tagName} class="${this.CUSTOMZIE_UL_CLASS}"/>`);
    switch (blocks.name) {
      case 'li':
   ...
   */
  toNormal(blocks: Array<NodeInterface> | NodeInterface, tagName: 'ul' | 'ol' = 'ul', start?: number) {
    const { node } = this.editor;
    if (Array.isArray(blocks)) {
      let nodes: Array<NodeInterface> = [];
      blocks.forEach(block => {
  ...
  isFirst(range: RangeInterface) {
    ...
    // 找到li节点
    const node = startNode.name === 'li' ? startNode : startNode.closest('li');
    // 如果没有li节点
    ...
    // 如果选区中只有一个节点，并且是br标签
    if (contents.childNodes.length === 1 && firstChild.name === 'br') return true;
    // 如果选区中只有一个节点，并且是自定义列表并且第一个是Card
    if (contents.childNodes.length === 1 && node.hasClass(this.CUSTOMZIE_LI_CLASS) && firstChild.isCard()) return true;
    const nodeApi = this.editor.node;
    // 如果选区中只有两个节点，并且是自定义列表并且第一个是Card，最后一个为空节点
    if (
      contents.childNodes.length === 2 &&
  ...
  isLast(range: RangeInterface) {
    ...
    const cloneRange = range.cloneRange();
    // 找到li节点
    const node = endNode.name === 'li' ? endNode : endNode.closest('li');
    // 如果没有li节点
    if (!node[0]) return false;
    // 让选区选择li节点
    cloneRange.select(node, true);
