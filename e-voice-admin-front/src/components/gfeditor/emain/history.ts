...
import type { Operation, TargetOp } from './types/ot';
...
class HistoryModel implements HistoryInterface {
  ...
  // 监听的所有收集本地操作的事件
  private selfEvents: ((ops: Op[]) => Promise<boolean> | boolean | undefined)[] = [];
  // 等待监听收集本地操作的回调
  ...
  handleSelfOps(ops: Op[]) {
    ...
    ops.forEach(op => {
      ...
      if (this.filterEvents.some(filter => filter(op))) {
        if (this.actionOps.length > 0 && !op.nl) this.actionOps[this.actionOps.length - 1].ops?.push(op);
      } else {
    ...
    // 监听收集
    if (isSave) {
      let callback: undefined | Promise<boolean> | boolean;
  ...
  handleNLCardValue(op: TargetOp) {
    ...
    if (op.nl === true && typeof key === 'string' && key === CARD_VALUE_KEY && op.oi) {
      const oiVal = op.oi;
  ...
  handlePath = <T extends Array<any> | ReadonlyArray<any>>(
    ...
    bi: number,
    isOp: boolean = true,
    filter: (node: Node) => boolean = (node: Node) => !isTransientElement($(node))
  ) => {
    ...
    if (targetElement.length > 0 && targetElement.inEditor()) {
      ...
      return (isOp ? newPath.map(p => p + 2) : newPath).concat(path.slice(bi)) as T;
    }
    return path;
  };
    ...
    this.actionOps.forEach(action => {
      ...
      if (action.rangePath) {
        ...
        if (start.id && start.bi !== undefined) {
          start.path = this.handlePath(start.path, start.id, start.bi, false, range.filterPath(true));
        }
        if (end.id && end.bi !== undefined) {
          end.path = this.handlePath(end.path, end.id, end.bi, false, range.filterPath(true));
        }
      }
        ...
        if (affect) {
          ...
          for (let i = index + 1; i < this.actionOps.length; i++) {
            ...
            const nextAffect = nextAction.ops?.some(actionOp => {
              return action.ops?.some(aop => OTJSON.type.canOpAffectPath(actionOp, aop.p));
            });
            if (nextAffect) {
              ...
              if (
                ...
                nextAction2.ops?.some(actionOp => {
                  return nextAction.ops?.some(aop => OTJSON.type.canOpAffectPath(actionOp, aop.p));
                })
              ) {
          ...
          const newActionOps = this.actionOps.filter(aop => removeArray.find(raop => raop.id === aop.id) === undefined);
  ...
  getCurrentRangePath() {
    ...
    return currentPath || change.range.get().toPath();
  }
