...
import type { EngineInterface, NodeInterface, Selector } from '../types';
import { $ } from '../node';
...
class Container {
  ...
  _init() {
    ...
    if (className !== undefined) {
      (Array.isArray(className) ? className : className.split(/\s+/)).forEach(name => {
  ...
  handleClick = (event: MouseEvent) => {
    ...
    if (event.target && !engine.readonly && isEngine(engine)) {
      ...
      const node = $('<p><br /></p>');
      const container = targetNode.closest(`${EDITABLE_SELECTOR},${ROOT_SELECTOR}`);
      // 获取编辑器内第一个子节点
      ...
      // 存在这个节点，并且鼠标单击位置要大于最后一个节点，并且这个节点不是一个空的节点
        ...
        lastBlock &&
        event.offsetY >
          (lastBlock.get<HTMLElement>()?.offsetTop || 0) + (lastBlock.get<Element>()?.clientHeight || 0) &&
  ...
  docMouseDown = (e: MouseEvent) => {
    ...
    const { engine } = this.options;
    if (this._focused && targetNode.closest(UI_SELECTOR).length === 0 && !targetNode.inEditor(engine.container)) {
