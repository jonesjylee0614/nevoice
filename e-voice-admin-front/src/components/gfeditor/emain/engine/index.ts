...
import OT from '../ot';
import type { NodeInterface, Selector } from '../types/node';
import type { ChangeInterface } from '../types/change';
import type { ContainerInterface, EngineInterface, EngineOptions } from '../types/engine';
import type { HistoryInterface } from '../types/history';
import type { OTInterface } from '../types/ot';
import type { HotkeyInterface } from '../types/hotkey';
import type { CardInterface } from '../types/card';
import History from '../history';
import Hotkey from '../hotkey';
import { getDocument } from '../utils';
import { ANCHOR, CURSOR, FOCUS } from '../constants/selection';
import { toDOM, toJSON0 } from '../ot/utils';
import Parser from '../parser';
import type { TypingInterface } from '../types';
import Typing from '../typing';
import Selection from '../selection';
import Editor from '../editor';
import { $ } from '../node';
import { DATA_CONTENTEDITABLE_KEY } from '../constants';
import Container, { DATA_PLACEHOLDER, DATA_PLACEHOLDER_CLASS } from './container';
import './index.css';

class Engine<T extends EngineOptions = EngineOptions> extends Editor<T> implements EngineInterface<T> {
  ...
  constructor(selector: Selector, options?: EngineOptions) {
    ...
    // 编辑器父节点
    this.root = $(this.options.root || this.container.parent() || getDocument().body);
    ...
    // 只读
    this._readonly = this.options.readonly === undefined ? false : this.options.readonly;
      ...
      if (this.node.isRootBlock(node)) {
        ...
        if (childrenLength === 0) {
          ...
          if (
            childrenLength === 1 &&
            child?.name === 'span' &&
            [CURSOR, ANCHOR, FOCUS].includes(child.attributes(DATA_ELEMENT))
          ) {
            node.prepend($('<br />'));
          }
        }
