import type { File, FileInfo, UploaderInterface, UploaderOptions } from '../../types/request';
...
class Uploader implements UploaderInterface {
  ...
  private async upload(files: Array<File>, name: string = 'file') {
    files.forEach(async file => {
      ...
      const { url, onUploading, onSuccess, onError, withCredentials, crossOrigin, headers } = this.options;
          ...
          xhr.upload.addEventListener(
            ...
            evt => {
              if (evt.lengthComputable) {
                if (onUploading)
                  onUploading(file, {
                    percent: Number.parseInt(((evt.loaded / evt.total) * 100).toString(), 10)
                  });
              }
            },
            false
    ...
    return new Promise<boolean>((resolve, reject) => {
      ...
      fileReader.addEventListener(
        ...
        () => {
          ...
          // 全部文件读取完成后再插入编辑器
          if (files.every(file => Boolean(this.uploadingFiles[file.uid!]))) {
            Promise.all([
              ...files.map(file => {
                return new Promise(async resolve => {
                  if (this.options.onReady) {
                    await this.options.onReady(this.uploadingFiles[file.uid!], file);
                  }
                  resolve(true);
                });
              })
