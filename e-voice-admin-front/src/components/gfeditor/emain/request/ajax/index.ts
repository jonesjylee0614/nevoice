...
import type { AjaxInterface, AjaxOptions, RequestHeaders } from '../../types/request';
...
class Ajax implements AjaxInterface {
  ...
  defaultXHR() {
    ...
    // is it x-domain
    if (crossOrigin === true) {
      const xhrInstance = context[XML_HTTP_REQUEST] ? new context[XML_HTTP_REQUEST]() : null;
  ...
  setHeaders(request: XMLHttpRequest, headers: Record<string, string>) {
    headers.Accept = headers.Accept || (globalSetup.accept[this.options.type || '*'] as any);
  ...
  setCredentials(request: XMLHttpRequest) {
    if (typeof this.options.withCredentials !== 'undefined' && typeof request.withCredentials !== 'undefined') {
      request.withCredentials = Boolean(this.options.withCredentials);
    }
  }
  ...
  handleJsonp(
    ...
    let loaded = 0;
    const isIE10 = navigator.userAgent.includes('MSIE 10.0');
  ...
  async getRequest(
    ...
    const { processData, traditional, type, context, xhr, async, before } = this.options;
    ...
    if ((this.options.contentType?.indexOf('json') || -1) > -1 && typeof data === 'object' && !isFormData(data)) {
      data = JSON.stringify(data);
    }

    data =
      processData !== false && data && typeof data !== 'string' && !isFormData(data)
    ...
    const http = (typeof xhr === 'function' ? xhr(this.options) : xhr) || this.defaultXHR();
    if (!http) return;
    http.open(method, url, async !== false);
  ...
  triggerSuccess(request: XMLHttpRequest) {
    ...
    const data = (dataFilter || globalSetup.dataFilter)(request.responseText, type);
    if (!type) {
