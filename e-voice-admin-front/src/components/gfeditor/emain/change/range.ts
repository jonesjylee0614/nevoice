import type { ChangeRangeInterface, EngineInterface, RangeInterface } from '../types';
import { $ } from '../node';
...
class ChangeRange implements ChangeRangeInterface {
  ...
  toTrusty(range: RangeInterface = this.get()) {
    ...
    if (!range.startNode.equal(rangeClone.startNode) || range.startOffset !== rangeClone.startOffset)
      range.setStart(rangeClone.startContainer, rangeClone.startOffset);

    rangeClone = range.cloneRange();
    rangeClone.collapse(false);
    this.setCardRang(rangeClone);
    if (!range.endNode.equal(rangeClone.endNode) || range.endOffset !== rangeClone.endOffset)
  ...
  select(range: RangeInterface, triggerSelect: boolean = true) {
    ...
    if (range.collapsed) {
      ...
      if (
        ((startNode.isElement() && startOffset === 1 && startNode.get<Node>()?.childNodes.length === 1) ||
          (startOffset === 2 && startNode.get<Node>()?.childNodes.length === 2 && startNode.first()?.isCard())) &&
        startNode.last()?.name === 'br'
      ) {
      ...
      if (startNode.isText()) {
        ...
        if (parent?.attributes(CARD_ELEMENT_KEY) === 'right' && startOffset === 0) {
          range.setStart(startNode, 1);
          range.collapse(true);
        } else if (parent?.attributes(CARD_ELEMENT_KEY) === 'left' && startOffset === 1) {
    ...
    // 修复inline光标
    let { startNode, endNode, startOffset, endOffset } = range.cloneRange().shrinkToTextNode();
    ...
    if (!inlineNode.isCard() && node.isInline(inlineNode) && !node.isVoid(inlineNode)) {
      // 左侧
      if (startNode.isText() && !startNode.prev() && startNode.parent()?.equal(inlineNode) && startOffset === 0) {
        const text = startNode.text();
        if (/^\u200B/g.test(text)) {
          range.setStart(startNode, startOffset + 1);
          if (range.collapsed) range.collapse(true);
      ...
      // 右侧
      if (endNode.isText() && !endNode.next() && endNode.parent()?.equal(inlineNode)) {
        const text = endNode.text();
    ...
    if (!range.collapsed && !otStopped && endNode.name === 'p' && endNode.get<Node>()?.childNodes.length === 0) {
      endNode.append('<br />');
    }
    ...
    if (
      selection &&
      (range.collapsed || (selection.rangeCount > 0 && !range.equal(selection.getRangeAt(0)))) &&
  ...
  blur() {
    ...
    range.commonAncestorNode.closest(EDITABLE_SELECTOR).get<HTMLElement>()?.blur();
