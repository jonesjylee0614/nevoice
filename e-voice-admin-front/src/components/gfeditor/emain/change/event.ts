...
import type { EventListener, NodeInterface } from '../types/node';
import type { ChangeEventInterface, ChangeEventOptions } from '../types/change';
...
class ChangeEvent implements ChangeEventInterface {
  ...
  onInput(callback: EventListener) {
    ...
    this.onContainer('compositionstart', event => {
      ...
      const range = change.range.get().cloneRange().shrinkToTextNode().enlargeToElementNode();
      ...
      if (!this.isComposing) {
        ...
        if (isAndroid && androidCustomeListComposingNode) {
          ...
          if (range.collapsed && startNode?.isText()) {
            ...
            if (/^\u200B/.test(sufix)) {
              startNode.text(text.substring(0, startOffset) + sufix.substring(1));
    ...
    this.onContainer('beforeinput', (event: InputEvent) => {
      ...
      const range = sourceRange.cloneRange().shrinkToTextNode().enlargeToElementNode();
      ...
      if (!range.collapsed) {
        if (range.commonAncestorNode.attributes(CARD_ELEMENT_KEY) === 'body') card.remove(range.commonAncestorNode);
        else {
      ...
      if (range.startNode.isRoot()) {
        ...
        if (startNode instanceof Element && isCard(startNode) && !startNode.querySelector(CARD_CENTER_SELECTOR)) {
  ...
  onSelect(callback: EventListener) {
    ...
    this.onContainer(isMobile ? 'touchstart' : 'mousedown', (event: MouseEvent | TouchEvent) => {
      if (this.isCardInput(event)) {
        return;
      }
    ...
    this.onContainer('keyup', e => {
      ...
      if (
        ...
        const range = Range.from(this.engine);
        if (this.keydownRange && range && range.equal(this.keydownRange)) return;
        if (!this.isComposing) {
          callback(e);
        }
      }
  ...
  onDrop(
    callback: (params: { event: DragEvent; range?: RangeInterface; card?: CardInterface; files: Array<File> }) => void
  ) {
    ...
    const dragStart = (e: DragEvent) => {
      ...
      cardComponet = this.engine.card.find(dragCardTrigger || targetNode);
  ...
  addEvent(type: GlobalEventType, eventType: string, listener: EventListener, index?: number) {
    if (!this.globalEvents[type]) {
      this.globalEvents[type] = [];
    }
    if (!this.globalEvents[type].find(event => event.type === eventType)) {
      const globalListener = (...args: any) => {
        const listeners = this.events[type].filter(event => event.type === eventType);
        let result;
