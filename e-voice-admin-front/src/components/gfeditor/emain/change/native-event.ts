...
import type { CardEntry, EngineInterface, NodeInterface, RangeInterface } from '../types';
...
class NativeEvent {
  ...
  repairInput(event: InputEvent, range: RangeInterface) {
    ...
    if (startNode.isText() && parent && node.isMark(parent)) {
      ...
      // 输入光标在mark节点末尾
      if (startOffset === text.length && event.data && event.inputType.indexOf('insert') === 0) {
        ...
        if (markTops.length > 0) {
          ...
          node.insertText(text.substring(text.length - event.data.length));
          mark.merge();
      ...
      // 输入光标在mark节点开始位置
      else if (event.data && startOffset === event.data.length && event.inputType.indexOf('insert') === 0) {
        let markParent: NodeInterface | undefined = parent;
        const markTops: Array<NodeInterface> = [];
        // 循环查找
        while (markParent && node.isMark(markParent)) {
      ...
      // 输入时删除mark标签内零宽字符。
      if (text.length > 0 && /^\u200B$/g.test(text.substring(0, 1))) {
    ...
    if (selection && selection.anchorNode) {
      ...
      let containsCard =
        ...
        (range.commonAncestorNode.closest(CARD_SELECTOR).length > 0 &&
          range.startNode.closest(`${CARD_LEFT_SELECTOR},${CARD_RIGHT_SELECTOR},${UI_SELECTOR}`).length === 0);

      let isSingle = range.collapsed;
      if (!isSingle) {
        const { startNode, endNode, startOffset, endOffset } = range;
        const startElement =
          startNode.isElement() && !startNode.isCard() ? startNode.children().eq(startOffset) : startNode;
        const endElement = endNode.isElement() && !endNode.isCard() ? endNode.children().eq(endOffset - 1) : endNode;
      ...
      card.each(card => {
        ...
        if (center && center.length > 0) {
          let isSelect = selection.containsNode ? selection.containsNode(center[0]) : false;
  ...
  init() {
    ...
    change.event.onDocument('mousedown', (e: MouseEvent) => {
      ...
      while (node) {
        ...
        if (attrValue && ![ROOT, EDITABLE].includes(attrValue)) {
          return;
        }
        node = node.parent();
      }
    ...
    change.event.onDocument('cut', event => {
      const range = change.range.get();
      if (!this.engine.container.contains(range.commonAncestorNode) || this.engine.readonly) return;
    ...
    const pasteMarkdown = async (text: string) => {
      ...
      // 提示是否要转换
      this.engine
        .messageConfirm('markdown', this.engine.language.get<string>('checkMarkdown', 'title'))
        .then(() => {
          handlePaste();
        })
        .catch(err => {
    ...
    change.event.onPaste(data => {
      ...
      if (files.length === 0) {
        ...
        if (isPasteText) {
          ...
          // 富文本粘贴
          if (html && html.includes('<meta name="source" content="aomao" />')) {
            source = html;
          } else if (html) {
            source = html;
          } else if (text) {
  ...
  paste(
    ...
    const fragment = new Paste(source, this.engine).normalize(forceGenerateAllId);
    ...
    if (insert) insert(fragment, range, undefined, followActiveMark);
      ...
      change.insert(
        ...
        range => {
          ...
          let cardId = '';
          if (endNode.isCard() && endNode.get<Node>()?.childNodes.length === 0) {
          ...
          range.collapse(false);
          // 卡片会出现未渲染的情况，选中在卡片后面
          const card = range.startNode.closest(`${CARD_SELECTOR},${READY_CARD_SELECTOR}`);
          if (card.length > 0) {
          ...
          this.engine.card.render(undefined, count => {
            selection.move();
            if (cardId) {
              const newCard = this.engine.container.find(`[data-id="${cardId}"]`);
              if (newCard.length > 0) {
