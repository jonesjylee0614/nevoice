...
import { CARD_KEY, READY_CARD_KEY, READY_CARD_SELECTOR } from '../constants/card';
...
export default class Paste {
  ...
  getDefaultStyle(container = this.engine.container) {
    const defaultStyle = {
      ...
      'background-color': tinycolor2(container.css('background-color')).toHexString(),
      ...
      node => {
        ...
        if (node.isText()) {
          ...
          if (/\u200B/.test(text)) {
            ...
            else if (parent && nodeApi.isInline(parent, this.schema)) isRemove = false;
            else if (next && nodeApi.isInline(next, this.schema)) isRemove = false;
            else if (prev && nodeApi.isInline(prev, this.schema)) isRemove = false;
            if (isRemove) {
              text = text.replace(/\u200B/g, '');
              node.text(text);
            }
          ...
          if (nodeApi.isList(parent)) {
            const next = node.next();
            const prev = node.prev();
            const addCardToCustomize = (node: NodeInterface, target: NodeInterface) => {
              if (nodeApi.isCustomize(node)) {
                const first = node.first();
                if (first && first.isCard()) {
                  const cardName = first.attributes(CARD_KEY) || first.attributes(READY_CARD_KEY);
        ...
        const nodeIsBlock = type ? type === 'block' : nodeApi.isBlock(node, this.schema);
        ...
        if (nodeApi.isList(node) && parent?.name === 'li') {
          ...
          children.each((child, index) => {
            ...
            if (!li) {
              li = isCustomizeList ? $(`<li class="${list.CUSTOMZIE_LI_CLASS}" />`) : $('<li />');
          ...
          return (next as NodeInterface | null)?.next() || leftList.next() || undefined;
        }
        // p 改成 li
        ...
        if (!nodeIsBlock && nodeApi.isInline(node) && !node.isCard() && !nodeIsVoid) {
          const isVoid = node.allChildren().some(node => nodeApi.isVoid(node, this.schema));
        ...
        if (node.length > 0 && nodeIsMark) {
          ...
          if (!block.equal(node)) {
            ...
            if (markPlugin && blockPlugin?.disableMark?.includes(markPlugin.name)) {
        ...
        if (nodeIsMark) {
          ...
          if (markPlugin) {
            const plugin = topMarkPlugins.find(
              item => item.plugin?.name === markPlugin.name && item.node.length > 0 && !item.node.equal(node)
            );
            if (plugin) {
        ...
        // mark 按级别排序
        nodeParent = parent;
        if (node.length > 0 && nodeParent && nodeApi.isMark(nodeParent, this.schema) && nodeIsMark) {
          const pMarkPlugin = markApi.findPlugin(nodeParent);
          const cMarkPlugin = currentMarkPlugins[currentMarkPlugins.length - 1]?.plugin;
  ...
  normalize(forceGenerateAllId = true) {
    ...
    if (root.inEditor() && root.isText() && range.startContainer === range.endContainer) {
      ...
      const leftText = text?.substring(0, range.startOffset);
      const rightText = text?.substring(range.endOffset);
      // 光标在 [text](|) 里
    ...
    $(fragment).traverse(node => {
      ...
      if (node.length > 0 && node[0].parentNode) this.engine.trigger('paste:each', node);
      // 删除非block节点的换行 \r\n\r\n<span
      if (node.isText()) {
