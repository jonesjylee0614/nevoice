...
import type { ChangeInterface, ChangeOptions, ChangeRangeInterface } from '../types/change';
import type { EngineInterface } from '../types/engine';
import type { RangeInterface, RangePath } from '../types/range';
...
import type { SelectionInterface } from '../types/selection';
...
class ChangeModel implements ChangeInterface {
  ...
  constructor(engine: EngineInterface, options: ChangeOptions = {}) {
    ...
    this.onRealtimeChange = this.options.onRealtimeChange || (() => {});
    ...
    this.onSetValue = this.options.onSetValue || (() => {});
  ...
  private _change() {
    if (!this.isComposing()) {
      this.engine.card.gc();
      const trigger =
        this.changeTrigger.length === 2 ? 'both' : this.changeTrigger[0] === 'remote' ? 'remote' : 'local';
  ...
  change(isRemote?: boolean, applyNodes?: Array<NodeInterface>) {
    ...
    let editableElement: NodeInterface | undefined;
    if (isRemote) {
      ...
      if (startNode.inEditor()) {
        ...
        if (cardId) {
          editableElement = this.engine.card.find(cardId)?.root.closest(EDITABLE_SELECTOR);
        }
      }
      if (editableElement && editableElement.length > 0) {
        const card = this.engine.card.find(editableElement, true);
    ...
    if (!this.changeTrigger.includes(trigger)) this.changeTrigger.push(trigger);
  ...
  initValue(range?: RangeInterface, apply: boolean = true, container = this.engine.container) {
    const html = container.html();
    const defaultHtml = '<p><br /></p>';
    if (html === defaultHtml || (container.get<Node>()?.childNodes.length || 0) > 0) return;
  ...
  setValue(value: string, onParse?: (node: NodeInterface) => void, callback?: (count: number) => void) {
    const range = this.range.get();
  ...
  getOriginValue(container: NodeInterface = this.engine.container) {
    ...
    return new Parser(container.clone(true), this.engine, undefined, false).toValue(schema, conversion);
  }
  ...
  apply(range?: RangeInterface) {
    ...
    if (range) {
      ...
      inline.findInlines(range).forEach(inlineNode => inline.repairCursor(inlineNode));
  ...
  insert(
    ...
    if (!isCollapsed) {
      ...
      if (range.startNode.isText()) {
        ...
        if (inlineNode.length === 0 && !inlineNode.equal(range.startNode) && /^\u200B/.test(text))
          range.startNode.text(text.substring(1));
      }
    }
    let startRange: { node: NodeInterface; offset: number } | undefined;
    const apply = (rg: RangeInterface) => {
      if (startRange && startRange.node[0].isConnected) {
        rg.shrinkToElementNode().setStart(startRange.node, startRange.offset);
        rg.enlargeToElementNode();
      }
      block.merge(rg);
      list.merge(undefined, rg);
      mark.merge(rg);
      inline.flat(rg);
      if (callback) callback(rg);
      this.apply(rg);
    };
    ...
    const cloneRange = range.cloneRange().enlargeToElementNode(true).collapse(false);
    const startNode = cloneRange.startContainer.childNodes[range.startOffset === 0 ? 0 : range.startOffset - 1];
    ...
    if (childNodes.length !== 0) {
      ...
      if (mergeNode[0]) {
        appendNodes.forEach(element => {
          if (!mergeTags.includes(element.name) && element.closest(mergeNode.name).length === 0) {
    ...
    const getFirstChild = (node: NodeInterface) => {
      ...
      while (allowInTags.includes(child ? child.name : '')) {
        child = child!.first();
      }
      return child;
    };
    ...
    const isSameListChild = (_lastNode: NodeInterface, _firstNode: NodeInterface) => {
      if (_lastNode.isCard() || firstNode.isCard()) return;
      const fParent = _firstNode.parent();
      const lParent = _lastNode.parent();
      const isSameParent =
        fParent && !fParent.isEditable() && lParent && !lParent.isEditable() && fParent.name === lParent.name;
      return (
        (_firstNode.name === 'p' && isSameParent) ||
        (_lastNode.name === _firstNode.name &&
          isSameParent &&
          !(_lastNode.name === 'li' && !list.isSame(_lastNode.parent()!, _firstNode.parent()!)))
      );
    };
    ...
    const clearList = (lastNode: NodeInterface, nextNode: NodeInterface) => {
      if (lastNode.name === nextNode.name && lastNode.name === 'p') {
  ...
  paste(html: string, range?: RangeInterface, callback?: (count: number) => void) {
   ...
   * @param followActiveMark 删除后空标签是否跟随当前激活的mark样式
   */
  delete(range?: RangeInterface, isDeepMerge?: boolean, followActiveMark: boolean = true) {
    ...
    let block = blockApi.closest(
      safeRange.cloneRange().shrinkToElementNode().shrinkToTextNode().enlargeToElementNode().startNode
    );
    // 获取的 block 超出编辑范围
    ...
    if (block.isRoot()) {
      ...
      while (child?.isCard()) {
        const isBreak = child.equal(safeRange.endNode) || child.contains(safeRange.endNode);
    ...
    const isMoreLine = !blockApi.closest(safeRange.startNode).equal(blockApi.closest(endNode));
    ...
    const endContentNode = safeRange.cloneRange().shrinkToElementNode().shrinkToTextNode().getEndOffsetNode();
    // 先删除范围内的所有内容
    safeRange.extractContents();

    let { startNode } = safeRange;
    if (startNode.isEditable() && startNode.get<Node>()?.childNodes.length === 0) {
    ...
    if (block.isElement() && !block.equal(startNode) && block.get<Element>()!.childNodes.length === 0) {
      block.remove();
    }
    // 只删除了文本，不做处理
    ...
    if (isMoreLine && startNode.get<Node>()?.childNodes.length === 0) {
      ...
      startNode.remove();
      if (selection.anchor?.get<Node>()?.isConnected && selection.focus?.get<Node>()?.isConnected) {
        selection.move();
      }
    ...
    const nextNode = endContentNode && endContentNode.isConnected ? startNode : null;
    ...
    const deepMergeNode = (
      ...
      if (nodeApi.isBlock(prevNode) && !nodeApi.isVoid(prevNode) && !prevNode.isCard()) {
        ...
        range.collapse(false);
        const selection = range.shrinkToElementNode().shrinkToTextNode().createSelection();
    ...
    startNode.children().each(node => {
      const domNode = $(node);
      if (!nodeApi.isVoid(domNode) && domNode.isElement() && nodeApi.html(domNode) === '') domNode.remove();
    ...
    if (nodeApi.isBlock(startNode) && startNode.get<Node>()?.childNodes.length === 0) {
      startNode.html('<br />');
    }

    if (isRemoveStartNode) {
      if (nodeApi.isBlock(prevNode) && prevNode.get<Node>()?.childNodes.length === 0) {
        prevNode.html('<br />');
      }
  ...
  unwrap(node?: NodeInterface) {
    ...
    const range = this.range.get();
    node ||= block.closest(range.startNode);
    if (!node.inEditor()) {
      return;
    }
  ...
  mergeAfterDelete(node?: NodeInterface) {
    ...
    // 前面是Card
    if (prevBlock.isCard()) {
      if ((children.length === 1 && first?.name === 'br') || nodeApi.isEmpty(node)) {
        node.remove();
      }
    ...
    if (children.length === 1 && first?.name === 'br') {
      first?.remove();
    } else if (prevBlock && prevBlock.get<Node>()?.childNodes.length === 1 && prevBlock.first()?.name === 'br') {
