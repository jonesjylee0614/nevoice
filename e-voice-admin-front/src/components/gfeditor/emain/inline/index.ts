import { CARD_EDITABLE_KEY, CARD_ELEMENT_KEY, CARD_KEY, CARD_SELECTOR, CARD_TYPE_KEY } from '../constants';
import type { EditorInterface, EngineInterface, InlineModelInterface, NodeInterface, RangeInterface } from '../types';
...
class Inline implements InlineModelInterface {
  ...
  /** 修复光标选区位置，<a><anchor />acde<focus /></a> -> <anchor /><a>acde</a><focus />否则在ot中，可能无法正确的应用inline节点两边的更改 */
    ...
    commonAncestorNode.traverse(child => {
      if (!child.equal(selection.anchor!)) {
        if (started) {
          ...
          if (inlineClone[0].childNodes.length !== 0 && Boolean(inlineClone.parent())) {
  ...
  unwrap(range?: RangeInterface | NodeInterface) {
    ...
    const safeRange = !range || !isRangeInterface(range) ? change.range.toTrusty() : range;
    this.repairRange(safeRange);
    mark.split(safeRange);
    const inlineNodes = range && !isRangeInterface(range) ? [range] : this.findInlines(safeRange);
    // 清除 Inline
    ...
    inlineNodes.forEach(node => {
      ...
      if (prev && prev.isText() && /\u200B$/g.test(prevText)) {
        ...
        else prev.text(prevText.substring(0, prevText.length - 1));
      }
      if (next && next.isText() && /^\u200B/g.test(nextText)) {
        if (/^\u200B$/g.test(nextText)) next.remove();
        else next.text(nextText.substring(1));
      }
   ...
   * @param callback 回调
   */
  unwrapEmptyInlines(root: NodeInterface, callback?: (node: NodeInterface) => boolean) {
  ...
  splitOnExpanded(range: RangeInterface) {
    ...
    if (
      ...
      const startParent = startNode.parent();
      const startIsInline = node.isInline(startNode) || (startParent && node.isInline(startParent));
      const endParent = endNode.parent();
      const endIsInline = node.isInline(endNode) || (endParent && node.isInline(endParent));
  ...
  findInlines(range: RangeInterface) {
    ...
    const handleRange = (allowBlock: boolean, range: RangeInterface, toStart: boolean = false) => {
      ...
      if (toStart) {
        ...
        const startOffsetNode = startChildren.eq(startOffset) || startChildren.eq(startOffset - 1);
        if (
          !allowBlock &&
          ...
          (startNode.type !== Node.ELEMENT_NODE ||
            (Boolean(startOffsetNode) && !nodeApi.isBlock(startOffsetNode as any)))
        )
          return;
        cloneRange.select(startParent, true);
        ...
        const startChildrenOffsetNode = startChildren.eq(startOffset) || startChildren.eq(startOffset - 1);
        if (
          !allowBlock &&
          ...
          (startNode.type !== Node.ELEMENT_NODE ||
            (startChildrenOffsetNode && !nodeApi.isBlock(startChildrenOffsetNode)))
        )
          return;
        cloneRange.select(startParent, true);
  ...
  repairBoth(node: NodeInterface | Node) {
    ...
    if (node.get()?.parentNode && !nodeApi.isVoid(node)) {
      ...
      if (
        ...
        !prev.isText() ||
        !/\u200B$/g.test(prevText) ||
        (prevPrev && nodeApi.isInline(prevPrev) && !/\u200B.*\u200B$/g.test(prevText))
      ) {
        if (prev && prev.isText()) {
          prev.text(`${prevText}\u200B`);
        } else {
        ...
        prev.text(prevText.substring(0, prevText.length - 1));
      }
      ...
      if (
        ...
        !next.isText() ||
        !/^\u200B/g.test(nextText) ||
        (nextNext && nodeApi.isInline(nextNext) && !/^\u200B\u200B/g.test(nextText))
      ) {
        if (next && next.isText()) {
          next.text(`\u200B${next.text()}`);
        } else {
        ...
        next.text(nextText.substring(0, nextText.length - 1));
      }
    }
  }
  ...
  flat(node: NodeInterface | RangeInterface, schema?: SchemaInterface) {
    ...
    if (isRangeInterface(node)) {
      const selection = node.cloneRange().shrinkToElementNode().createSelection();
      ...
      applyInlines.forEach(inline => {
        ...
        if (prev && nodeApi.isMark(prev) && prev.get<Element>()!.childNodes.length === 0) {
          prev.remove();
        }
        if (next && nodeApi.isMark(next) && next.get<Element>()!.childNodes.length === 0) {
          next.remove();
        }
      });
      return;
    ...
    if (nodeApi.isInline(node, schema) && node.name !== 'br') {
      ...
      // 不允许放入mark
        ...
        while (parentMark && !parentMark.equal(node) && nodeApi.isMark(parentMark, schema)) {
          ...
          parentMark.children().each((child: any) => {
            // 零宽字符的文本跳过
