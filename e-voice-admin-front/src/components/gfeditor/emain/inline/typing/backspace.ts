...
class Backspace {
  ...
  trigger(event: KeyboardEvent) {
    ...
    const { collapsed, endNode, startNode, startOffset } = range.cloneRange().shrinkToTextNode();
    if (endNode.type === Node.TEXT_NODE || startNode.type === Node.TEXT_NODE) {
      ...
      if (!collapsed) {
        ...
        if (node.isInline(inlineNode) && !inlineNode.isCard() && node.isEmpty(inlineNode)) {
      ...
      // 在inline节点外
        ...
        let prev = startNode.prev();
        let parent = startNode.parent();
        let inlineNode: NodeInterface | undefined;
