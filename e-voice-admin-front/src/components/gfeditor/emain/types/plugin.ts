...
import type { SchemaGlobal, SchemaRule, SchemaValue } from './schema';
import type { BlockInterface, InlineInterface, MarkInterface } from '.';
...
export interface PluginEntry {
  ...
  new (editor: EditorInterface, options: PluginOptions): PluginInterface<PluginOptions>;
...
export interface PluginInterface<T extends PluginOptions = PluginOptions> {
  ...
  hotkey?(
    event?: KeyboardEvent
  ): string | { key: string; args: any } | Array<{ key: string; args: any }> | Array<string>;
  /**
   * 插件是否在等待处理中
   *
...
export interface ElementPluginInterface<T extends PluginOptions = PluginOptions> extends PluginInterface<T> {
  /** 标签名称 */
