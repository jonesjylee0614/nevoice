...
import type { SchemaBlock } from './schema';
/** block 插件管理器 */
export interface BlockModelInterface {
  ...
  /**
   * 生成 cursor 左侧或右侧的节点，放在一个和父节点一样的容器里 isLeft = true：左侧 isLeft = false：右侧
   *
   * @param param0
   */
  getBlockByRange({
...
/** block 插件 */
export interface BlockInterface<T extends PluginOptions = PluginOptions> extends ElementPluginInterface<T> {
