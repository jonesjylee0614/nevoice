...
import type { Doc, Op, Path, ShareDBSourceOptions } from 'sharedb';
import type { Callback, Type } from 'sharedb/lib/sharedb';
import type { CardInterface } from './card';
import type { NodeInterface } from './node';
import type { RangeInterface, RangePath } from './range';
import type { DrawStyle, TinyCanvasInterface } from './tiny-canvas';
...
export interface DocInterface<T = any> extends EventEmitter2 {
  ...
  // 提交操作到協同作業
  submitOp(ops: Op[], options?: ShareDBSourceOptions, callback?: Callback): void;
  // 注销
  destroy(): void;
}
...
export interface RangeColoringInterface {
   ...
   * @param options
   */
  drawBackground(range: RangeInterface, options: { uuid: string; color: string }): Array<RangeInterface>;
  /**
   * 获取节点 rect
   *
   * @param node
   ...
   * @param member
   */
  drawCursor(selector: RangeInterface | NodeInterface, member: Member, showInfo?: boolean): NodeInterface | undefined;
  /**
   * 绘制卡片光标
   *
   * @param node
...
export interface ConsumerInterface {
   ...
   * @param value 属性值
   */
  setAttribute(root: NodeInterface, path: Path, attr: string, value: string): void;
  /**
   * 删除属性操作
   *
   * @param root 根节点
   ...
   * @param value 操作值
   */
  insertNode(root: NodeInterface, path: Path, value: string | Op[] | Op[][]): void;
  /**
   * 删除一个节点
   *
   * @param root 根节点
   ...
   * @param text 插入的文本
   */
  insertText(root: NodeInterface, path: Path, offset: number, text: string): void;
  /**
   * 删除文本
   *
   * @param root 根节点
   ...
   * @param text 删除的文本
   */
  deleteText(root: NodeInterface, path: Path, offset: number, text: string): void;
  /**
   * 处理操作
   *
   * @param op
...
export interface OTInterface extends EventEmitter2 {
   ...
   */
  initRemote(doc: Doc, defaultValue?: string, onSelectionChange?: (path: Attribute) => void): void;
  /**
   * 处理操作改变
   *
   * @param ops 操作集合
