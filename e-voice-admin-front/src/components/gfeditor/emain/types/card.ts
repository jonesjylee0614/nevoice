import type { CardActiveTrigger, CardType, SelectStyleType } from '../card/enum';
import type { EditorInterface } from './editor';
import type { NodeInterface } from './node';
import type { TinyCanvasInterface } from './tiny-canvas';
import type { RangeInterface } from './range';
import type { DropdownButtonOptions, DropdownSwitchOptions, ToolbarItemOptions } from './toolbar';
...
export interface CardModelInterface {
   ...
   * @param value 卡片值
   */
  replaceNode<V extends CardValue>(node: NodeInterface, name: string, value?: Partial<V>): NodeInterface;
  /**
   * 更新卡片重新渲染
   *
   * @param card 卡片
   ...
   */
  updateNode<E extends CardValue = {}, T extends CardInterface<E> = CardInterface<E>>(
    card: T,
    value: Partial<E>,
    ...args: any
  ): void;
