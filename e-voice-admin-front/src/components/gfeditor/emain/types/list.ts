...
import type { BlockInterface } from './block';
...
/** 列表接口 */
export interface ListInterface<T extends PluginOptions = PluginOptions> extends BlockInterface<T> {
  /** 自定义列表卡片名称 */
  cardName?: string;
  /**
...
export interface ListModelInterface {
   ...
   * @param value 卡片值
   */
  addCardToCustomize(node: NodeInterface | Node, cardName: string, value?: any): CardInterface | undefined;
  /**
   * 为自定义列表项添加待渲染卡片节点
   *
   * @param node 列表节点项
