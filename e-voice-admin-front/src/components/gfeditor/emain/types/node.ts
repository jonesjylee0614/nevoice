...
import type { SchemaInterface, SchemaRule } from './schema';
...
export interface EventInterface {
   ...
   * @param listener
   */
  off(eventType: string, listener: EventListener, options?: boolean | EventListenerOptions): void;
  /**
   * 触发事件
   *
...
export type Selector = string | HTMLElement | Node | Array<Node> | NodeList | NodeInterface | EventTarget;
...
export interface NodeInterface {
   ...
   *
   * @param {Number} index
   * @returns {NodeInterface | undefined} NodeInterface 类，或 undefined
   */
   ...
   * @returns {NodeInterface} 符合条件的子节点
   */
   ...
   * 获取当前节点第一个子节点
   *
   * @returns {NodeInterface} NodeInterface 子节点
   */
   ...
   *
   * @returns {NodeInterface} NodeInterface 节点
   */
   ...
   * @returns {NodeInterface} 返回一个 NodeInterface 实例
   */
   ...
   * 移除当前元素所有已绑定的事件
   *
   * @returns {NodeInterface} 当前 NodeInterface 实例
   */
   ...
   * @returns {NodeInterface | { [k: string]: string }} 返回值或当前实例
   */
...
export interface NodeIdInterface {
   ...
   * @returns
   */
  generate(root: Element | NodeInterface | DocumentFragment, force?: boolean): string | undefined;
  /**
   * 判断一个节点是否需要创建data-id
   *
   * @param name 节点名称
