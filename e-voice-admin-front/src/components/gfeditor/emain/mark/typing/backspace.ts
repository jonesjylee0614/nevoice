...
class Backspace {
  ...
  trigger(event: KeyboardEvent) {
    ...
    const { collapsed, endNode, startNode, startOffset, endOffset } = range.cloneRange().shrinkToTextNode();
    // 空的block节点下不处理mark
    if (collapsed) {
    ...
    if (endNode.type === Node.TEXT_NODE || startNode.type === Node.TEXT_NODE) {
      ...
      if (!collapsed) {
        ...
        if (markNode && endNode.type === Node.TEXT_NODE && node.isMark(markNode)) {
          const text = endNode.text();
          const rightText = text.substring(endOffset);
          // 不位于零宽字符前，不处理
      ...
      if (markNode && startNode.type === Node.TEXT_NODE && node.isMark(markNode)) {
        ...
        if (startOffset === 1) {
          ...
          if (prev && !node.isEmpty(prev)) {
            const { startNode, startOffset } = range.cloneRange().select(prev, true).shrinkToTextNode().collapse(false);
