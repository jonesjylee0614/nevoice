...
import type { EditorInterface, NodeInterface, Placement } from '../types';
import { $ } from '../node';
...
class Position {
  ...
  bind(
    container: NodeInterface,
    target: NodeInterface,
    defaultAlign: Placement = this.align,
    ...
    this.offset = offset;
    this.root = $(`<div style="position: absolute; top: 0px; left: 0px; width: 100%;"></div>`);
  ...
  update = (triggerUpdate: boolean = true) => {
    if (!this.container || this.container.length === 0 || !this.target || this.target.length === 0) return;
    const rect = domAlign(this.container.get<HTMLElement>(), this.target.get<HTMLElement>(), {
