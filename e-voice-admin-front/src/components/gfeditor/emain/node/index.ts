...
class NodeModel implements NodeModelInterface {
  ...
  isVoid(node: NodeInterface | Node | string, schema: SchemaInterface = this.editor.schema) {
    let name = typeof node === 'string' ? node : '';
    if (isNode(node)) name = node.nodeName.toLowerCase();
    else if (isNodeEntry(node)) name = node.name;
    return schema.find(rule => rule.name === name && rule.isVoid === true).length > 0;
  }

  isMark(node: NodeInterface | Node, schema: SchemaInterface = this.editor.schema) {
    return (
      schema
        .getTags('marks')
        .includes(((node as Node).nodeName?.toLowerCase() ?? (node as NodeInterface).name).toLowerCase()) &&
   ...
   * @param node 节点
   */
  isInline(node: NodeInterface | Node, schema: SchemaInterface = this.editor.schema) {
    return (
      schema.getTags('inlines').includes((node as Node).nodeName?.toLowerCase() ?? (node as NodeInterface).name) &&
   ...
   * @param node 节点
   */
  isBlock(node: NodeInterface | Node, schema: SchemaInterface = this.editor.schema) {
    return (
      schema
        .getTags('blocks')
        .includes(((node as Node).nodeName?.toLowerCase() ?? (node as NodeInterface).name).toLowerCase()) &&
  ...
  isNestedBlock(node: NodeInterface | Node) {
    ...
    const element = ((node as NodeInterface).length ? node[0] : node) as Element;
  ...
  isEmpty(node: NodeInterface, withTrim?: boolean) {
    if (node.length > 0 && node.isElement()) {
      ...
      const results = Array.from(
        element.querySelectorAll(`${CARD_SELECTOR},${READY_CARD_SELECTOR},${EDITABLE_SELECTOR},br`)
      );
      if (
        attributes[CARD_KEY] ||
        attributes[READY_CARD_KEY] ||
        (results.some(e => e.hasAttribute(CARD_KEY) || e.hasAttribute(READY_CARD_KEY)) &&
   ...
   */
  wrap(source: NodeInterface | Node, outer: NodeInterface, mergeSame: boolean = false) {
    ...
    if (mergeSame && this.isMark(outer)) {
      ...
      if (source.name === outer.name) {
        ...
        Object.keys(attributes).forEach(key => {
          if (!outer.attributes(key)) outer.attributes(key, attributes[key]);
            ...
            if (attributes[key] !== undefined && !attributes.includes(attributes[key]))
      ...
      children.forEach(child => {
        if (!child.isText() && this.isMark(child) && mark.compare(child, outerClone)) {
          this.unwrap(child);
        }
      });
      return source.replaceWith(outer);
   ...
   */
  merge(source: NodeInterface, target: NodeInterface, remove: boolean = true) {
    ...
    if (this.isCustomize(source)) {
      ...
      if (!sourceFirst?.isCard()) {
        ...
        const plugin = plugins.find(p => (p.constructor as PluginEntry).pluginName === pluginName);
        if (plugin?.cardName) {
      ...
      if (this.isCustomize(target) && !sourceFirst?.equal(target)) {
        ...
        if (targetFirst?.isCard() && sourceFirst!.attributes(CARD_KEY) === targetFirst.attributes(CARD_KEY))
          targetFirst.remove();
      }
    }
    ...
    while (child) {
      ...
      if (
        ...
        markPlugin &&
        plugin.disableMark &&
        plugin.disableMark!.includes((markPlugin.constructor as PluginEntry).pluginName)
      ) {
      ...
      // 孤立的零宽字符删除
        ...
        if (!parent || (!this.isMark(parent) && ((prev && !this.isInline(prev)) || (next && !this.isInline(next))))) {
          child.remove();
          child = next;
          continue;
        }
      ...
      // 移除mark插件下面的所有零宽字符
        ...
        if (!prev || prev.isText()) {
          child.allChildren().forEach(child => {
            ...
            if (child.type === getDocument().TEXT_NODE && Boolean(text) && !child.next()?.isCursor()) {
      ...
      // 追加到要合并的列表中
      if (child.length > 0 && !child.equal(source) && !child.parent()?.equal(source)) source.append(child);
      child = next;
    }
    // 移除需要合并的节点
   ...
   * @param target 新节点
   */
  replace(source: NodeInterface, target: NodeInterface, copyId: boolean = false) {
    const clone = this.clone(target, false, copyId);
    let childNode =
      this.isCustomize(source) && source.name === 'li' && source.first()?.isCard()
        ? source.first()?.next()
        : source.first();
   ...
   */
  insert(node: Node | NodeInterface, range?: RangeInterface, removeCurrentEmptyBlock: boolean = false) {
    if (isNodeEntry(node)) {
    ...
    const leftText = text.substring(0, startOffset);
    // 文本节点
    ...
    if (this.isBlock(node)) {
      ...
      let blockNode = block.closest(
        range.startNode.isEditable()
          ? range.cloneRange().shrinkToElementNode().shrinkToTextNode().startNode
          : range.startNode
      );
      if (blockNode.isRoot() && !range.startNode.next()) {
        ...
        if (splitNode && blockNode.name === name && canMergeTags.includes(blockNode.name)) {
          blockNode = splitNode;
        ...
        if (blockNode.isEditable() && blockNode.get<Node>()?.childNodes.length === 0) {
          blockNode.append(node);
        } else {
          if (this.isEmptyWidthChild(blockNode) || block.isLastOffset(range, 'start')) {
            blockNode.after(node);
            // 没有分割就不会有新增的行就不用删除
            if (this.isEmptyWidthChild(blockNode) && splitNode) blockNode.remove();
          } else {
            blockNode.before(node);
            if (splitNode && this.isEmptyWidthChild(splitNode)) splitNode.remove();
          }
      ...
      const targetPlugin = targetNode ? block.findPlugin(targetNode) : undefined;
      // 先移除不能放入块级节点的mark标签
      if (targetPlugin) {
        const nodeDom = $(node);
        const isUnwrap = (markNode: NodeInterface) => {
          if (this.isMark(markNode)) {
            ...
            if (
              ...
              targetPlugin.disableMark.includes((markPlugin.constructor as PluginEntry).pluginName)
            ) {
              return true;
            }
          }
    ...
    if (
      ...
      ((node as Element).hasAttribute(READY_CARD_KEY) || (node as Element).hasAttribute(CARD_KEY))
    ) {
      return range.collapse(false);
    }
    return range
      .select(node, !(this.isVoid(node) || node.nodeType === Node.TEXT_NODE))
      .shrinkToElementNode()
      .collapse(false);
  }
  ...
  removeMinusStyle(node: NodeInterface, style: string) {
    if (node.isElement()) {
      ...
      if (styles[style]) {
        const val = Number.parseInt(styles[style] || '0', 10) || 0;
  ...
  mergeChild(node: NodeInterface) {
    ...
    while (childDom) {
      ...
      while (
        ...
        ((topTags.includes(childDom.name) && !this.isList(childDom)) ||
  ...
  addBrForBlock = (blockNode: NodeInterface) => {
    ...
    for (const child of children) {
      if (allText && child.nodeType === Node.TEXT_NODE) {
        allText = false;
      } else if (!isP && !hasBlock && child.nodeType === Node.ELEMENT_NODE && this.isBlock(child)) {
        hasBlock = true;
      }
      if (
        child.nodeType === Node.ELEMENT_NODE &&
        [ANCHOR, FOCUS, CURSOR].indexOf((child as Element).getAttribute(DATA_ELEMENT) || '') < 1
      ) {
        notCursorCount++;
      }
  ...
  flat(node: NodeInterface, root: NodeInterface = node) {
    ...
    while (childNode) {
      ...
      // 如果当前是块级标签，递归循环
        ...
        const isLI = cloneNode.name === 'li';
        childNode.before(cloneNode);

        while (childNode) {
          nextNode = childNode.next();

          const isBR = childNode.name === 'br' && !isLI;
          ...
          if (childNode.isText()) {
            ...
            match = /((\n|\r)+)$/.exec(text);
            if (match) {
              childNode.text(text.substring(0, match.index));
              cloneNode.append(childNode);
              break;
   ...
   * @returns {NodeEntry | string} 当前实例或html文本
   */
   ...
   * @returns 复制后的元素节点
   */
  clone(node: NodeInterface, deep?: boolean, copyId: boolean = true): NodeInterface {
  ...
  getBatchAppendHTML(nodes: Array<NodeInterface>, selector: string) {
    ...
    let appendNode = selector.startsWith('\\u') || selector.startsWith('&#') ? $(selector, null) : $(selector);
    nodes.forEach(node => {
  ...
  removeZeroWidthSpace(node: NodeInterface) {
    node.traverse(child => {
      ...
      if (
        ...
        [ANCHOR, FOCUS, CURSOR].includes((<Element>next).getAttribute(DATA_ELEMENT) || '')
      ) {
        return;
      }
      ...
      if (
        text.charCodeAt(1) === 0x200b &&
        ((!next && parent && this.isInline(parent)) || (next && this.isInline(next)))
      ) {
        return;
      }
      ...
      if (
        ...
        ((!prev && parent && this.isInline(parent)) || (prev && this.isInline(prev)))
      ) {
        return;
      }
      ...
      if (text.charCodeAt(0) === 0x200b) {
        ...
        if (newNode.previousSibling) newNode.parentNode?.removeChild(newNode.previousSibling);
      }
    });
  }
}
