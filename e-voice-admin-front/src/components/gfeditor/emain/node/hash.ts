...
export default (value: any, unique: boolean = true) => {
  let prefix = '';
  if (typeof value !== 'string') {
    ...
    if (node.nodeType === Node.ELEMENT_NODE) {
      ...
      for (let i = attributes.length; i--; ) {
        ...
        if (![DATA_ID, 'id'].includes(item.name)) value += `${item.name}="${item.value}"`;
      }
    } else {
  ...
  if (!cachePerfix) {
    ...
    prefix = prefix + md5Value.substring(0, 4) + md5Value.substring(md5Value.length - 3);
