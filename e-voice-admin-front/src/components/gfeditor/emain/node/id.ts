import type { NodeIdInterface, NodeInterface, SchemaInterface, SchemaRule } from '../types';
import { EditorInterface } from '../types';
import { CARD_SELECTOR, DATA_ELEMENT, DATA_ID, UI, UI_SELECTOR } from '../constants';
...
class NodeId implements NodeIdInterface {
   ...
   * @param root 根节点
   */
  generateAll(root: Element | NodeInterface | DocumentFragment, force: boolean = false) {
  ...
  generate(node: Node | NodeInterface, force: boolean = false) {
    ...
    if (
      !nodeRules ||
      nodeRules.length === 0 ||
      !nodeRules.some(rule => this.schema.checkNode(node as NodeInterface, rule.attributes))
    ) {
      return;
    }
    // 检测节点是否再ui或者不可编辑卡片内
    const closestNode = node.closest(`${CARD_SELECTOR},${UI_SELECTOR}`, getParentInRoot);
    // ui 节点内
