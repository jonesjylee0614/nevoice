...
import { EDITABLE_SELECTOR, ROOT_SELECTOR } from '../constants/root';
import { getComputedStyle, getDocument, getStyleMap, removeUnit, toCamelCase } from '../utils';
import type { Context, ElementInterface, EventInterface, EventListener, NodeInterface, Selector } from '../types';
...
class NodeEntry implements NodeInterface {
  ...
  isRoot(root?: Node | NodeInterface) {
    const element = this.get<HTMLElement>();
    return element?.nodeType === Node.ELEMENT_NODE && isRoot(element, root ? (root[0] ?? root) : undefined);
  }
  ...
  inEditor(root?: Node | NodeInterface) {
    ...
    return Boolean(element) && inEditor(element, root ? (root[0] ?? root) : undefined);
  }
   ...
   *
   * @param {number} index
   * @returns {NodeEntry | undefined} NodeEntry 类，或 undefined
   */
  ...
  children(selector?: string): NodeInterface {
    if (this.length === 0) return new NodeEntry([]);
  ...
  prevElement(): NodeInterface | null {
    const node = this.length === 0 ? null : this.get<Element>()!.previousElementSibling;
    return node ? new NodeEntry(node) : null;
  }
  ...
  nextElement(): NodeInterface | null {
    const node = this.length === 0 ? null : this.get<Element>()!.nextElementSibling;
    return node ? new NodeEntry(node) : null;
  }
  ...
  getPath(
    ...
    callback?: (index: number, path: number[], node: NodeInterface) => number[] | undefined
  ): Array<number> {
  ...
  contains(node: NodeInterface | Node): boolean {
    ...
    if (this.get()!.nodeType === Node.DOCUMENT_NODE && domNode?.nodeType !== Node.DOCUMENT_NODE) {
      return true;
    }
   ...
   * @returns 返回一个 NodeEntry 实例
   */
  find(selector: string): NodeInterface {
    if (this.length > 0 && (this.isElement() || this.fragment)) {
      const nodeList = (this.fragment ? this.fragment : this.get<Element>())?.querySelectorAll(selector);
   ...
   * @returns {NodeEntry | { [k: string]: string }} 返回值或当前实例
   */
  ...
  attributes(
    ...
    if (val === undefined) {
      const element = this.get<Element>();
      return this.length > 0 && this.isElement() ? element?.getAttribute(key) || '' : '';
    }
  ...
  css(
    ...
    if (val === undefined) {
      ...
      const element = this.get<HTMLElement>();
      if (!element) return '';
      return element.style[toCamelCase(key)] || getComputedStyle(<Element>this[0], key) || '';
    }
  ...
  height(): number {
    ...
    return height ? Number.parseFloat(height) || 0 : 0;
  }
  ...
  traverse(
    ...
    const walk = (node: NodeInterface) => {
      ...
      if (isCard && (!includeCard || (includeCard === 'editable' && !node.isEditableCard()))) {
        return;
      }
  ...
  findParent(container: Node | NodeInterface = this.closest(ROOT_SELECTOR)): NodeInterface | null {
  ...
  getViewport() {
    ...
    const element = this.isText() ? this.parent()?.get<Element>() : this.get<Element>();
    if (!element)
      return {
        top: 0,
        left: 0,
  ...
  scrollIntoView(view: NodeInterface, align: 'start' | 'center' | 'end' | 'nearest' = 'nearest') {
