...
import type { ElementInterface, NodeInterface, Selector } from '../types';
import { DATA_ELEMENT, EDITABLE, EDITABLE_SELECTOR, ROOT, ROOT_SELECTOR } from '../constants/root';
...
export const isNodeEntry = (selector: Selector): selector is NodeInterface => {
  return Boolean(selector) && (selector as NodeInterface).get !== undefined;
};

export const isNodeList = (selector: Selector): selector is NodeList => {
  return Boolean(selector) && (selector as NodeList).entries !== undefined;
};

export const isNode = (selector: Selector): selector is Node => {
  return Boolean(selector) && (selector as Node).nodeType !== undefined;
};
 ...
 * @param selector 选择器
 */
export const isMatchesSelector = (element: ElementInterface, selector: string) => {
  ...
  const defaultMatches = (element: Element, selector: string) => {
    ...
    let i = matches ? matches.length : 0;
...
export const isCard = (element: Element) => {
  return element.nodeName === CARD_TAG || Boolean(element.getAttribute(CARD_TYPE_KEY));
};
/** 判断当前节点是否为block类型的Card组件 */
export const isBlockCard = (element: Element) => {
  return element.getAttribute(CARD_TYPE_KEY) === 'block';
};
/**
...
export const isInlineCard = (element: Element) => {
  return element.getAttribute(CARD_TYPE_KEY) === 'inline';
};
/**
 * 是否是可编辑器卡片
 *
...
export const isEditableCard = (element: Element) => {
  return (
    ...
    Boolean(element?.querySelector(EDITABLE_SELECTOR))
  );
};
...
export const inEditor = (element: Node, root?: Node) => {
  if (element.nodeType === Node.ELEMENT_NODE && isRoot(element as Element, root)) {
    return false;
  }
  const closetRoot = closest(element, ROOT_SELECTOR);
  return Boolean(closetRoot) && (!root || closetRoot === root);
};
...
export const isCursor = (element: Element) => {
  ...
  return Boolean(dataElement) && [ANCHOR, FOCUS, CURSOR].includes(dataElement);
};
