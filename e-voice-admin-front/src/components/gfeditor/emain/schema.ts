...
class Schema implements SchemaInterface {
  ...
  /**
   * 增加规则 只有 type 和 attributes 时，将作为此类型全局属性，与其它所有同类型标签属性将合并
   *
   * @param rules 规则
   */
  add(rules: SchemaRule | SchemaGlobal | Array<SchemaRule | SchemaGlobal>) {
    ...
    rules.forEach(rule => {
      if (isSchemaRule(rule)) {
            ...
            if (key === 'style') {
              Object.keys(rule.attributes!.style).forEach(styleName => {
                if (
                  ...
                  this.data.globals[rule.type][key][styleName] === rule.attributes!.style[styleName]
                ) {
                  delete rule.attributes!.style[styleName];
                }
              });
            } else if (this.data.globals[rule.type][key] === rule.attributes![key]) {
        ...
        this.data.globals[rule.type] = merge({ ...this.data.globals[rule.type] }, rule.attributes);
      }
    });
    this.updateTagMap();
    // 按照必要属性个数排序
  ...
  getType(node: NodeInterface | Node, filter?: (rule: SchemaRule) => boolean) {
    ...
    if (Boolean(this._typeMap[id]) && (!filter || filter(this._typeMap[id]!))) return this._typeMap[id].type;
  ...
  getRule(node: NodeInterface | Node, filter?: (rule: SchemaRule) => boolean) {
    const element = (isNode(node) ? node : node[0]) as Element;
    filter ||= rule => rule.name === element.localName;
    return this._all.find(rule => filter!(rule) && this.checkNode(element, rule.attributes));
  }
  ...
  checkNode(node: NodeInterface | Node, attributes: SchemaAttributes | SchemaStyle = {}): boolean {
    ...
    for (const attributesName in attributes) {
      ...
      if (!this.checkValue(schema, attributesName, element.getAttribute(attributesName) ?? undefined)) return false;
    }
  ...
  checkValue(schema: SchemaAttributes, attributesName: string, attributesValue?: string, force?: boolean): boolean {
    ...
    if (typeof rule === 'string') {
      ...
      if (attributesName === 'class') {
        return (attributesValue || '').split(/\s+/).some(value => value.trim() === rule);
      }
    ...
    if (Array.isArray(rule)) {
      if (attributesName === 'class') {
        ...
        return attributesValue
          .split(/\s+/)
          .every(value => (value.trim() === '' ? true : (rule as Array<string>).includes(value.trim())));
      }
      return rule.includes(attributesValue);
    }
    /** 解析正则表达式 */
    if (typeof rule === 'object' && typeof rule.test === 'function') {
      if (attributesName === 'class') {
        return (attributesValue || '')
          .split(/\s+/)
          .every(value => (value.trim() === '' ? true : (rule as RegExp).test(value.trim())));
      }
   ...
   * @param rule 规则
   */
  filterStyles(styles: { [k: string]: string }, rule: SchemaRule, callback?: (name: string, value: string) => void) {
    Object.keys(styles).forEach(styleName => {
      if (
        !rule.attributes?.style ||
        !this.checkValue(rule.attributes!.style as SchemaAttributes, styleName, styles[styleName], true)
      ) {
  ...
  filterAttributes(
    ...
    Object.keys(attributes).forEach(attributesName => {
      if (
        !rule.attributes ||
        !this.checkValue(rule.attributes as SchemaAttributes, attributesName, attributes[attributesName], true)
      ) {
  ...
  filter(
    ...
    const allRule = { ...rule, attributes: merge({}, rule.attributes, globalRule ? globals[globalRule] : {}) };
    this.filterAttributes(attributes, allRule, apply ? name => node.removeAttributes(name) : undefined);
    this.filterStyles(styles, allRule, apply ? name => node.css(name, '') : undefined);
  }
  ...
  isAllowIn(source: string, target: string) {
    ...
    return this.data.blocks.some(block => {
      ...
      const schema = block as SchemaBlock;
      if (schema.allowIn && schema.allowIn.includes(source)) {
        return true;
      }
    });
  }
  addAllowIn(parent: string, child: string = 'p') {
    const rule = this.data.blocks.find(rule => rule.name === child) as SchemaBlock;
    if (!rule.allowIn) {
      rule.allowIn = [];
    }
  ...
  getCanMergeTags() {
    ...
    this.data.blocks.forEach(rule => {
      ...
      if (schema.canMerge === true) {
        if (!tags.includes(schema.name)) tags.push(schema.name);
      }
    });
    return tags;
  }
...
export const isSchemaValueObject = (value: SchemaValue): value is SchemaValueObject => {
  return (value as SchemaValueObject).required !== undefined;
};

export const isSchemaRule = (rule: SchemaRule | SchemaGlobal): rule is SchemaRule => {
