...
import type { InputInterface, InputOptions } from '../types/toolbar';
...
const template = (options: InputOptions) => {
  return `
    <span class="data-toolbar-item data-toolbar-item-input">
        ${
          options.prefix ? `<span class='data-toolbar-input-prefix'>${escape(options.prefix)}</span>` : ''
        }<input data-role="input" placeholder="${escape(
          options.placeholder
        )}" class="data-toolbar-input" type="input" value="${escape(options.value.toString())}" />${
          options.suffix ? `<span class='data-toolbar-input-suffix'>${escape(options.suffix)}</span>` : ''
        }
    </span>`;
};
