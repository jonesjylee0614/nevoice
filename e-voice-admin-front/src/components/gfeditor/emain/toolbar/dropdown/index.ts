...
import type { DropdownInterface, DropdownOptions } from '../../types/toolbar';
import { $ } from '../../node';
...
export default class Dropdown implements DropdownInterface {
  ...
  documentMouseDown = (e: MouseEvent) => {
    if (!this.root) return;
    if (!this.root[0].contains(e.target as Node) && this.dropdown?.hasClass('show')) {
      this.hideDropdown();
    }
  };
  ...
  getPlacement() {
    const dataPlacement = this.root!.closest('.data-toolbar').attributes('data-placement') || 'top';
