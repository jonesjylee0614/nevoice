...
import type { EngineInterface, TypingHandle, TypingHandleInterface, TypingInterface } from '../types';
import { $ } from '../node';
...
class Typing implements TypingInterface {
  ...
  getHandleListener(name: string, type: 'keydown' | 'keyup'): TypingHandleInterface | undefined {
    return this.handleListeners.find(listener => listener.name === name && listener.handle.type === type)?.handle;
  }
  ...
  trigger(type: 'keydown' | 'keyup', event: KeyboardEvent) {
    ...
    const result = this.handleListeners
      ...
      .some(listener => {
        ...
        if (name === 'default' || Boolean(!handle.hotkey)) return false;
        if (typeof handle.hotkey === 'function' ? handle.hotkey(event) : isHotkey(handle.hotkey, event)) {
          let params = [event];
          if (typeof triggerParams === 'function') params = triggerParams(this.engine, event);
          if (!triggerName || this.engine.trigger(triggerName, ...params) !== false) {
