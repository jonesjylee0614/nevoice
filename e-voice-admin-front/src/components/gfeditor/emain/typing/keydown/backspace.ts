import type { NodeInterface, TypingHandleInterface } from '../../types';
import { $ } from '../../node';
...
class Backspace extends DefaultKeydown implements TypingHandleInterface {
  ...
  trigger(event: KeyboardEvent) {
    ...
    if (!range.collapsed) {
      ...
      if (afterPrev?.isCard() && afterPrev.find(CARD_CENTER_SELECTOR).length === 0) {
        afterPrev.remove();
      }
      change.apply(range);
      if (this.engine.scrollNode) range.scrollIntoViewIfNeeded(this.engine.container, this.engine.scrollNode);
    } else {
      let brNode: NodeInterface | undefined;
      ...
      if (brNode?.name === 'br') {
        ...
        } else if (next?.name !== 'br' && prev?.name === 'br' && p?.name !== 'br') {
          event.preventDefault();
          brNode.remove();
          prev.remove();
          change.apply(range.shrinkToTextNode());
