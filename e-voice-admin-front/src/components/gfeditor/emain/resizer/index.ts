...
class Resizer implements ResizerInterface {
  ...
  onMouseDown(event: MouseEvent | TouchEvent, position: ResizerPosition) {
    ...
    this.root.css('top', ['right-top', 'left-top'].includes(position) ? 'auto' : 0);
    this.root.css('left', ['left-top', 'left-bottom'].includes(position) ? 'auto' : 0);
    this.root.css('bottom', ['right-bottom', 'left-bottom'].includes(position) ? 'auto' : 0);
    this.root.css('right', ['right-top', 'right-bottom'].includes(position) ? 'auto' : 0);
    this.point = {
  ...
  onMouseUp = (event: MouseEvent | TouchEvent) => {
    ...
    document.removeEventListener(isMobile ? 'touchmove' : 'mousemove', this.onMouseMove);
    document.removeEventListener(isMobile ? 'touchend' : 'mouseup', this.onMouseUp);
  ...
  updateSize(width: number, height: number) {
    if (['right-top', 'right-bottom'].includes(this.position || '')) {
  ...
  render() {
    const { width, height } = this.options;
    this.setSize(width, height);
    this.root.find('.data-resizer-holder-right-top').on(isMobile ? 'touchstart' : 'mousedown', event => {
      return this.onMouseDown(event, 'right-top');
    });
    this.root.find('.data-resizer-holder-right-bottom').on(isMobile ? 'touchstart' : 'mousedown', event => {
      return this.onMouseDown(event, 'right-bottom');
    });
    this.root.find('.data-resizer-holder-left-bottom').on(isMobile ? 'touchstart' : 'mousedown', event => {
      return this.onMouseDown(event, 'left-bottom');
    });
    this.root.find('.data-resizer-holder-left-top').on(isMobile ? 'touchstart' : 'mousedown', event => {
