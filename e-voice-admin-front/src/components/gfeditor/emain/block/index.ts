import { CARD_KEY, CARD_SELECTOR, CURSOR_SELECTOR, DATA_ID, READY_CARD_KEY, ROOT_SELECTOR } from '../constants';
import Range from '../range';
import type { EditorInterface, NodeInterface, PluginEntry, RangeInterface } from '../types';
import type { BlockInterface, BlockModelInterface } from '../types/block';
import { convertMarkdown, createMarkdownIt, getDocument, isEngine } from '../utils';
import { $ } from '../node';
...
class Block implements BlockModelInterface {
    ...
    for (const pluginName in plugin.components) {
      ...
      if (
        ...
        (!blockPlugin.tagName || typeof blockPlugin.tagName === 'string'
          ...
          : blockPlugin.tagName.includes(block.name))
      ) {
        const schemaRule = blockPlugin.schema();
        if (
          Array.isArray(schemaRule)
            ? schemaRule.find(rule => schema.checkNode(block, rule.attributes))
  ...
  findTop(parentNode: NodeInterface, childNode: NodeInterface) {
    ...
    if (topParentName === parent.name || topChildName === childNode.name) return childNode;
    ...
    if (node.isList(parentNode) && node.isList(childNode)) {
      const childIndent = Number.parseInt(childNode.attributes(list.INDENT_KEY), 10) || 0;
      const parentIndent = Number.parseInt(parentNode.attributes(list.INDENT_KEY), 10) || 0;
   ...
   * @param callback 回调
   */
  closest(node: NodeInterface, callback: (node: NodeInterface) => boolean = () => true) {
        ...
        if (targetPlugin) {
          child.allChildren().forEach(markNode => {
            if (node.isMark(markNode)) {
              ...
              if (targetPlugin.disableMark?.indexOf((markPlugin.constructor as PluginEntry).pluginName)) {
  ...
  unwrap(block: NodeInterface | Node | string, range?: RangeInterface) {
    ...
    let leftParent: NodeInterface | undefined;
    ...
    let rightParent: NodeInterface | undefined;
    if (hasRight) {
    ...
    blocks.forEach(item => {
      ...
      if (status === 'center') {
        if (parent?.name === (block as NodeInterface)?.name && parent?.inEditor()) {
    ...
    if (leftParent && leftParent.name === 'ol' && rightParent && rightParent.name === 'ol') {
      rightParent.attributes(
        'start',
        (Number.parseInt(leftParent.attributes('start'), 10) || 1) + leftParent.find('li').length
      );
    }
    selection.move();
    if (!range) change.apply(safeRange);
  ...
  split(range?: RangeInterface) {
    ...
    const isEmptyElement = (node: Node) => {
      return nodeApi.isBlock(node) && (node.childNodes.length === 0 || (node as HTMLElement).textContent === '');
    };
    ...
    if (nodeApi.isEmpty(block)) {
      nodeApi.html(block, nodeApi.getBatchAppendHTML(activeMarks, activeMarks.length > 0 ? '&#8203;' : '<br />'));
    }

    if (nodeApi.isEmpty(sideBlock)) {
      nodeApi.html(sideBlock, nodeApi.getBatchAppendHTML(activeMarks, activeMarks.length > 0 ? '&#8203;' : '<br />'));
    }
    ...
    if (sideBlock.get<Node>()?.childNodes.length === 1 && sideBlock.first()?.name === 'br') {
  ...
  insert(
    ...
    // to
    // <p><br /><cursor /></p>
    if (container.get<Node>()?.childNodes.length === 1 && container.first()?.name === 'br') {
  ...
  setBlocks(block: string | { [k: string]: any }, range?: RangeInterface) {
    ...
    if (startNode.isEditable() && blocks.length === 0) {
      ...
      // 复制全局属性
      const globals = (schema.data.globals.block || {}) as any;
      ...
      const globalStyles = (globals.style || {}) as any;
    ...
    blocks.forEach(child => {
      ...
      // 相同标签，或者只传入样式属性
      if (!targetNode || (this.findPlugin(child) === targetPlugin && child.name === targetNode.name)) {
    ...
    if (block.length > 0) {
      ...
      while (nextNode && tags.indexOf(nextNode.name) > 0) {
        ...
        if (
          ...
          nextAttributes.class === (prevAttributes ? prevAttributes.class : undefined) &&
          Object.keys(nextAttributes).join(',') === Object.keys(prevAttributes || {}).join(',')
        ) {
  ...
  findBlocks(range: RangeInterface) {
    ...
    if (!range.startNode.inEditor() || editor.card.find(range.startNode)?.type === CardType.BLOCK) return [];
    ...
    // 不存在时添加
    const addNode = (nodes: Array<NodeInterface>, nodeB: NodeInterface, preppend?: boolean) => {
      if (
        !nodes.some(nodeA => {
    ...
    const selectionNodes = isEditable ? (card?.getSelectionNodes ? card.getSelectionNodes() : []) : [];
  ...
  isLastOffset(range: RangeInterface, edge: 'start' | 'end') {
    ...
    return emptyNode.find('br').length <= 0 && node.isEmpty(emptyNode);
  }
  ...
  getBlocks(range: RangeInterface) {
    ...
    const card = editor.card.find(commonAncestorNode, true);
    let isEditable = card?.isEditable;
    const selectionNodes = isEditable ? (card?.getSelectionNodes ? card.getSelectionNodes() : []) : [closest];
    if (selectionNodes.length === 0) {
      isEditable = false;
  ...
  getBlockByRange({
    ...
    const cloneBlock = keepDataId ? block.clone(false) : editor.node.clone(block, false, false);
  ...
  flat(block: NodeInterface, root: NodeInterface) {
    ...
    while (parentNode && rootElement && parentNode.get() !== rootElement && parentNode.inEditor()) {
      ...
      if (block.isCard()) parentNode.before(block);
        ...
        (node.isList(parentNode) && block.name === 'li') ||
        // 如果是父级可合并标签，并且当前节点是根block节点，并且不是 父节点一样的block节点
        (mergeTags.includes(parentNode.name) && node.isBlock(block) && parentNode.name !== block.name)
      ) {
        // 复制节点
  ...
  insertOrSplit(range: RangeInterface, block: NodeInterface) {
    ...
    if (
      ...
      (cloneRange.endNode.type === Node.ELEMENT_NODE &&
        ...
        cloneRange.endContainer.childNodes[cloneRange.endOffset] === block.last()?.get() &&
        block.first()?.name === 'br')
    ) {
