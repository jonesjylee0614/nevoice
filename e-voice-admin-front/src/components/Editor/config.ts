...
import type { CardEntry, EngineInterface, PluginEntry, PluginOptions } from '@/components/gfeditor/emain';
// 引入插件
...
import { ToolbarComponent, ToolbarPlugin, fontFamilyDefaultData } from '@/components/gfeditor/toolbar';
    ...
    file: {
      ...
      headers: () => {
        ...
        return {
          ...
          'verify-encrypt': md5(import.meta.env.VITE_ENCRYPT + timestamp),
          Authorization: token
        };
      }
    },
    remote: {
      action: `${apiHost}/upload/thirdImage`,
      headers: () => {
        const token = getToken();
        const timestamp: number = Date.parse(new Date().toString()) / 1000;
        return {
