<template>
  <GfLoading :loading="loading" text="编辑器初始中">
    <div class="editor-wrapper" :style="{ height: `${minHeight}px` }">
      <GfToolbar v-if="engine" :engine="engine" :items="items" />
...
import Engine from '@/components/gfeditor/emain';
import GfToolbar from '@/components/gfeditor/toolbar';
import GfLoading from './Loading.vue';
import { cards, onLoad, pluginConfig, plugins } from './config';
...
onMounted(() => {
  ...
  document.addEventListener('keydown', (e: any) => {
    if (e.keyCode == 83 && (/Mac/i.test(navigator.userAgent) ? e.metaKey : e.ctrlKey)) {
      e.preventDefault();
    }
  });
});
