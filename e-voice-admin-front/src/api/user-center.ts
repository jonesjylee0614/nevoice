...
export function saveInfo(params: any) {
  return defHttp.post({ url: Api.saveInfo, params }, { errorMessageMode: 'message' });
}
// 校验密码正确性
...
export function checkPassword(params: any) {
  ...
  return defHttp.post({ url: Api.checkPassword, params }, { errorMessageMode: 'message' });
}
// 修改密码
...
export function changePassword(params: ChangeData) {
  params = { ...params, oldpassword: md5(params.oldpassword) }; // 加密推送
  params = { ...params, password: md5(params.password) }; // 加密推送
  return defHttp.post({ url: Api.changePassword, params }, { errorMessageMode: 'message' });
}
