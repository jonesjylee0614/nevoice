...
// 更新生成代码数据表
export function upCodeTable(params: any) {
  return defHttp.post({ url: Api.upCodeTable, params }, { errorMessageMode: 'message' });
}
// 提交数据
...
// 获取数据表字段
export function getdbfield(params: any) {
  return defHttp.get({ url: Api.getdbfield, params }, { errorMessageMode: 'message' });
}
// 获取数据表字段
export function get_tablelist(params: any) {
  return defHttp.get({ url: Api.get_tablelist, params }, { errorMessageMode: 'message' });
}
/** 数据类型 */
