  ...
  <div class="login-form-wrapper">
    ...
    <AForm :model="userInfo" class="login-form" layout="vertical" @submit="handleSubmit">
      <AFormItem
        ...
        hide-label
      >
        <AInput v-model="userInfo.email" :placeholder="$t('login.form.getpemail.placeholder')">
      ...
      <AFormItem
        ...
        hide-label
      >
        <AInput v-model="userInfo.code" :placeholder="$t('login.form.verification.placeholder')">
      ...
      <ASpace :size="16" direction="vertical">
        <div class="login-form-password-actions">
          <span></span>
          <ALink @click="GoToType('login')">去登陆</ALink>
        </div>
        <AButton type="primary" html-type="submit" long :loading="loading">重置密码</AButton>
      </ASpace>
    </AForm>
  </div>
</template>
