    ...
    <div class="addFormbox" :style="{ 'min-height': `${windHeight}px` }">
      <div class="tabs-header">
        <div class="tabs-nav-wrap">
          <template v-for="item in tapList">
            <div
              v-if="item.id == 1 || (item.id == 2 && selectdata.filetype == 'image')"
              :key="item.id"
              class="tap_item"
              :class="{ item_active: activeKey == item.id }"
      ...
      <div class="tabs-content">
        <div class="content_box">
          <!--我的附件-->
          <div v-show="activeKey == 1" class="myfile" :style="{ height: `${windHeight}px` }">
            <div class="header_toopl">
              <div class="btnlist">
                <AUpload :accept="upaccept" :show-file-list="false" :multiple="true" :custom-request="customRequest">
                ...
                <AButton style="margin-left: 10px" @click="onAddFolder">
                  <template #icon>
                    <icon-folder-add />
                  </template>
                  新增文件夹
              ...
              <div class="search">
                <AInputSearch
                  ...
                  allow-clear
                  placeholder="搜索文件名称"
                  @press-enter="onSearchMyfile"
                  @search="onSearchMyfile"
                />
              </div>
            </div>
            <!--目录菜单h: 22+12-->
            <div v-if="dirmenu && dirmenu.length > 0" class="dirmun">
              <span v-for="item in dirmenu" :key="item.id" class="dirmun_item" @click="onDirmun(item)">
            ...
            <div
              class="file_list"
              :style="{
                height: `${windHeight - (dirmenu.length > 0 ? 80 : 55)}px`
              }"
            >
              <AScrollbar
                style="overflow: auto"
              ...
              >
                <div class="file_list_box">
                  <div v-for="item in myFileList" :key="item.id" class="file_item">
                    <div>
                      <div
                        :class="{
                          imgbox: item.type == 0 || item.type == 2,
                          folderbox: item.type == 1
                        }"
                        @click="onSelectImg(item)"
                      >
                        <!--图片-->
                        <div v-if="item.type == 0" class="img_raw">
                          <div class="img_preview">
                            <img :src="item.url" class="img_item" alt="" />
                          </div>
                        </div>
                        <!--视频-->
                        <div v-else-if="item.type == 2" class="img_raw">
                        ...
                        <div
                          class="edit_layer"
                          :class="{
                            l_active: selectList.includes(`my_${item.id}`)
                          }"
                          :draggable="item.type == 0 || item.type == 2"
                          @dragstart="drag($event, item.id)"
                        >
                          <Icon
                            v-if="item.is_common != 1"
                            icon="svgfont-guanbi"
                            class="colse"
                            :size="18"
                            color="red"
                            @click="onDel($event, item.id)"
                          />
...
// api
import type { FileItem, PictureItem } from '@/api/datacenter/attachment';
import { delFile, getMyFiles, saveFile, upImgPid } from '@/api/datacenter/attachment';
...
export default defineComponent({
  ...
  setup(_, { emit }) {
    ...
    const pagedata = reactive({
      activeKey: 1,
      myFileList: file_item as any, // 我的文件
      searchword: '',
      typeid: 0, // 素材图/插图
      pid: 0, // 当前层级id 0=父级
          ...
          const resdata = (await userUploadApi(
            {
              ...
              data: {
                pid: pagedata.pid,
                filetype: pagedata.selectdata.filetype
              }
            },
            onUploadProgress
          )) as any;
          ...
          if (resdata) {
            if (resdata.code == 0) {
              if (resdata.message == '文件已上传') {
                pagedata.myFileList = pagedata.myFileList.filter((item: any) => resdata.data.id != item.id);
              }
    ...
    const selectLibrary = (imgs: any, picture: any) => {
      ...
      pagedata.selectList = pagedata.selectList.filter(item => !item.includes('mt_'));
      imgs.forEach((item: any) => {
        if (!pagedata.selectList.includes(item)) {
        ...
        if (pagedata.selectList && pagedata.selectList.length > 0) {
          ...
          pagedata.selectList.forEach(item => {
            ...
            if (pagedata.selectdata.filetype == 'image') {
              if (id_arr[0] == 'my') {
                // 去我的附件找
                const myfile = pagedata.myFileList.find((data: any) => data.id == Number.parseInt(id_arr[1], 10));
                if (myfile) {
                  img_list.push(myfile.url);
                }
              } else if (id_arr[0] == 'mt') {
                // 去图库找
                const pic = pagedata.picture.find(data => data.id == Number.parseInt(id_arr[1], 10));
                if (pic) {
          ...
          if (pagedata.selectdata.filetype == 'image') {
            ...
            if (pagedata.selectdata.getnumber == 'one') {
              emit('success', {
                param: param.value,
                type: pagedata.selectdata.getnumber,
                url: img_list[0],
                list: [],
                data: pagedata.selectdata.exdata
              });
            } else {
              emit('success', {
                param: param.value,
    ...
    const getMyfile = async (searchword: any) => {
      setLoading(true);
      const getdbdata = await getMyFiles({
        searchword,
        pid: pagedata.pid,
        filetype: pagedata.selectdata.filetype
      });
      if (getdbdata) {
        ...
        if (getdbdata.dirmenu.length > 0) {
          const parntList: any = [{ id: 0, title: '默认文件夹', pid: 0 }];
    ...
    function onSelectImg(item: any) {
      if (item.type == 0) {
        // 是图片//或图片库
        const idStr = `my_${item.id}`;
        if (pagedata.selectList.includes(idStr)) {
          pagedata.selectList = pagedata.selectList.filter(id => id != idStr);
        } else if (pagedata.selectdata.getnumber == 'one') {
          pagedata.selectList = [idStr];
        } else {
          pagedata.selectList.push(idStr);
        }
      } else if (item.type == 2) {
        const idStr = `my_${item.id}`;
        if (pagedata.selectList.includes(idStr)) {
    ...
    const onUpImgPid = async (imgid: any, pid: any) => {
      const resultdata = await upImgPid({ imgid, pid });
      if (resultdata) {
        pagedata.myFileList = pagedata.myFileList.filter((item: any) => imgid != item.id);
        Message.success({
          content: '移动成功！',
          id: 'upImgPid',
          duration: 2000
    ...
    const onDel = (event: any, id: any) => {
      ...
      Modal.warning({
        ...
        onOk: async () => {
          ...
          if (resultdata) {
            pagedata.myFileList = pagedata.myFileList.filter((item: any) => id != item.id);
            Message.success({
              content: '删除成功',
              id: 'delFile',
              duration: 2
    ...
    const onUpFile = async (id: any, ev: any) => {
      ...
      if (ischangname.value) {
        ...
        const resultdata = await saveFile({ id, title });
        if (resultdata) {
          Message.success({
            content: '更新成功！',
            id: 'upFile',
