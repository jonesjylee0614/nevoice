<template>
  <div class="file_Library">
    <div class="header_toopl">
      <div class="btnlist">
        <ARadioGroup v-model="typeid" type="button" size="large" @change="changeMeterType">
          <ARadio :value="0">素材图</ARadio>
          <ARadio :value="1">插图</ARadio>
        </ARadioGroup>
      </div>
      <div class="search">
        <AInputSearch
          ...
          allow-clear
          placeholder="搜索素材名称"
          @press-enter="onSearchMatter"
          @search="onSearchMatter"
        />
      </div>
    </div>
    <!--分类-->
    <div v-if="typeid == 0" class="catelist">
      <AScrollbar style="height: 52px; overflow: auto; width: 100%">
        <ul class="cateul">
    ...
    <div v-if="typeid == 1" class="catelist">
      <AScrollbar style="height: 52px; overflow: auto; width: 100%">
        <ul class="cateul">
          <li class="cate_item" :class="{ select: cid == 0 }" @click="selectCate(0)">全部</li>
          <template v-for="item in picturecate">
    ...
    <ASpin :loading="loading" tip="加载中..." style="width: 100%">
      <div class="file_list">
        <AScrollbar v-if="picture && picture.length > 0" height="435px">
          <div class="file_list_box">
            <div v-for="item in picture" :key="item.id" class="file_item">
              <div>
                <div class="imgbox" @click="onSelectImg(item)">
                  ...
                  <div
                    class="edit_layer"
                    :class="{
                      l_active: selectList.includes(`mt_${item.id}`)
                    }"
                  >
                    <Icon icon="svgfont-mark_sel" class="choose" :size="18" />
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div v-if="pagetotall && pagetotall > 14" class="mypagination">
            <APagination v-model:current="pagenum" :total="pagetotall" show-total show-jumper @change="changePage" />
          </div>
        </AScrollbar>
        <!--空-->
        <div v-if="!loading && (!picture || picture.length == 0)" class="emptylist" :style="{ height: `435px` }">
          <div class="emptybox">
            <AEmpty description="暂时没有该类素材，去看看其他类型吧！" />
          </div>
        </div>
      </div>
    </ASpin>
...
<script lang="ts">
// api
import type { CateItem, PictureItem } from '@/api/datacenter/attachment';
import { getPictureCate, get_picture } from '@/api/datacenter/attachment';
...
export default defineComponent({
  ...
  setup(_, { emit }) {
    ...
    const fetchData = async (searchword: string) => {
      setLoading(true);
      try {
        const getdbdata = await get_picture({
          searchword,
    ...
    const onSelectImg = (item: any) => {
      const id_str = `mt_${item.id}`;
      if (pagedata.selectList.includes(id_str)) {
        pagedata.selectList = pagedata.selectList.filter(id => id != id_str);
      } else if (pagedata.getnumber == 'one') {
