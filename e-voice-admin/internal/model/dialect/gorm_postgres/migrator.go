	...
	err = m.RunWithValue(value, func(stmt *gorm.Statement) error {
		var (
			...
			columns, err         = m.DB.Raw(
				"SELECT c.column_name, c.is_nullable = 'YES' is_nullable, c.udt_name, c.character_maximum_length, c.numeric_precision, c.numeric_precision_radix, c.numeric_scale, c.datetime_precision, 8 * typlen typlen, c.column_default, pd.description, c.identity_increment FROM information_schema.columns AS c JOIN pg_type AS pgt ON c.udt_name = pgt.typname LEFT JOIN pg_catalog.pg_description as pd ON pd.objsubid = c.ordinal_position AND pd.objoid = (SELECT oid FROM pg_catalog.pg_class WHERE relname = c.table_name AND relnamespace = (SELECT oid FROM pg_catalog.pg_namespace WHERE nspname = c.table_schema)) where table_catalog = ? AND table_schema = ? AND table_name = ?",
		...
		// check primary, unique field
		{
			...
			for columnTypeRows.Next() {
				...
				_ = columnTypeRows.Scan(&constraintName)
...
func (m Migrator) GetRows(currentSchema interface{}, table interface{}) (*sql.Rows, error) {
	...
	tx := m.DB.Session(&gorm.Session{}).Table(name).Limit(1)
