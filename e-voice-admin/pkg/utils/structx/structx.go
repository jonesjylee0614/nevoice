...
func ToMap(elem reflect.Value, elemMap map[string]interface{}) {
	// 遍历结构体的每个字段
	for j := 0; j < elem.NumField(); j++ {
		field := elem.Type().Field(j)
		// 如果字段是结构体，则递归调用
		if field.Type.Kind() == reflect.Struct {
			ToMap(elem.Field(j), elemMap)
			continue
		}
...
func init() {
	arr := []reflect.Type{
		...
		reflect.TypeOf(json2.JsonTime{}),
		reflect.TypeOf(json2.JsonInt64(1)),
	}

	for _, t := range arr {
		singleTypes = append(singleTypes, fmt.Sprintf("%s.%s", t.PkgPath(), t.Name()))
	}
}
...
func SetDefaults(obj interface{}) {
	...
	if t.Kind() == reflect.Slice || t.Kind() == reflect.Array {
		for i := 0; i < v.Len(); i++ {
			SetDefaults(v.Index(i).Addr().Interface())
		}
		return
	}
	...
	if t.Kind() == reflect.Map {
		for _, k := range v.MapKeys() {
			...
			if mapValue.Kind() == reflect.Struct {
				...
				newStruct.Elem().Set(mapValue)
				// 调用 SetDefaults
				SetDefaults(newStruct.Interface())
				// 如果需要更新回 map，可以这样做：
				v.SetMapIndex(k, newStruct.Elem())
			}
		}
		return
	}
	...
	for i := 0; i < t.NumField(); i++ {
		...
		// 如果字段是结构体，则递归调用
		if field.Type.Kind() == reflect.Struct && !collx.ArrayAnyContains(singleTypes, fmt.Sprintf("%s.%s", field.Type.PkgPath(), field.Name)) {
			SetDefaults(value.Addr().Interface())
			continue
		}
		if field.Type.Kind() == reflect.Pointer && value.Elem().Kind() == reflect.Struct {
			SetDefaults(value.Interface())
			continue
		}
		// 如果字段是map，则判断map的value，如果value是结构体，则递归调用
		if field.Type.Kind() == reflect.Map {
			for _, k := range value.MapKeys() {
				...
				if mapValue.Kind() == reflect.Struct {
					...
					SetDefaults(newStruct.Interface())
					// 如果需要更新回 map，可以这样做：
					value.SetMapIndex(k, newStruct.Elem())
				}
			}
			continue
		}

		// 如果字段是数组或切片，且数组对象是结构体，则递归调用
		if (field.Type.Kind() == reflect.Array || field.Type.Kind() == reflect.Slice) && field.Type.Elem().Kind() == reflect.Struct {
			for j := 0; j < value.Len(); j++ {
				SetDefaults(value.Index(j).Addr().Interface())
			}
			continue
		}
		...
		// 忽略默认值为0值的字段
		tagValue, _ := field.Tag.Lookup("default")
		if tagValue == "" {
			continue
		}
		...
		case bool:
			...
			// 这里可以添加更多类型处理
		case *json2.JsonInt64:
			intValue, err := strconv.ParseInt(tagValue, 10, 64)
			if err == nil {
				value.Set(reflect.ValueOf(json2.NewJsonInt64(intValue)))
			}
		case json2.JsonInt64:
			intValue, err := strconv.ParseInt(tagValue, 10, 64)
			if err == nil {
		...
		case *json2.JsonTime:
			value.Set(reflect.ValueOf(json2.NewJsonTime(timex.ParseTime(tagValue))))
		case json2.JsonTime:
			value.Set(reflect.ValueOf(*json2.NewJsonTime(timex.ParseTime(tagValue))))
		default:
		}
