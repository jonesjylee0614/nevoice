...
import (
	"gofly/pkg/captcha/capts/base/helper"
	"gofly/pkg/logx"
	"image"
			...
			for _, char := range chars {
				if helper.IsChineseChar(char) {
					if helper.LenChineseChar(char) > 1 {
						logx.Errorf("WithChars error: the chinese char [%s] must be equal to 1", char)
						return
					}
				} else if helper.LenChineseChar(char) > 2 {
					logx.Errorf("WithChars error: the char [%s] must be less than or equal to 2", char)
					return
				}
			}
		}
