...
func WithRangeColors(colors []string) Option {
	return func(opts *Options) {
		if len(colors) > 255 {
			logx.Warnf("withRangeColors(): %v", ColorLenErr)
			return
		}
...
func WithRangeVerifyLen(val option.RangeVal) Option {
	return func(opts *Options) {
		if val.Max > opts.rangeLen.Min {
			logx.Warnf("withRangeVerifyLen(): %v", RangeVerifyLenErr)
			return
		}
...
func WithRangeThumbColors(val []string) Option {
	return func(opts *Options) {
		if len(val) > 255 {
			logx.Warnf("WithRangeThumbColors(): %v", ColorLenErr)
			return
		}
...
func WithRangeThumbBgColors(val []string) Option {
	return func(opts *Options) {
		if len(val) > 255 {
			logx.Warnf("withRangeThumbBgColors(): %v", ColorLenErr)
			return
		}
