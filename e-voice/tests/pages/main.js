...
// nevoice 修改：智能设置 WebSocket 地址
// - 如果通过 file:// 打开，使用默认地址
// - 如果通过 http(s):// 打开，自动推断 WebSocket 地址
var now_ipaddress;
if (window.location.protocol === 'file:') {
    // 直接打开 HTML 文件，使用默认地址
    now_ipaddress = 'ws://127.0.0.1:10096/ws/recognize';
} else {
    // 通过 HTTP 服务器访问，自动推断地址
    var wsProtocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
    var wsHost = window.location.hostname || '127.0.0.1';
    var wsPort = '10096';  // nevoice WebSocket 服务端口
    now_ipaddress = wsProtocol + '//' + wsHost + ':' + wsPort + '/ws/recognize';
}
...
function stop() {
		var chunk_size = new Array( 5, 10, 5 );  // FunASR 官方默认配置
		var request = {
